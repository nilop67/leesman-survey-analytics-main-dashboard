...
const regions = ["Europe", "Oceania", "Asia", "Africa", "North America", "Antarctica", "South America"];
// Example result:
// {
//   cascade: [
//     {
...
export function getRegionByShortCountryCode(country) {
  const countryName = CountryRegionData.filter((i) => i.countryShortCode === country)[0].countryName;
...
export function getRegionCountryCityTreeData(regions) {
  ...
  countries.forEach((i) => {
    CountryRegionData.filter((item) => item.countryShortCode === i.key)[0].regions.forEach((e) => {
...
export const findTextFromTranslations = (type, language_id, allLabels) => {
  return (
    allLabels?.filter((i) => i.type?.capitalize() === type?.capitalize() && i.language_id === parseInt(language_id))[0]?.label ||
    type
  );
};
export const findDepartmentFromTranslations = (department_id, label, language_id, allLabels) => {
  return allLabels?.filter((i) => i.department_id === department_id && i.language_id === parseInt(language_id))[0]?.label || label;
};
export const findLocationFromTranslations = (location_id, label, language_id, allLabels) => {
  return allLabels?.filter((i) => i.location_id === location_id && i.language_id === parseInt(language_id))[0]?.label || label;
};
export const findQuestionFromTranslations = (question_id, label, language_id, allLabels) => {
  return allLabels?.filter((i) => i.question_id === question_id && i.language_id === parseInt(language_id))[0]?.label;
};
export const findHelpTextFromTranslations = (question_id, label, language_id, allLabels) => {
  return allLabels?.filter((i) => i.question_id === question_id && i.language_id === parseInt(language_id))[0]?.help_text || label;
};
export const findHelpText2FromTranslations = (question_id, label, language_id, allLabels) => {
  return (
    allLabels?.filter((i) => i.question_id === question_id && i.language_id === parseInt(language_id))[0]?.help_text_2 || label
  );
};
export const findHelpTextFromOptionTranslations = (option_id, label, language_id, allLabels) => {
  return allLabels?.filter((i) => i.option_id === option_id && i.language_id === parseInt(language_id))[0]?.help_text || label;
};
export const findHelpText2FromOptionTranslations = (option_id, label, language_id, allLabels) => {
  return allLabels?.filter((i) => i.option_id === option_id && i.language_id === parseInt(language_id))[0]?.help_text_2 || label;
};
export const findOptionFromTranslations = (option_id, label, language_id, allLabels) => {
  return allLabels?.filter((i) => i.option_id === option_id && i.language_id === parseInt(language_id))[0]?.label;
};
export const findPageFromTranslations = (page_id, label, language_id, allLabels) => {
  return allLabels?.filter((i) => i.page_id === page_id && i.language_id === parseInt(language_id))[0]?.heading;
};
export const findTranslationFromData = (id, label, language_id, allLabels) => {
  ...
  if (parseInt(language_id) === 12) {
    ...
    if (!findPageFromTranslations(id, label, language_id, allLabels?.pageTranslations)) {
      //not found
      if (!findQuestionFromTranslations(id, label, language_id, allLabels?.questionTranslations)) {
        //not found

        if (!findOptionFromTranslations(id, label, language_id, allLabels?.optionTranslations)) {
          //not found
          return label;
        } else {
          return findOptionFromTranslations(id, label, language_id, allLabels?.optionTranslations);
        }
      } else {
        return findQuestionFromTranslations(id, label, language_id, allLabels?.questionTranslations);
      }
    } else {
      return findPageFromTranslations(id, label, language_id, allLabels?.pageTranslations);
    }
  }
};
...
export const modifyDates = (startDate, endDate, text) => {
  text = endDate ? text.replace("%end%", moment(endDate).format("YYYY-MM-DD")) : text.replace("%end%", "");
  text = startDate ? text.replace("%start%", moment(startDate).format("YYYY-MM-DD")) : text.replace("%start%", "");
  return text;
};
export const getCampaingCurrentStatus = (record) => {
  let status = "";
  if (record.status === "active") {
    if (record.end_date && moment(record.end_date).isBefore(moment().add(1, "days"))) {
      status = "CLOSED";
...
Array.prototype.isSubset = function (arr2) {
  return this.every((identifier) => arr2?.findIndex((identifier1) => identifier1 === identifier) >= 0);
};
Array.prototype.isParentset = function (arr2) {
  return arr2.every((identifier) => this?.findIndex((identifier1) => identifier1 === identifier) >= 0);
};
...
String.prototype.removeWhiteSpaces = () => {
  return this && this.length > 0 && this.replace(/^[ ]+/g, "").replace(/[ ]+/g, "");
};
...
export function makeUniqueWithDoubleAttrubes(data, att1, att2) {
  ...
  return arr.map((i) => data.filter((o) => i[att1] === o[att1] && i[att2] === o[att2])[0]);
}
export function makeUniqueWithTripleAttrubes(data, att1, att2, att3) {
  ...
  return arr.map((i) => data.filter((o) => i[att1] === o[att1] && i[att2] === o[att2] && i[att3] === o[att3])[0]);
}
export function makeUniqueWithfourthAttributes(data, att1, att2, att3, att4) {
  ...
  return arr.map(
    (i) => data.filter((o) => i[att1] === o[att1] && i[att2] === o[att2] && i[att3] === o[att3] && i[att4] === o[att4])[0]
  );
}
export function makeUniqueWithAttributeArr(data, attributeArr) {
  ...
  return arr.map((i) => data.filter((o) => arr.every(({ field, value }) => o[field] === i[field]))[0]);
}
