    ...
    <div className="col-12 col-xl-6 col-lg-12 col-md-12 pt-20">
      <div className="row headline-stat-closed">
        <div className="col-lg-4 col-sm-12 max-h-200">
          <h4 className="font-16r text-white  text-leftt">{data?.stats?.responses}</h4>
          <label className="mb-0  text-leftt">Responses</label>
        </div>
        <div className="col-lg-4 col-sm-12 max-h-200">
          <h4 className="font-16r text-white  text-leftt">{data?.uniqueUserCount}</h4>
        ...
        <div className="col-lg-6 col-sm-12 max-h-200 pt-40">
          <h4 className="font-16r text-white  text-leftt">{moment(campaignDetails?.start).format("DD MMM YY")}</h4>
          <label className="mb-0  text-leftt">Start date</label>
        </div>
        <div className="col-lg-6 col-sm-12 max-h-200 pt-40">
          <h4 className="font-16r text-white  text-leftt">{moment(campaignDetails?.end).format("DD MMM YY")}</h4>
        ...
        <div className="col-lg-6 col-sm-12">
          {!data?.available ? (
            ...
            <div className="donut-new donut-new-main" style={{ height: "200px" }}>
              <ProgressBar
                radius={70}
                progress={parseInt(data && data.stats && data.progress) >= 0 ? parseInt(data && data.stats && data.progress) : 0}
                strokeWidth={7}
                strokeColor="#fff"
              ...
              >
                <div className="indicator i-small">
                  ...
                  <div>
                    {parseInt(data && data.stats && data.progress) >= 0 ? parseInt(data && data.stats && data.progress) : 0}%
                  </div>
                </div>
              </ProgressBar>

              <span className="mb-0 sub-title small-chart-link">Campaign progressbar</span>
            </div>
          )}
        </div>
          ...
          {!data?.available ? (
            ...
            <div className="donut-new donut-new-main" style={{ height: "200px" }}>
              <ProgressBar
                radius={70}
                progress={
                  parseInt(data && data.stats && data.response_rate) >= 0 ? parseInt(data && data.stats && data.response_rate) : 0
                }
                strokeWidth={7}
                strokeColor="#fff"
                strokeLinecap="square"
              ...
              <span className="mb-0 sub-title small-chart-link">Response rate</span>
      ...
      <div className="row text-center campaigns-details">
        <div className="col-12 col-xl-6 col-lg-12 col-md-12 pt-20">
          <p className="campaign-details-text">
            ...
            <br />
            Number of locations: {campaignDetails?.locations?.length - campaignDetails?.commonLocations?.length}
            <br />
