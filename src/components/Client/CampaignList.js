...
const CampaignList = (props) => {
  ...
  const columns = [
    ...
    },
    {
      ...
      sorter: (a, b) => {
        if (props.getStatus(a).toLowerCase() < props.getStatus(b).toLowerCase()) return -1;
        if (props.getStatus(a).toLowerCase() > props.getStatus(b).toLowerCase()) return 1;
        return 0;
      },
    ...
    },
    {
      ...
      width: 110,
      render: (text) => <a>{text ? moment(text.splitDate()).format("DD MMM YY") : ""}</a>,
    ...
    },
    {
      ...
      render: (text, record) => (
        <>
          ...
          <Link to={`/campaign-details?campaign_id=${record.id}`} className="btn btn-sm btn-liste cursorp" title="Result">
          ...
          {props.role !== "Admin" && (moment(record.end_date).isAfter(moment()) || !record.end_date) && (
            <Link
  ...
  return (
    <>
      ...
      />
      <Drawer setIsVisible={setIsDetail} visible={isDetail} data={selectedRow}></Drawer>
    </>
  );
};
