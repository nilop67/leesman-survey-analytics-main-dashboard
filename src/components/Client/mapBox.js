...
import MapGL, { NavigationControl, Marker, Popup, GeolocateControl, FlyToInterpolator } from "react-map-gl";
  ...
  renderPopup() {
    return (
      ...
      (this.state.popupInfo.xp === null ? (
        ...
        <>
          <Popup tipSize={5} anchor="bottom-right" longitude={this.state.popupInfo.long} latitude={this.state.popupInfo.lat}>
            <p>
            ...
            <div className="tab-pane map-tooltips">
              <div className="form-group">
                <label className="d-block">
                  XP Score <span className="float-right">{parseFloat(this.state.popupInfo.xp).toFixed(1)}</span>
                </label>
                  ...
                  <div
                    className={`progress-bar ${
                      this.state.popupInfo.xp <= 20
                        ...
                        : this.state.popupInfo.xp > 20 && this.state.popupInfo.xp <= 40
                        ? " progress-bar-orange-map"
                        : this.state.popupInfo.xp > 40 && this.state.popupInfo.xp <= 60
                        ? " progress-bar-orange-soft-map"
                        : this.state.popupInfo.xp > 60 && this.state.popupInfo.xp <= 80
              ...
              <div className="form-group">
                <label className="d-block">
                  No. of active users{" "}
                  <span className="float-right">
                    {this.state.popupInfo.users} <i className="fa fa-long-arrow-up"></i>
                  </span>
                </label>
                <div className="progress progress-xxs">
                  <div
      ...
      (this.state.summaryModal.xp === null ? (
        ...
        <>
          <Modal
            ...
            footer={[
              <Link
                to={{ pathname: "/results", location_id: this.state.summaryModal.id }}
          ...
          >
            <div className="row clearfix">
              <div className="col-xl-4 col-lg-6 col-md-6 more-group-filters  a--summary-s">
                ...
                <div className="donut-new donut-new-main" style={{ height: "200px" }}>
                  <ProgressBar
                    radius={90}
                    ...
                    strokeColor={
                      parseFloat(this.state.summaryModal.xp).toFixed(1) <= 20
                        ? "#bf1717"
                        : parseFloat(this.state.summaryModal.xp).toFixed(1) > 20 &&
                          parseFloat(this.state.summaryModal.xp).toFixed(1) <= 40
                        ? "#e95c0c"
                        : parseFloat(this.state.summaryModal.xp).toFixed(1) > 40 &&
                          parseFloat(this.state.summaryModal.xp).toFixed(1) <= 60
                        ? "#FFE054"
                        : parseFloat(this.state.summaryModal.xp).toFixed(1) > 60 &&
                          parseFloat(this.state.summaryModal.xp).toFixed(1) <= 80
                        ? "#63B450"
                        : "#119336"
                    }
                  ...
                  >
                    <Tooltip title="Employee experience score" placement="right">
                      <div className="progress-hover-info is--campaign"></div>
                    </Tooltip>
                    <div className="indicator">
                      <div>{parseFloat(this.state.summaryModal.xp).toFixed(1)}</div>
                    </div>
                  </ProgressBar>
                </div>
                <div className="row info-section-b">
                  <div className="col-lg-6 col-sm-6 max-h-200">
                    <h4 className="font-16r text-white text-leftt">{this.state.summaryModal.users}</h4>
                    <label className="mb-0 font-1r text-leftt light-black">Registered users</label>
                  </div>
                  <div className="col-lg-6 col-sm-6 max-h-200">
                    <h4 className="font-16r text-white text-leftt">{this.state.summaryModal.engaged}</h4>
                    <label className="mb-0 font-1r text-leftt light-black">Active users</label>
                  </div>
                </div>
              </div>
              <div className="col-xl-4 col-lg-6 col-md-6 more-group-filters  b--summary-s">
                ...
                <div className="donut-new donut-new-main" style={{ height: "200px" }}>
                  <ProgressBar
                    radius={90}
                    progress={this.state.summaryModal.comment >= 0 ? this.state.summaryModal.comment : 0}
                    strokeWidth={8}
                    strokeColor={
                      this.state.summaryModal.comment <= 14
                        ? "#bf1717"
                        : this.state.summaryModal.comment > 14 && this.state.summaryModal.comment <= 28
                        ? "#e95c0c"
                        : this.state.summaryModal.comment > 28 && this.state.summaryModal.comment <= 42
                        ? "#f5a04c"
                        : this.state.summaryModal.comment > 42 && this.state.summaryModal.comment <= 56
                        ? "#ffe054"
                        : this.state.summaryModal.comment > 56 && this.state.summaryModal.comment <= 70
                        ? "#b6d484"
                        : this.state.summaryModal.comment > 70 && this.state.summaryModal.comment <= 84
                        ? "#63b450"
                        : "#119336"
                    }
                    strokeLinecap="square"
                  ...
                  >
                    ...
                    <div className="indicator">
                      <div>
                        {parseFloat(this.state.summaryModal.comment).toFixed(1) >= 0
                          ? parseFloat(this.state.summaryModal.comment).toFixed(1)
                          : 0}
                        %
                      </div>
                    </div>
              ...
              <div className="col-xl-4 col-lg-12 col-md-12 more-group-filters c--summary-s">
                ...
                {this.state.summaryModal.campaigns
                  ?.filter((i) => i.status === "Live" || i.status === "Always on")
                  ?.sort((a, b) => a.id - b.id)
                  .slice(0, 3)
                  .map((i) => {
                    return (
                      <div className="response-section pb--23">
                        <div className="row clearfix">
                          <div className="col-9">
                            <h6 className="score-title-left text-nowrap">{truncate(i.title, 40)}</h6>
                          </div>
                          <div className="col-3">
                            <h6 className="score-title-right">{parseFloat(i.avg).toFixed(1)}%</h6>
                          </div>
                        </div>
                ...
                <div className="d-flex justify-content-between align-items-center info-section-c">
                  <div>
                    <h2 className="card-title mb-0">Recently closed campaigns</h2>
                  </div>
                </div>
      ...
      {this.state.loader ? <LoaderLarge /> : <></>}
        ...
        >
          <div className="nav" style={navStyle}>
            <NavigationControl onViewportChange={this.zoomInOutViewportChange} />
            <GeolocateControl
