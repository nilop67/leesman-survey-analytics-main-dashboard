...
import { feedbackClientDataViewAction , feedbackJoinDataViewAction} from "../../actions/clientActions";
const chartOptions = {
...
const FeedbackDataView = () => {
  ...
  const {clientData, error } = useSelector(store => store.feedbackClientDataViewReducer)
  const {joinData } = useSelector(store => store.feedbackJoinDataViewReducer)
  useEffect(() => {
    if (window.location.pathname === "/feedback-data-view" && document.querySelector("#comments-_link") !== null) {
  ...
  useEffect(() => {
    clientData.length > 0 && joinData?.themeParentsJoin?.length > 0 && CalcData();
  ...
  useEffect(() => {
    scales.length > 0 && ApexCharts.exec("treeChart", "updateSeries", scales.series, true);
  }, [scales]);
  const getData = () => {
    let fdata = {
      ...filterData,
  ...
  const calcThemeData = () => {
    let uniqueThemeIds = [...new Set(joinData.themeParentsJoin.map((i) => i.theme_id))];

    let uniqueThemes = uniqueThemeIds.map((item, index) => {
      let parents = joinData.themeParentsJoin.filter((i) => i.theme_id === item)[0].parent_ids;

      let score = clientData.filter((i) => parents.includes(i.parent_id) && i.answer_bool).length;
      let singleTheme = clientData.filter((i) => parents.includes(i.parent_id));
      let all = singleTheme.length;
      let innerscore = (100 * score) / all;
      return {
        id: item,
        label: joinData.themeParentsJoin.filter((i) => i.theme_id === item)[0].label,
  ...
  const calcParentData = () => {
    ...
    let uniqueParentIds = [...new Set(joinData.parentThemeJoin.map((i) => i.parent_id))];
    let uniqueParents = uniqueParentIds.map((item) => {
      let score = clientData.filter((i) => i.parent_id === item && i.answer_bool).length;
      ...
      return {
        id: item,
        label: joinData.parentThemeJoin.filter((i) => i.parent_id === item)[0].label,
    ...
    if (selectedIndex !== "") {
      console.log(selectedIndex);
      const getThemeId = joinData.themeParentsJoin.filter((i) => i.label.includes(selectedIndex))[0].theme_id;
      const getConnectedParents = joinData.parentThemeJoin.filter((i) => i.theme_ids.includes(getThemeId));
      console.log(getConnectedParents, getThemeId);
      uniqueParents = uniqueParents.filter((i) => getConnectedParents.map((o) => o.parent_id).includes(i.id) && i.weight > 0);
    }
  ...
  const calcTagsData = () => {
    ...
    let uniqueTags = uniqueTagIds.map((item) => {
      let score = clientData.filter((i) => i.tag_id === item && i.answer_bool).length;
    ...
    if (selectedParentLabel !== "") {
      uniqueTags = uniqueTags.filter((i) => i.parent_label === selectedParentLabel);
    }
        ...
        chart: {
          ...
          events: {
            click: function (event, chartContext, config) {
              ...
              //console.log(graphState);
              setSelectedIndex(config.globals.categoryLabels[config.dataPointIndex]);
              console.log(config);
              console.log(config.globals.categoryLabels[config.dataPointIndex]);
              setSelectedParentLabel(config.globals.categoryLabels[config.dataPointIndex]);
            },
          },
        },
        dataLabels: { enabled: true, style: { fontSize: "12px" }, offsetY: -4 },
          ...
          <div className="row clearfix control--nav">
            <div className="col-md-3 col-sm-12 user-feedback-top">
              ...
              <p className="small-_info for-title">
                Size of each box represents the volume of comments and the colour represents the Comments score
              </p>
            </div>
          ...
          <div className="col-md-12 col-sm-12 mb-3">
            <ul className="nav">
              <li className="nav-item mr--8">
                <Link to="/results-feedbacks" className="btn btn-default btn-block">
                  Summary
                </Link>
              </li>
              <li className="nav-item mr--8">
                <Link to="/feedback-theme-view" className="btn btn-default btn-block">
                  Themes
                </Link>
              </li>
              <li className="nav-item mr--8">
                <Link to="/feedback-data-view" className="btn btn-default active btn-block">
                  Theme breakdown
                </Link>
              </li>
              <li className="nav-item">
                <Link to="/feedback-comments-view" className="btn btn-default btn-block">
                  Comments
                </Link>
              </li>
              <li>
          ...
          <div className="col-md-12 col-sm-12">
            <div className="user-feedback-top data-view-buttons">
              <button
                ...
                className={`ant-btn ant-btn-sm ${graphState === "theme" ? "ant-btn-primary" : "ant-btn-default"}`}
              >
                Theme
              </button>
              <button
                ...
                className={`ant-btn ant-btn-sm ml-1 ${graphState === "parent" ? "ant-btn-primary" : "ant-btn-default"}`}
              >
                Parent
              </button>
              <button
                ...
                className={`ant-btn ant-btn-sm ml-1 ${graphState === "tags" ? "ant-btn-primary" : "ant-btn-default"}`}
              >
                Tags
              </button>
            </div>
          ...
          <div className="col-lg-12 col-md-12 leesman-section  text-center">
            <div className="card">
              <div className="body min-h-290">
                {loading ? <Loader /> : scales.series.length > 0 && <TreeMapChart data={scales} />}
              </div>
            </div>
          </div>
        </div>
