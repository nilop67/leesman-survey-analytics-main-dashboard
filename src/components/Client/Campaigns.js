...
import { campaignsGrafAction , bufferCampaignsAction, campaignsAction} from "../../actions/clientActions";
...
const Campaigns = (props) => {
  ...
  const {bufferCampaigns} = useSelector(store => store.bufferCampaignsReducer)
  ...
  const filterFeedbacks = (e) => {
    ...
    campaigns.forEach((item) => {
      if (
        ...
        String(getCampaingCurrentStatus(item)).toLowerCase().includes(e.toLowerCase()) ||
        ...
        String(moment(item.start_date.splitDate()).format("DD MMM")).toLowerCase().includes(e.toLowerCase()) ||
        (item.end_date && String(moment(item.end_date.splitDate()).format("DD MMM")).toLowerCase().includes(e.toLowerCase())) ||
        ...
        <div className="block-header">
          <div className="row clearfix">
            <div className="col-md-12 col-sm-12">
              {role !== "Admin" ? <h1>Admin management</h1> : <h1>Campaigns</h1>}
              {/* {("the role is", role)} */}
              ...
              {role !== "Admin" ? (
                ...
                <p className="small-_info for-title">Sets of questions aimed to explore a particular topic in more depth</p>
              )}
            </div>
          </div>
        </div>
                  ...
                  <div className="body scroll-campaigns">
                    <div className="d-flex justify-content-between align-items-center">
                      <div>
                        ...
                        <p className="small-_info">Current score for each live campaign</p>
                      </div>
                    </div>
                        ...
                        {loading ? (
                          ...
                          <Slider {...slideSettings}>
                            ...
                            {graphs &&
                              graphs.length > 0 &&
                              graphs
                                .filter((i) => i.status === "Always on" || i.status === "Live")
                                .map((i, index) => {
                                  return (
                                    <div key={index} className="campaign-multi-small">
                                      <div className="donut-new donut-new-main" style={{ height: "180px" }}>
                                        <ProgressBar
                                          radius={80}
                                          progress={parseFloat(i.avg).toFixed(1)}
                                          strokeWidth={8}
                                          strokeColor={i.status === "Always on" ? "#2793ff" : "#fff"}
                                        ...
                                        <span className="mb-0 sub-title donut-txt-b">Status: {i.status}</span>
                                      </div>
                                    </div>
                                  );
                                })}
                      ...
                      <div className="col-lg-12 col-sm-12">
                        <nav className="d-flex text-muted top-icons-bar float-leftt chart-keys">
                          <span className="icon mr-3 font-13px d-flex">
                            <i className="iconx-controller-stop icon-blue" /> Always on campaign
                          </span>
                          <span className="icon mr-3 font-13px d-flex">
                            <i className="iconx-controller-stop icon-white" /> Active campaign
                          </span>
                        </nav>
                      </div>
                    </div>
                  ...
                  <div className="body scroll-campaigns">
                    <div className="d-flex justify-content-between align-items-center">
                      <div>
                        ...
                        <p className="small-_info">Finalised score for each completed campaign</p>
                      </div>
                    </div>
                                  ...
                                  <div className="campaign-multi-small">
                                    <div className="donut-new donut-new-main" style={{ height: "180px" }}>
                                      <ProgressBar
                                        ...
                                        progress={parseFloat(i.avg).toFixed(1)}
                                        strokeWidth={8}
                                        strokeColor={i.avg <= 33 ? "#bf1717" : i.avg > 33 && i.avg <= 66 ? "#f5a04c" : "#119336"}
                                        strokeLinecap="square"
                                        trackStrokeColor="#70777a"
                                        trackStrokeWidth={5}
                                        transition="0.3s ease"
                                      ...
                                      <span className="mb-0 sub-title donut-txt-b">Close date: {i.end}</span>
                                    </div>
                                  </div>
                                );
                              })}
              ...
              <div className="col-xl-12 col-lg-12 col-md-12">
                <div className="row mb-4 page-__header">
                  ...
                  <div className="col-xl-6 col-lg-6 col-md-6 jus-end">
                    {role !== "Admin" && clientId && (
                      <Link to={`/create-campaign?client_id=${clientId}`} className="btn btn-primary ml-3" title="">
                        New campaign
                      </Link>
                    )}
                    <ImportExport refresh={getData} type={"campaigns"} data={campaigns} import={false} export={true} />
                    <div className="input-group ml-3">
                      ...
                      {/* yeni arama */}
                      <input type="text" className="form-control" value={val} onChange={change} placeholder="Search" />
                    </div>
                  </div>
                </div>
              </div>
