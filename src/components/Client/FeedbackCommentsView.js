...
const FeedbackCommentsView = (props) => {
  ...
  const {userFeedback, error} = useSelector(store => store.feedbackCommentsViewReducer)
  useEffect(() => {
    if (window.location.pathname === "/feedback-comments-view" && document.querySelector("#comments-_link") !== null) {
  ...
  const populateFeedbacks = () => {
    ...
    if (locationUrl && isPositiveUrl) {
      ...
      e = e.filter((i) => i.ispositive === bool && i.location_type === locationUrl);
    }
    ...
    e.map((item, i) => {
      if (item.ispositive) {
        positiveBuffer.push(
          <Panel header={item.text} key={i} className="collapse-Positive-status">
            {/* Açılan alan */}
            ...
            <div className="row text-leftt">
              ...
              <div className="col-lg-6 col-6">
                <p className="question-text float-r">Date: {item.date.splitDate()}</p>
              </div>
            </div>
          </Panel>
        );
        ...
        negativeBuffer.push(
          <Panel header={item.text} key={i} className="collapse-Negative-status">
            {/* Açılan alan */}
            <div className="row text-leftt pt-20">
              <div className="col-lg-12">
                <h6 className="score-title-left">
  ...
  return (
    <>
      ...
      {/* MODAL PENCERESİ */}
      <div className={`map-results-modal overlay ${openModal === "active" ? " d-block show" : ""}`}>
        {/* FEEDBACK EĞER POZİTİFSE positive-feed / NEGATİFSE negative-feed ETİKETİNE SAHİP OLUR */}
        <div className={`card feedback-results ${FeedbackData.ispositive === true ? " positive-feed" : " negative-feed"}`}>
          <div className="body">
            <div className="d-flex justify-content-between align-items-center feedbacks-header">
              <div>
                {/* O ANKİ TARİH GÖSTERİLECEK */}
                <label className="mb-0 card-info">Date: {FeedbackData.date}</label>
              </div>
              <ul className="nav">
          ...
          <div className="body card-bottom-section">
            <div className="row">
              ...
              <div className="col-lg-6 col-sm-12">
                <label className="mb-0 card-info float-r">{FeedbackData.location}</label>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="block-header">
          <div className="row clearfix control--nav">
            <div className="col-md-3 col-sm-12 user-feedback-top">
              <h1>Comments</h1>
              <p className="small-_info for-title">Comments submitted by employees</p>
            </div>
        ...
        <div className="row clearfix" onScroll={handleScroll}>
          <div className="col-md-12 col-sm-12 mb-3">
            <ul className="nav">
              ...
              <li className="nav-item">
                <Link to="/feedback-comments-view" className="btn btn-default active btn-block">
                  Comments
                </Link>
              </li>
            </ul>
          ...
          <div className="col-md-12 col-sm-12">
            <div className="user-feedback-top data-view-buttons">
              <button
                className={`ant-btn ant-btn-sm ${type === "all" ? "ant-btn-primary" : "ant-btn-default"}`}
                onClick={() => {
                  setType("all");
                }}
              >
              ...
              <button
                className={`ant-btn ant-btn-sm ml-1 ${type === "positive" ? "ant-btn-primary" : "ant-btn-default"}`}
                onClick={() => {
                  setType("positive");
                }}
              >
              ...
              <button
                className={`ant-btn ant-btn-sm ml-1 ${type === "negative" ? "ant-btn-primary" : "ant-btn-default"}`}
                onClick={() => {
                  setType("negative");
                }}
              >
              ...
              <div className="top-small-srch">
                <div className="input-group search-top-right">
                  <input value={val} onChange={change} type="text" className="form-control" placeholder="Search..." />
          ...
          <div className="col-lg-12 col-md-12 leesman-section">
            <Collapse accordion className="all-feedbacks-collapse">
              ...
              {loading && <Loader />}
              {type === "all" ? [...positives, ...negatives] : type === "positive" ? positives : negatives}
              {!loading &&
