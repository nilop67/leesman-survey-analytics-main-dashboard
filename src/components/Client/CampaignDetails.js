...
export default function CampaignDetails({ visible, data, setIsVisible }) {
  ...
  const getData = (id) => {
    ...
    let url = role === "Admin" ? `/campaigns/labels/${id}` : `/admin/clients/${clientId}/campaigns/${id}/labels`;
  ...
  return (
    <Drawer width={640} placement="right" title="Campaign details" onClose={() => setIsVisible(false)} visible={visible}>
      {!loading ? (
        <>
          <Row style={{ marginBottom: 8 }}>
            ...
            <Col span={12}>
              ...
              <span className="d-block">{moment(data?.start_date?.splitDate()).format("DD-MM-YYYY")}</span>
            </Col>
            <Col span={12}>
          ...
          <Row style={{ marginBottom: 8 }}>
            <Col span={12}>
              ...
              <ul className="campaign-det-ul">
                {campaignData.departments && campaignData.departments.map((item) => <li>{item}</li>)}
              </ul>
            </Col>
            ...
            <Col span={12}>
              ...
              <ul className="campaign-det-ul">
                {campaignData.locations && campaignData.locations.map((item) => <li>{item}</li>)}
              </ul>
            </Col>
          </Row>
          <Divider />
          <Row>
            <Col span={12}>
              ...
              <ul className="campaign-det-ul">
                {campaignData.questions && campaignData.questions.map((item) => <li>{item}</li>)}
              </ul>
            </Col>
            <Col span={12}>
              <h6 className="d-block">Selected demographics options:</h6>
              <ul className="campaign-det-ul">{campaignData.demos && campaignData.demos.map((item) => <li>{item}</li>)}</ul>
            </Col>
          </Row>
        </>
      ) : (
