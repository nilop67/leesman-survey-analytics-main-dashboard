...
const Opentext = ({ data, currentPage, handleAnswer, answers, selectedLanguageId, initData }) => {
  const [text, setText] = useState(answers?.filter((i) => i.open_value && i.question_id == data.id)[0]?.open_value);
  useEffect(() => {
    let answer = {
    ...
    <>
      <div className="question_div" id="scroll__that">
        ...
        <h3 className="main_question">
          {findTranslationFromData(data.questions[0].id, data.questions[0].label, selectedLanguageId, initData)}
        </h3>
