...
import { findDepartmentFromTranslations, findTextFromTranslations } from "../../../../util/functions";
...
const DropdownSelect = ({
  ...
  useEffect(() => {
    let path = [];
    if (userInfo?.parent_department && userInfo?.department_id === initData?.otherOptions?.other_department?.id) {
      path = [...findCascadePath(userInfo?.parent_department, initData?.departments), userInfo?.department_id];
    } else if (userInfo?.department_id !== initData?.otherOptions?.other_department?.id) {
    ...
    setSelectedDepartmentPath(
      userInfo?.department_id
        ? userInfo?.department_id === initData?.otherOptions?.other_department?.id
          ? [...findCascadePath(userInfo?.parent_department, initData?.departments), initData?.otherOptions?.other_department?.id]
  ...
  const setSelectedThenCloseDropdown = (index, option, level, id) => {
    ...
    let selectedId = option === "Other" ? initData?.otherOptions?.other_department?.id : id;
    ...
    return (
      departments_.length > 0 && (
        <>
          {" "}
          <div
            className={`survey-dropdown_select-container ${i === 0 ? " first-_itm" : " "} ${
              arr?.length - 1 === i ? " last-_itm" : ""
            }`}
            id={item?.id}
          >
            {" "}
            <label className={isOptionsOpen?.includes(level) ? "expanded" : ""}>{tailoredName}</label>
            <div className={`survey-dropdown_select ${selection === "Please select" ? " color-_mt" : ""} `}>
              <div className="dropdown_select__items">
                ...
                >
                  {findTextFromTranslations(selection, selectedLanguageId, initData?.customisations) ||
                    findTextFromTranslations("Please select", selectedLanguageId, initData?.customisations)}
                </button>
                <ul
                  className={`options ${isOptionsOpen?.includes(level) ? "show" : ""}`}
                  role="listbox"
                  aria-activedescendant={selection}
                  tabIndex={0}
                >
                  {sortBySelection(departments_, initData?.general?.department_sort)?.map((option, index) => (
                    <li
                      id={option?.label}
                      role="option"
                      ...
                      onClick={() => {
                        setSelectedThenCloseDropdown(index, option?.label, level, option.id);
                      }}
                    >
                  ...
                  {initData?.general?.enable_other_department && (
                    <li
                      ...
                      onClick={() => {
                        setSelectedThenCloseDropdown(0, "Other", level, initData?.otherOptions?.other_department?.id);
                      }}
                    >
  ...
  const findDepartmentId = (label) => {
    return initData?.departments?.filter((i) => i.label === label) && initData?.departments.filter((i) => i.label === label)[0]?.id;
  };
      ...
      <>
        {question(
          ...
          (initData?.departmentLabels &&
            initData?.departmentLabels.filter((i) => i.language_id === selectedLanguageId)[0]?.label) ||
            findTextFromTranslations("Department", selectedLanguageId, initData?.customisations)
        )}
        {selectedDeparmentPath.map((department, index) => {
          return question(
            initData?.departments?.filter((i) => parseInt(i.parent_id) === selectedDeparmentPath[index]),
            findDepartmentLabel(selectedDeparmentPath[index + 1]),
            index + 1,
            (initData?.departmentLabels &&
              initData?.departmentLabels.filter((i) => i.language_id === selectedLanguageId)[index + 1]?.label) ||
              findTextFromTranslations("Sub department", selectedLanguageId, initData?.customisations)
          );
        })}
