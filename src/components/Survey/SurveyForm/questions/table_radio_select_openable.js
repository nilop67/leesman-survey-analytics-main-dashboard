...
import { findOptionFromTranslations, findPageFromTranslations, findTranslationFromData } from "../../../../util/functions";
import ScrollContainer from "react-indiana-drag-scroll";

const TableRadioSelectOpenable = ({ data, answers, handleAnswer, currentPage, setAnswers, selectedLanguageId, initData }) => {
  const [checkedValues, setCheckedValues] = useState(answers.filter((i) => i.page_id ===currentPage[0].id) || []);
  const [checkedOptions, setCheckedOptions] = useState(answers.filter((i) => i.page_id === currentPage[0].id) || []);
  const [checkedDepartments, setCheckedDepartments] = useState(answers.filter((i) => i.page_id === currentPage[0].id) || []);
  useEffect(() => {
    setCheckedOptions(answers.filter((i) => i.page_id === currentPage[0].id) || []);
  }, [currentPage]);
  useEffect(() => {
  ...
  function onChange(e) {
    ...
    if (arr.includes(parseInt(val))) {
      ...
      setAnswers(answers.filter((i) => !(i.page_id === currentPage[0].id && i.question_id === val)));
    } else {
  ...
  function onChangeDepartments(e) {
    ...
    if (arr.includes(parseInt(val))) {
      arr = arr.filter((i) => parseInt(i) !== parseInt(val));
      setAnswers(answers.filter((i) => !(i.page_id === currentPage[0].id && i.department_id === val)));
    } else {
      arr.push(parseInt(val));
      handleAnswer({
        module_id: currentPage[0].survey_module_id,
  ...
  useEffect(() => {
    setCheckedValues(answers?.filter((i) => i.page_id === currentPage[0].id).map((i) => i.question_id));
    setCheckedDepartments(answers?.filter((i) => i.page_id === currentPage[0].id).map((i) => i.department_id));
  }, [data, answers]);

  const handle = (question_id, option_id) => {
    setAnswers(answers.filter((i) => !(i.page_id === currentPage[0].id && i.question_id === question_id && !i.option_id)));
    handleAnswer({
      module_id: currentPage[0].survey_module_id,
      page_group_id: currentPage[0].page_group_id,
      survey_page_id: currentPage[0].survey_page_id,
  ...
  const handleDepartment = (department_id, option_id) => {
    setAnswers(answers.filter((i) => !(i.page_id === currentPage[0].id && i.department_id === department_id && !i.option_id)));
    handleAnswer({
      module_id: currentPage[0].survey_module_id,
      page_group_id: currentPage[0].page_group_id,
      survey_page_id: currentPage[0].survey_page_id,
  ...
  return (
    <>
      <div className="question_div" id="scroll__that">
        ...
        <ScrollContainer
          id="tableSC"
          className={`responsive-survey-table table--_radio  ${data?.hoverStates.length > 4 ? " scroll__table" : " "}`}
                  ...
                  .map((o) => {
                    return (
                      <th className="colored-th minus-3-color">
                        <p>{findTranslationFromData(o.option_id, o.option_label, selectedLanguageId, initData)}</p>
                      </th>
                    );
                  })}
              </tr>
                                    ...
                                    return (
                                      <td className="middle_it">
                                        <label className="container_radio">
                                          <input
                                            type="radio"
                                            name={"single-table-colored-nc-4 " + q.id}
                                            onChange={() => {
                                              handleDepartment(q.id, o.option_id);
                                            }}
                                            checked={
                                              answers.filter(
                                                (i) =>
