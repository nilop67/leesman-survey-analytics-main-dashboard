...
const TableCheckboxRadioSelect = ({
  ...
  const [values, setValues] = useState(
    answers.filter((i) => i.page_id === currentPage[0].id && i.open_value === "other").map((i) => i.option_id)
  );
  const [value, setValue] = useState(
    answers.filter((i) => i.page_id === currentPage[0].id && i.open_value === "main")[0]?.option_id
  );
  ...
  useEffect(() => {
    ...
    if (currentPage && answers) {
      arr_ = arr_?.filter((i) => !(i.page_id === currentPage[0]?.id && i.open_value === "main"));
      [value].forEach((v) => {
        arr_.push({
  ...
  useEffect(() => {
    let arr_ = answers;
    if (currentPage && answers) {
      arr_ = arr_?.filter((i) => !(i.page_id === currentPage[0]?.id && i.open_value === "other"));
      values.forEach((v) => {
        arr_.push({
          module_id: currentPage[0].survey_module_id,
          page_group_id: currentPage[0].page_group_id,
    ...
    <>
      <div className="question_div" id="scroll__that">
        ...
        <h3 className="main_question">
          {textModifier(findTranslationFromData(data.questions[0].id, data.questions[0].label, selectedLanguageId, initData))}
        </h3>

        {/* Answers */}
        <div className="responsive-survey-table table--_radio">
          <table className="survey-table-matrix check-radio nn_table" border="1" cellSpacing="0">
            {/* HEAD */}
            <thead>
              <tr className="head-style-2 hide_first-border-1">
                ...
                <th className="colored-th none-color">
                  <p>{findTextFromTranslations("Main mode", selectedLanguageId, initData?.customisations)}</p>
                </th>
                <th className="colored-th none-color">
                  <p>{findTextFromTranslations("Other modes", selectedLanguageId, initData?.customisations)}</p>
                </th>
              </tr>
            </thead>
                  ...
                  return (
                    <>
                      ...
                      <tr className="hide_in-mobile">
                        ...
                        <td></td>
                        <td colSpan="2">{findTranslationFromData(o.option_id, o.option_label, selectedLanguageId, initData)}</td>
                      </tr>
                      <tr>
