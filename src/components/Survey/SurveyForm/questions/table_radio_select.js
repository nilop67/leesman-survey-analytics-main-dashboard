...
const TableRadioSelect = ({
  ...
  const [selectedValues, setSelectedValues] = useState(answers.filter((i) => i.page_id === currentPage[0].id) || []);
  ...
  useEffect(() => {
    setSelectedValues(answers.filter((i) => i.page_id === currentPage[0].id) || []);
  }, [currentPage]);
  useEffect(() => {
    ...
    <>
      <div className="question_div" id="scroll__that" onScroll={onScroll}>
        ...
        <h3 className="main_question">
          {" "}
          {textModifier(findTranslationFromData(currentPage[0].id, currentPage[0].heading, selectedLanguageId, initData), "")}
        </h3>

        {/* Answers */}
        <div className="responsive-survey-table table--_radio has__breadcrump">
          <table className="survey-table-matrix nn_table" border="1" cellSpacing="0">
            {/* HEAD */}
            <thead>
              ...
              >
                <ul>
                  <li className="red">
                    {findTextFromTranslations(data?.hover_negative, selectedLanguageId, initData?.customisations) ===
                    data?.hover_negative
                      ? data?.hover_negative
                      : data?.lower_limit +
                        " = " +
                  ...
                  {data.neutral && (
                    <li className="muted">
                      {findTextFromTranslations(data?.hover_neutral, selectedLanguageId, initData?.customisations) ===
                      data?.hover_neutral
                        ? data?.hover_neutral
                        : "0 = " +
                          findTextFromTranslations(
                  ...
                  <li className="green">
                    {findTextFromTranslations(data?.hover_positive, selectedLanguageId, initData?.customisations) ===
                    data?.hover_positive
                      ? data?.hover_positive
                      : "+" +
                        data?.upper_limit +
                  ...
                  return (
                    <>
                      ...
                      >
                        ...
                        >
                          {findTranslationFromData(o.option_id, o.option_label, selectedLanguageId, initData)}
                          {o?.help_text && (
                        ...
                        {data.x_option && (
                          <td className="middle_it none-td">
                            <label className="container_radio cancel">
                              <input
                                type="radio"
                                name={"single-table-colored-nc-11-" + o.option_id + "-" + data.id}
                                value="n/a"
                                onChange={() => {
                                  handle(o.option_id, -4);
                                }}
                                checked={selectedValues.filter((i) => i.question_id === o.option_id && i.value === -4)?.length > 0}
                              />
                          ...
                          <td className="middle_it minus-3-td">
                            <label className="container_radio">
                              <input
                                ...
                                checked={selectedValues.filter((i) => i.question_id === o.option_id && i.value === -3)?.length > 0}
                              />
                        ...
                        <td className="middle_it minus-2-td">
                          <label className="container_radio">
                            <input
                              ...
                              checked={selectedValues.filter((i) => i.question_id === o.option_id && i.value === -2)?.length > 0}
                            />
                            <span className="checkmark"></span>
                          </label>
                        </td>
                        <td className="middle_it minus-1-td">
                          <label className="container_radio">
                            <input
                              ...
                              checked={selectedValues.filter((i) => i.question_id === o.option_id && i.value === -1)?.length > 0}
                            />
                            <span className="checkmark"></span>
                          </label>
                        </td>
                          ...
                          <td className="middle_it neutral-td">
                            <label className="container_radio">
                              <input
                                ...
                                checked={selectedValues.filter((i) => i.question_id === o.option_id && i.value === 0)?.length > 0}
                              />
                              <span className="checkmark"></span>
                            </label>
                          </td>
