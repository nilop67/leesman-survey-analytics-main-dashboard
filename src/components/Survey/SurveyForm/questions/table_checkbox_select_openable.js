...
const TableCheckboxSelectOpenable = ({
  ...
  function onChange(e) {
    ...
    if (!selectAll) {
      if (arr.includes(parseInt(val))) {
        ...
        setAnswers(answers.filter((i) => !(i.page_id === currentPage[0].id && i.question_id === parseInt(val))));
      } else {
  ...
  const handle = (question_id, option_id) => {
    ...
    if (
      answers.filter(
        (i) => i.page_id === currentPage[0].id && i.question_id === question_id && i.option_id === option_id && i.option_id
      ).length > 0
    ) {
      setAnswers(
        answers.filter((i) => !(i.page_id === currentPage[0].id && i.question_id === question_id && i.option_id === option_id))
      );
    } else {
