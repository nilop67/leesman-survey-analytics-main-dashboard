...
const SliderPercentageTotal = ({
  ...
  useEffect(() => {
    handleAnswer({
      ...
      value: decideValue(answers?.filter((i) => i.question_id === item.id)[0]?.value || 0, values, total),
    });
  }, [values]);

  useEffect(() => {
    values[0] !== answers?.filter((i) => i.question_id === item.id)[0]?.value &&
      setValues([answers?.filter((i) => i.question_id === item.id)[0]?.value || 0]);
    let t = 0;
    answers
      .filter((i) => currentPage[0]?.questions.map((q) => q.id).includes(i.question_id))
      .forEach((i) => {
        t += parseInt(i.value);
      });
