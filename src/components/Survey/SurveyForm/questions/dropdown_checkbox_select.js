...
const DropdownSelect = ({ item, i, arr, currentPage, handleAnswer, answers, data, selectedLanguageId, initData }) => {
  ...
  const setSelectedThenCloseDropdown = (index, option) => {
    ...
    let answer = {
      ...
      question_id: data.id,
      option_id: data.options.filter((i) => i.option_label.includes(option))[0]?.option_id,
      value: null,
    };
    handleAnswer(answer);
  };
  ...
  const handleListKeyDown = (e) => {
    switch (e.key) {
      ...
      case "ArrowUp":
        e.preventDefault();
        setSelectedOption(selectedOption - 1 >= 0 ? selectedOption - 1 : item.options.length - 1);
        break;
      case "ArrowDown":
        e.preventDefault();
        setSelectedOption(selectedOption === item.options.length - 1 ? 0 : selectedOption + 1);
        break;
      default:
        break;
    }
  ...
  return (
    <>
      ...
      >
        ...
        <div className={`survey-dropdown_select ${selectedOption === -1 ? " color-_mt" : ""} `}>
