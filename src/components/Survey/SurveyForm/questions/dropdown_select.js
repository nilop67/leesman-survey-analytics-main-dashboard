...
const DropdownSelect = ({ item, i, arr, currentPage, handleAnswer, answers, data, initData, selectedLanguageId }) => {
      ...
      text = reactStringReplace(text, "* ", (match, i) => {
        return (
          <Popover
            content={() =>
              InfoPopupContent(
                findHelpTextFromTranslations(data?.id, data?.help_text, selectedLanguageId, initData?.questionTranslations)
              )
            }
      ...
      text = reactStringReplace(text, "**", (match, i) => {
        return (
          <Popover
            content={() =>
              InfoPopupContent(
                findHelpText2FromTranslations(data?.id, data?.help_text_2, selectedLanguageId, initData?.questionTranslations)
              )
            }
            overlayClassName="maxt popup_i"
          >
  ...
  useEffect(() => {
    let selectedOptionId = answers?.filter((i) => i.page_id === currentPage[0].id && i.question_id === data.id)[0]?.option_id;
    console.log(
      selectedOptionId,
      answers?.filter((i) => i.page_id === currentPage[0].id)
    );

    setSelectedOption(data?.options?.filter((i) => i?.option_id === selectedOptionId)[0]?.option_label);
    ...
    let answer = {
      ...
      type:
        currentPage[0]?.name === "DEMOGRAPHICS"
          ...
          : currentPage[0]?.name,
      question_id: data.id,
      option_id: data?.options?.filter((i) => i.option_label === option)[0]?.option_id,
      value: null,
    };
    handleAnswer(answer);
  };
  ...
  return (
    <>
      <div
        className={`survey-dropdown_select-container lss_c ${i === 0 ? " first-_itm" : " "} ${
          arr?.length - 1 === i ? " last-_itm" : ""
        }`}
        id={item?.id}
      >
        <label className={isOptionsOpen ? "expanded" : ""}>
          <m>{modifyHelpText(data, findTranslationFromData(item?.id, item?.label, selectedLanguageId, initData))}</m>
        </label>
        <div className={`survey-dropdown_select ${selectedOption === "Please select" ? " color-_mt" : ""} `}>
          <div className="dropdown_select__items">
            ...
            >
              {findTranslationFromData(
                data?.options?.filter((i) => i.option_label?.includes(selectedOption))[0]?.option_id,
                selectedOption,
                selectedLanguageId,
                initData
            ...
            >
              {item?.options?.map((option, index) => (
                ...
                >
                  {findTranslationFromData(
                    data?.options?.filter((i) => i.option_label?.includes(option))[0]?.option_id,
                    option,
                    selectedLanguageId,
                    initData
                  )}
