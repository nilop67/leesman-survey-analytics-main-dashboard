...
const RadioSelect = ({
  ...
  const [selectedLocationId, setSelectedLocationId] = useState(answers?.filter((i) => i.location_id)[0]?.location_id);
  ...
  useEffect(() => {
    setLabels(initData?.locationLabels.sort((a, b) => a.level - b.level).map((i) => i.label));
    ...
    setLocationGroups(
      locationGroupIds.map((i) => {
        return {
          id: i,
          label: initData?.locations?.filter((o) => o.location_group_id === i)[0].location_group_name,
        };
      })
    );
  ...
  useEffect(() => {
    ...
    if (userInfo?.location_id && !isSelectedLocationGroup(userInfo.location_id)) {
      userInfo?.location_id !== initData?.otherOptions?.other_location?.id && setSelectedLocationId(userInfo.location_id);

      if (userInfo.floor) {
        setLocationGroupLevel(2);
      } else if (userInfo?.location_id) {
        if (userInfo?.parent_location === initData?.otherOptions?.other_location?.id) {
    ...
    console.log(
      answers.filter(
        (i) =>
          ...
          ((locationGroupLevel === 2 && i.floor === initData?.otherOptions?.other_location?.id) ||
            (locationGroupLevel === 0 && i.parent === initData?.otherOptions?.other_location?.id) ||
            (locationGroupLevel === 1 && i.option_id === initData?.otherOptions?.other_location?.id))
      ).length
    );
  }, [answers]);
  const gotoTop = () => {
  ...
  const sortBySelection = (data, type) => {
    switch (type) {
      ...
      case "client":
        return data.sort((a, b) => a.client_setup_position - b.client_setup_position);
      default:
        return data.sortAlphabetically("label");
    }
  };
  const question = (locations) => {
    ...
    let currentCount =
      locationGroupLevel === 0
        ...
        : allLocations?.filter((i) => i.location_group_id === selectedLocationId)?.length;
    ...
    return locations?.length > 0 ? (
      <>
        ...
        <div className="form-group right-_bordered">
          {sortBySelection(locations, initData?.general?.location_sort)?.map((o) => {
            return (
              <label className="container_radio version_2 survey ss">
                <input
                  ...
                  checked={
                    ...
                    (o.id === allLocations.filter((i) => i.id === selectedLocationId)[0]?.location_group_id ||
                      answers.filter((i) => i.page_id === currentPage[0].id && i.option_id === o.id).length > 0 ||
                      answers.filter((i) => i.page_id === currentPage[0].id && i.parent === o.id).length > 0 ||
                      answers.filter((i) => i.page_id === currentPage[0].id && i.floor === o.id).length > 0)
                  }
                  name={`${data.id}-${o.id}`}
                  value={o.id}
                />
                <span className="label-_text">
                  {findLocationFromTranslations(o.id, o.label, selectedLanguageId, initData?.locationTranslations)}
                </span>
                <span className="checkmark"></span>
              </label>
            );
          })}
          {((locationGroupLevel === 1 && initData?.general?.enable_other_location) ||
            (locationGroupLevel === 0 && initData?.general?.enable_other_location_group) ||
            (locationGroupLevel === 2 && initData?.general?.enable_other_floor)) && (
            <label className="container_radio version_2 survey ss">
              <input
                type="radio"
                onChange={() => {
                  ...
                  switch (locationGroupLevel) {
                    case 0:
                      setUserInfo((pre) => {
                        return {
                          ...pre,
                          parent_location: initData?.otherOptions?.other_location?.id,
                          location_id: null,
                          floor: null,
                        };
                      });
                    ...
                    case 1:
                      setUserInfo((pre) => {
                        return {
                          ...pre,
                          location_id: initData?.otherOptions?.other_location?.id,
                          floor: null,
                        };
                      });
                      //setSelectedLocationId(initData?.otherOptions?.other_location?.id);
                    ...
                    case 2:
                      setUserInfo((pre) => {
                        return { ...pre, floor: initData?.otherOptions?.other_location?.id };
                      });
                      setSelectedFloor(initData?.otherOptions?.other_location?.id);
              ...
              />
              <span className="label-_text">
                {findTextFromTranslations("Other", selectedLanguageId, initData?.customisations)}
              </span>
              <span className="checkmark"></span>
            </label>
          )}
    ...
    switch (locationGroupLevel) {
      ...
      case 1:
        ...
        console.log(
          ...
          bufferLocations.filter((i) => i.location_group_id === userInfo.parent_location),
          bufferLocations
        );
        if (selectedLocationId) {
          if (allLocations.filter((i) => i.id === selectedLocationId).length === 0) {
            return question(bufferLocations?.filter((i) => i.location_group_id === selectedLocationId && !i.parent_id));
          } else {
            return question(
              bufferLocations.filter(
                (i) =>
                  i.location_group_id === allLocations.filter((i) => i.id === selectedLocationId)[0].location_group_id &&
                  !i.parent_id
              )
            );
          }
        } else {
          return question(bufferLocations.filter((i) => i.location_group_id === userInfo.parent_location && !i.parent_id));
        }
      ...
      case 2:
        ...
        return question(bufferLocations?.filter((i) => i.parent_id === selectedLocationId));
      default:
        return <></>;
    }
  };
        ...
        <div className="question_div single-_select" id="scroll__that">
          ...
          <h3 className="main_question">
            {locationGroupLevel === 1
              ? findTranslationFromData(data.id, labels[locationGroupLevel], selectedLanguageId, initData)
