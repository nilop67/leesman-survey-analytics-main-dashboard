...
const SurveyForm = ({
  ...
  const [isReferance, setIsReferance] = useState(params.get("referance") === "4");
  useEffect(() => {
  ...
  const [selectedLanguageText, setSelectedLanguageText] = useState("English (UK)");
  ...
  const next = () => {
    ...
    document.getElementById("middle-wizard") && document.getElementById("middle-wizard").scroll(0, 0, "smooth");
    document.getElementById("scroll__that") && document.getElementById("scroll__that").scroll(0, 0, "smooth");
  };
      ...
      .then((res) => {
        if (res?.data?.general?.status === "closed") {
          ...
          if (res?.data?.status === "not found") {
            ...
            setSelectedLanguageText(res.data?.languages?.filter((i) => i.language_id === res.data?.general?.language_id)[0]?.text);
            setInitData(res.data);
  ...
  useEffect(() => {
    ...
    params.append("page_name", pages?.filter((i) => i.position === isPage)[0]?.name);
  ...
  const textModifier = (oldText) => {
    let newText = oldText ? oldText.replace("(Organisation name)", `(${initData?.general?.organisation_name})`) : "";
    newText = newText.replace("[Location name]", `${selectedLocationName ? "(" + selectedLocationName + ")" : ""}`);
    newText = newText.replace(
      "(Organisation X)",
      `${initData?.general?.organisation_name ? "(" + initData?.general?.organisation_name + ")" : ""}`
    );
    newText = newText.replace(
      "(Workplace X / Other workplace)",
      `${selectedLocationName ? "(" + selectedLocationName + ")" : ""}`
    );
    newText = newText.replace("(Workplace X/Other workplace)", `${selectedLocationName ? "(" + selectedLocationName + ")" : ""}`);
    newText = newText.replace(
      "(Workplace X / Other Workplace)",
      `${selectedLocationName ? "(" + selectedLocationName + ")" : ""}`
    );
    ...
    newText = newText.replace("([BuildingX])", `${selectedLocationName ? "(" + selectedLocationName + ")" : ""}`);
    newText = newText.replace("(Location name)", `${selectedLocationName ? "(" + selectedLocationName + ")" : ""}`);
    newText = newText.replace(
      "(Organization name)",
      `${initData?.general?.organisation_name ? "(" + initData?.general?.organisation_name + ")" : ""}`
    ...
    switch (type) {
      case "dropdown":
        return page.questions
          ...
          .map((data) => {
            let filterOptions = [];
            if (["DEMOGRAPHICS", "CAMPDEMOGRAPHICS", "RETAILDEMOGRAPHICS"].includes(page.name)) {
              if (data.label === "Department") {
                filterOptions = initData.departments.sort((a, b) => a.position - b.position);
              } else {
              ...
              filterOptions = data.options.sort((a, b) => a.position - b.position).map((i) => i.option_label);
            }
              ...
              <h3 className="main_question">
                {textModifier(
                  currentPage[0]?.heading
                    ? findTranslationFromData(currentPage[0]?.id, currentPage[0].heading, selectedLanguageId, initData)
                    : findTranslationFromData(
      ...
      case "radio":
        return (
          <div id="middle-wizard" className={`${data?.questions?.length > 1 && " multi_radio_container"} `}>
            {/* Boş geliyor? */}
        ...
        return (
          <div id="middle-wizard">
            <TableCheckboxSelectOpenable
              ...
              initData={initData}
              selectedLanguageId={selectedLanguageId}
              selectAll={["THIRDSPACEREASON", "ITDISSATISFIED (ITDIS)"].includes(currentPage[0].name)}
            />
          </div>
        );
      case "open":
  ...
  useEffect(() => {
    selectedLocationName !==
      findLocationFromTranslations(
        initData?.locations?.filter((l) => l.id === userInfo?.location_id)[0]?.id,
        initData?.locations?.filter((l) => l.id === userInfo?.location_id)[0]?.label,
        selectedLanguageId,
  ...
  useEffect(() => {
    if (pages.filter((i) => i.name === "AVERAGETIME")[0]?.position === isPage - 1) {
  ...
  const checkNavigationButtonsAvailability = () => {
    ...
    const checkOpenables = () => {
      let arr = [];
      let questionIds = answers.filter((i) => i.page_id === currentPage[0].id && !i.option_id);
      console.log(questionIds);
      questionIds.forEach((i) => {
        if (answers.filter((o) => o.page_id === currentPage[0].id && o.question_id === i.question_id && o.option_id).length === 0) {
          arr.push(false);
        }
      });
      console.log(arr);
      return arr.length === 0 && answers.filter((i) => i.page_id === currentPage[0].id && !i.option_id).length > 0;
    };
    ...
    switch (currentPage[0]?.type) {
        ...
        if (
          ...
          currentPage[0]?.questions
            ?.map((q) => {
              return {
                ...q,
                options: q.options.filter((e) => initData?.demographics?.includes(e.option_id)),
              };
            })
            ?.filter((i) => i?.options.length > 0)
            ?.map((i) => i.id)
            ?.isSubset(answers.map((i) => i.question_id)) &&
          answers.filter((i) => i.question_id === flatedPageData.filter((o) => o.question_label === "Department")[0]?.question_id)
            ?.length > 0
        ) {
          setNextButtonDisabled(false);
        } else {
          if (
            currentPage[0]?.questions
              ?.map((i) => i.id)
              ?.isSubset(answers.filter((i) => i.page_id === currentPage[0].id).map((i) => i.question_id))
          ) {
            setNextButtonDisabled(false);
          }
        }
      ...
      case "opt_scale":
        console.log(
          answers.filter((i) => i.page_id === currentPage[0].id).map((o) => o.question_id),
          answers.filter((i) => i.page_id === currentPage[0].id && i.option_id).map((o) => o.option_id)
        );
        if (
          answers.filter((i) => i.page_id === currentPage[0].id).map((o) => o.question_id).length ===
            answers.filter((i) => i.page_id === currentPage[0].id && i.option_id).map((o) => o.option_id).length &&
          answers.filter((i) => i.page_id === currentPage[0].id).map((o) => o.question_id).length > 0
        ) {
          setNextButtonDisabled(false);
        }
  ...
  const populatePages = () => {
    ...
    setInitPages([
      ...pages_?.sort((a, b) => a.position - b.position).sort((a, b) => a.dependencyPosition - b.dependencyPosition),
    ]);
  ...
  const applyDependencies = () => {
    ...
    const checkLogicConditions = (arr) => {
      ...
      let TrueOrConditions = arr.filter((i) => i.logic === "or").filter((i) => i.valid)?.length;
      let TrueAndCondtions = arr.filter((i) => i.logic === "and").filter((i) => i.valid)?.length;

      //if or conditions, true or conditions length should be bigger then 0
      //if and conditions, all and conditions should be true

      if ((orConditions > 0 ? TrueOrConditions > 0 : true) && (andCondtions > 0 ? andCondtions === TrueAndCondtions : true)) {
        return true;
      } else {
        return false;
      }
    ...
    const checkCoreModuleDependencies = () => {
      ...
      dependencies
        .filter((d) => d.target_type === "core_module" && initData.general.survey_module_id === d.target_id)
        .map((d) => {
          let conditionArr = [];
          ...
          d.conditions.map((c) => {
            ...
            switch (c.target_type_1) {
              case "questions":
                conditionAnswer1 = answers?.filter((a) => a.question_id === c.target_id_1)[0]?.value || 0;
                break;
              case "value":
            ...
            switch (c.target_type_2) {
              case "questions":
                conditionAnswer2 = answers?.filter((a) => a.question_id === c.target_id_2)[0]?.value || 0;
                break;
              case "value":
            ...
            switch (c.comparison_type) {
              case ">":
                isConditionValid = parseInt(conditionAnswer1) > parseInt(conditionAnswer2);
                break;
              case "<":
                isConditionValid = parseInt(conditionAnswer1) < parseInt(conditionAnswer2);
                break;
              ...
              case ">=":
                ...
                break;
              case "+":
                dependencyActionAnswer = parseInt(conditionAnswer1) + parseInt(conditionAnswer2);
            ...
            d.conditions.map((c) => {
              ...
              switch (c.target_type_1) {
                ...
                case "questions":
                  ...
                  break;
                case "options":
                  conditionAnswer1 = answers?.filter((a) => a.option_id === c.target_id_1).length > 0;
              ...
              switch (c.target_type_2) {
                ...
                case "questions":
                  ...
                  break;
                case "options":
                  conditionAnswer2 = answers?.filter((a) => a.option_id === c.target_id_2)[0]?.value || 0;
                  break;
    ...
    const checkPageGroupDependencies = () => {
      ...
      dependencies
        .filter(
          (d) => d.target_type === "page_group" && initData?.pagesAndQuestions?.filter((i) => i.page_group_id === d.target_id)
        )
        .map((d) => {
          let conditionArr = [];
          let dependencyActionAnswer = {};
            ...
            d.conditions.map((c) => {
              ...
              switch (c.target_type_1) {
                ...
                case "questions":
                  ...
                  break;
                case "options":
                  conditionAnswer1 = answers?.filter((a) => a.option_id === c.target_id_1)[0]?.value || 0;
                  break;
                case "value":
                  conditionAnswer1 = c.target_value_1;
                  break;
              ...
              switch (c.comparison_type) {
                ...
                case ">":
                  ...
                  break;
                case "=":
                  isConditionValid = parseInt(conditionAnswer1) === parseInt(conditionAnswer2) && conditionAnswer1 !== 0;
                  break;
                case "<=":
                  isConditionValid = parseInt(conditionAnswer1) <= parseInt(conditionAnswer2);
                  break;
    ...
    const checkPageDependencies = () => {
      let validDependencies = [];
      dependencies
        .filter((d) => d.target_type === "pages" && initData?.pagesAndQuestions?.map((i) => i.id).includes(d.target_id))
        .map((d) => {
          let conditionArr = [];
          let dependencyActionAnswer = {};
          d?.conditions &&
            ...
            d.conditions.map((c) => {
              ...
              switch (c.target_type_1) {
                case "questions":
                  conditionAnswer1 = answers?.filter((a) => a.question_id === parseInt(c.target_id_1))[0]?.value || 0;
                  break;
                case "options":
                  conditionAnswer1 = answers?.filter((a) => a.option_id === parseInt(c.target_id_1))[0]?.value || 0;
                  break;
                case "selected_questions":
                  conditionAnswer1 =
                    answers?.filter((a) => a.page_id === parseInt(c.target_id_1))?.map((i) => i.question_id) || [];
                  break;
                case "value":
                  conditionAnswer1 = c.target_value_1;
                  break;
              ...
              switch (c.comparison_type) {
                ...
                case ">=":
                  ...
                  break;
                case "selected":
                  isConditionValid = answers.filter((i) => i.option_id === parseInt(c.target_id_1)).length > 0;
                  break;
                default:
                  break;
              }
    ...
    const checkQuestionDependencies = () => {
      let validDependencies = [];
      dependencies
        .filter((d) => d.target_type === "question" && flatedPageData.map((i) => i.question_id).includes(d.target_id))
        .map((d) => {
          let conditionArr = [];
          ...
          d?.conditions &&
            ...
            d.conditions.map((c) => {
              ...
              switch (c.target_type_1) {
                case "leesman_module":
                  conditionAnswer1 = flatedPageData.filter((i) => i.survey_module_id === c.target_id_1).length > 0;
                  break;
                case "questions":
                  conditionAnswer1 = answers.filter((i) => i.question_id === c.target_id_1).length > 0;
                  break;
                default:
    ...
    const checkOptionDependencies = () => {
      ...
      dependencies
        .filter((d) => d.target_type === "options" && flatedPageData.map((i) => i.option_id).includes(d.target_id))
        .map((d) => {
          let conditionArr = [];
          d?.conditions &&
            d.conditions.length &&
            d.conditions.map((c) => {
              ...
              switch (c.target_type_1) {
                case "options":
                  conditionAnswer1 = answers.filter((i) => i.option_id === c.target_id_1).length > 0;
                  break;
    ...
    const handleActions = (dependency) => {
      const handlePageGroups = (actions) => {
        let changingPart = currentPages?.filter((i) => i.survey_module_id === dependency.target_id && i.page_group_id);
        currentPages = currentPages?.filter((i) => !changingPart.map((o) => o.id).includes(i.id));

        switch (actions[0].action_type) {
          case "show_only":
            changingPart = changingPart.filter((i) => actions.map((o) => o.target_id).includes(i.page_group_id));
            break;
        ...
        changingPart = changingPart.map((c) => {
          return {
            ...c,
            dependencyPosition: actions.filter((a) => a.target_id === c.page_group_id)[0]?.position,
          };
        });
        ...
        actions.map((a) => {
          switch (a.action_type) {
            case "selected_questions":
              ...
              let selectedQuestionsOnPage = answers?.filter((i) => i.page_id === a.target_id);

              let changingPart = currentPages?.filter((i) => i?.id === dependency?.target_id);
              let currentPart = currentPages?.filter((i) => i?.id !== dependency?.target_id);

              if (changingPart?.length > 0) {
                changingPart = changingPart.map((i) => {
                  return {
                    ...i,
                    questions: i?.questions?.filter((q) => selectedQuestionsOnPage.map((o) => o.question_id).includes(q.id)),
                  };
                });
              }
              currentPages = currentPages?.filter((i) => i?.id !== dependency.target_id);
              ...
              break;
            case "hide":
              currentPages = currentPages?.filter((i) => !actions.map((a) => a.target_id).includes(i?.id));
            ...
            case "selected_questions<=-1":
              ...
              let selectedQuestionsOnPage_ = answers?.filter(
                (i) => dependencyActionTargets.includes(i.page_id) && i.value <= -1 && i.value > -4
              );

              let changingPart_ = currentPages?.filter((i) => i?.id === dependency?.target_id);
              let currentPart_ = currentPages?.filter((i) => i?.id !== dependency?.target_id);

              if (changingPart_.length > 0) {
                changingPart_ = changingPart_.map((i) => {
                  return {
                    ...i,
                    questions: i?.questions?.filter((q) => selectedQuestionsOnPage_.map((o) => o.question_id).includes(q.id)),
                  };
                });
              }
              currentPages = currentPages?.filter((i) => i?.id !== dependency.target_id);
      ...
      const handleLeesmanModules = (actions) => {
        switch (actions[0].action_type) {
          case "hide":
            currentPages = currentPages.filter((i) => !actions.map((a) => a.target_id).includes(i.survey_module_id));
            break;
          case "follow_office":
            const isOfficeBefore = () => {
              return (
                currentPages.filter((i) => i.page_type === "office")[0]?.dependencyPosition <
                currentPages.filter((i) => i.page_type === "home")[0]?.dependencyPosition
              );
            };
            const decidePosition = (page) => {
              ...
              } else if (!page.page_group_id && actions.map((i) => i.target_id).includes(page.survey_module_id)) {
            ...
            if (currentPages.filter((i) => i.page_type === "office")?.length === 0) {
              currentPages = currentPages.filter((item) => !actions.map((i) => i.target_id).includes(item.survey_module_id));
            }
      ...
      const handleQuestions = (actions) => {
        const removeQuestion = (id) => {
          let questionPageId = flatedPageData?.filter((i) => i.question_id === id)[0]?.page_id;
          let changingPart = currentPages?.filter((i) => i?.id === questionPageId)[0];
      ...
      const handleOptions = (actions) => {
        const showOptions = () => {
          let changedPageId = flatedPageData.filter((i) => i.option_id === actions[0].target_id)[0]?.page_id;
          let questionId = flatedPageData.filter((i) => i.option_id === actions[0].target_id)[0]?.question_id;
          let changedPagePosition = pages.filter((i) => i.id === changedPageId)[0]?.position;
          ...
          if (isPage === changedPagePosition) {
                ...
                return {
                  ...
                  questions: i.questions.map((q) => {
                    return {
                      ...
                      options:
                        questionId === q.id
                          ? q.options.filter((e) => actions.map((w) => w.target_id).includes(e.option_id))
                          : q.options,
                    };
                  }),
                };
            ...
            isPage === changingPart[0].position && SetCurrentPage([...changingPart]);
          }
        };
  ...
  const handleAnswer = (answer) => {
    ...
    if (currentAnswers?.map((a) => a.question_id).includes(answer.question_id)) {
      if (!["tick", "checkbox_scale"].includes(answer?.type)) {
        currentAnswers = currentAnswers?.filter(
          (a) =>
            ...
            (a?.department_id && answer?.department_id && a.department_id !== answer.department_id)
        );
      }
    }
  ...
  var patternChinese = /[\u3040-\u30ff\u3400-\u4dbf\u4e00-\u9fff\uf900-\ufaff\uff66-\uff9f]/;
  var patternJapanese = /[\u3000-\u303f\u3040-\u309f\u30a0-\u30ff\uff00-\uff9f\u4e00-\u9faf\u3400-\u4dbf]/;
  ...
  useEffect(() => {
    setTitle(
      "Leesman® Analytics | " +
        ...
        (isPage === 4 + pages?.length || userInfo?.is_completed ? "Thanks page" : "") +
  ...
  return (
    <>
      ...
      <div className="container-fluid survey_form-container" id="survey-_section">
        <div className="row row-height">
          ...
          */}
          ...
          ${(isPage === 4 + pages?.length || userInfo?.is_completed) && " bg-green"}
          ...
          >
            <div id="survey-_scroll" className="survey_-wrapper">
              ...
              {/* Logo */}
              <Link to={history.location.pathname} id="logo" className={`logo-_srv ${checkedMenu && " opened"}`}>
                <img src="/assets/images/leesman-logos/leesman_logo_survey.svg" alt="" />
              </Link>

              {isPage > 5 && isPage < 3 + pages?.length ? (
                <label id="survey_mobile_burger">
                  <input type="checkbox" checked={checkedMenu} onChange={() => setCheckedMenu(!checkedMenu)} />
                  <span className="menu">
                    {" "}
              ...
              {initData?.languages?.length >= 1 ? (
                <select
                  className={`custom-select survey-language-select ${isPage > 5 && isPage < 3 + pages?.length ? " opened" : " "}`}
                  style={{
                    width: `${
                      selectedLanguageText?.length * 8 + 65 > 215
                        ? selectedLanguageText?.length * 8 + 100
                        : selectedLanguageText?.length * 8 + 65 < 110 && selectedLanguageText?.length * 8 + 65 > 85
              ...
              {isPage > 6 && isPage < 3 + pages?.length ? (
                <div className={`dropdown_-content ${checkedMenu && " opened"}`}>
              ...
              {/* Reset my answers */}
              <ResetModal resetAnswer={resetAnswer} isResetModal={isResetModal} setResetModal={setResetModal} />
              {(userInfo?.email || userInfo?.refCode) && initData.general?.status !== "live" && (
                <div className={`reset_answers ${isPage > 1 && isPage < 3 + pages?.length ? " jump" : " "}`}>
              ...
              */}
              <div
                className={`left_info-survey 
                  ${isPage > 2 && isPage < 3 + pages?.length ? " full-_height" : " chapters-_height"} 
                  ${isPage === 3 + pages?.length || isPage === 4 + pages?.length || notFound ? " has_figure" : " "} 
              ...
              >
                ...
                {isPage === 2 && !isReferance && (
                  <WelcomePage setPage={setPage} data={initData} selectedLanguageId={selectedLanguageId} />
                )}
                ...
                {/* Text - Intro */}
                {isPage === "details" && <TextPage setPage={setPage} selectedLanguageId={selectedLanguageId} data={initData} />}
                ...
                <div className="page_survey">
                  ...
                  {/* Question Buttons */}
                  {isPage > 2 && isPage < 3 + pages?.length && !hideButtonGroups && (
                    <BtnGroup
                      isPage={isPage}
                      setPage={setPage}
                      prev={prev}
                ...
                {(isPage === 4 + pages?.length || userInfo?.is_completed) && (
                  <ThanksPage setPage={setPage} isPage={isPage} data={initData} selectedLanguageId={selectedLanguageId} />
                )}
                {surveyClosed && (
                  <ClosedSurveyThanksPage data={initData} selectedLanguageId={selectedLanguageId}></ClosedSurveyThanksPage>
                )}
                {notFound && <SurveyNotFoundPage data={initData} selectedLanguageId={selectedLanguageId}></SurveyNotFoundPage>}
              </div>
            </div>
          </div>
        </div>
