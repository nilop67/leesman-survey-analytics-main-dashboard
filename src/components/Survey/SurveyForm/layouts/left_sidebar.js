    ...
    setNavigations(
      pages
        ...
        .map((i) => {
          return {
            position: i.position,
            parent_name: i.module_client_id ? i.module_description : i?.page_group_name ? i?.page_type : "Additional questions",
          };
        })
    ...
    <>
      <div className="dropdown_-progress">
        ...
        <label htmlFor="surveyJourney" className="drop-_item" id="hover-_hand">
          <span>{findTextFromTranslations("Survey journey", selectedLanguageId, initData?.customisations)}</span>
          <div className="lil_arrow"></div>
          <div className="bar"></div>
        </label>
        <div className={`dropdown_-progress__content ${isJourney ? " active_it" : ""} `}>
          <ul>
            {[...new Set(navigations?.map((i) => i.parent_name))].map((i) => {
              return (
                ...
                <li
                  className={
                    navigations?.filter((n) => n.parent_name === i)?.length > 0 &&
                  ...
                  id="hover-_hand"
                >
                  {truncate(findTextFromTranslations(i?.capitalize(), selectedLanguageId, initData?.customisations), 20)}
                  ...
                  <ul>
                    ...
                    {navigations
                      ...
                      ?.map((p, index) => {
                        return (
                          <li
                            className={`${isPage === p.position ? "sub_active" : isPage > p.position ? "sub_completed" : ""}`}
                            id="hover-_hand"
                          >
                            {findTextFromTranslations("Question (X) of (Y)", selectedLanguageId, initData?.customisations)
                              .replace("X", index + 1)
                              .replace("Y", navigations?.filter((n) => n.parent_name === i).length)}
                          </li>
                        );
                      })}
                  </ul>
        ...
        />
        <label htmlFor="saveForLater" className="drop-_item save_later" id="hover-_hand">
          <span>{findTextFromTranslations("Save for later", selectedLanguageId, initData?.customisations)}</span>
