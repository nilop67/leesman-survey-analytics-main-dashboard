...
import { findTextFromTranslations, modifyDates } from "../../../../util/functions";
...
const Modal = ({ isModal, setModal, initData, selectedLanguageId, currentPage, isPage }) => {
  const [isWelcome, setIsWelcome] = useState(false);
  const [label, setLabel] = useState("");
  useEffect(() => {
    let pagePopup = initData?.customisations?.filter(
      (i) =>
        i.survey_id && i.survey_id === initData?.general?.id && i.language_id === 12 && i.type === `page_${currentPage?.id}_popup`
    );
  ...
  useEffect(() => {
    console.log(
      ...
      initData?.customisations?.filter(
        (i) => i.survey_id && i.survey_id === initData?.general?.id && i.type === "welcome_popup" && i.language_id === 12
      )[0]
    );
    if (
      ((initData?.general?.access === 4 && isPage === 2) || (initData?.general?.access === 1 && isPage === 1)) &&
      initData?.customisations?.filter(
        (i) => i.survey_id && i.survey_id === initData?.general?.id && i.type === "welcome_popup" && i.language_id === 12
      )[0]?.label?.length > 0
    ) {
              ...
              <div className="modal-body">
                <ReactMarkdown>
                  {modifyDates(
                    ...
                    isWelcome
                      ? initData?.customisations?.filter(
                          (i) => i.survey_id && i.type === `welcome_popup` && i.language_id === selectedLanguageId
                        )[0]?.label ||
                          initData?.customisations?.filter(
                            (i) => i.survey_id && i.type === "welcome_popup" && i.language_id === 12
                          )[0]?.label
                      : findTextFromTranslations(`page_${currentPage?.id}_popup`, selectedLanguageId, initData.customisations) !==
                        `page_${currentPage?.id}_popup`
                      ? findTextFromTranslations(`page_${currentPage?.id}_popup`, selectedLanguageId, initData.customisations)
                      : findTextFromTranslations(`page_${currentPage?.id}_popup`, 12, initData.customisations)
                  )}
                </ReactMarkdown>
              </div>
