      ...
      case "next":
        if (
          ...
          (locationGroupLevel === 0 ||
            (locationGroupLevel === 1 && initData?.locations?.filter((i) => i.parent_id === userInfo.location_id).length > 0)) &&
          answers.filter((i) => i.type === "location")[0]?.option_id !== initData?.otherOptions?.other_location?.id
        ) {
        ...
        break;
      case "back":
        if (currentPage?.name === "LOCATION" && (locationGroupLevel === 1 || locationGroupLevel === 2)) {
          setLocationGroupLevel(locationGroupLevel - 1);
        } else {
          prev();
        }
    ...
    <>
      <div id="bottom-wizard">
        ...
        {!(
          ...
          ((currentPage?.name === "LOCATION" && locationGroupLevel === 0) ||
            (currentPage?.name === "LOCATION" &&
              ...
              initData?.locations?.filter((i) => i.location_group_id !== null).length === 0))
        ) && (
          <button
            id="hover-_hand"
            onClick={() => {
              if (currentPage?.name === "LOCATION") {
                {
                  !backButtonDisabled || (!(currentPage?.name === "LOCATION" && locationGroupLevel === 0) && checkPages("back"));
                }
              } else {
                !backButtonDisabled && checkPages("back");
              }
            }}
            className={`btn_2 mr-3 validate ${backButtonDisabled ? " disable_it" : " "}`}
          >
        ...
        <button
          ...
          className={`btn_1 validate ${nextButtonDisabled ? " disable_it" : " "}`}
        >
