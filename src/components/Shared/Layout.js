...
import { setLightTheme, setLoginBackground, setFullScreen } from "../../actions/settingsAction";
...
function Layout(props) {
  ...
  const openResponderModal = () => {
    ...
    console.log(responderMode + " " + "responder mode status for admin/client side");
  };
  ...
  }, [props.location.pathname, props.lightVersion, props.openFullScreen, responderMode]);
  ...
  useEffect(() => {
    if (
      ...
      (props.location.pathname === "/campaign-details" && props.lightVersion === true) ||
      (props.location.pathname === "/survey-details" && props.lightVersion === true)
    ) {
      props.setLightTheme(false);
    }
    ...
    return history.listen((location) => {
      ...
      if (history.action === "POP") {
        if (locationKeys[1] === location.key) {
          ...
          if (props.location.pathname === "/share-thinking" && responderMode === "ON") {
            props.setLightTheme(true);
            props.setFullScreen(true);
          }
          if (props.location.pathname === "/answer-question" && responderMode === "ON") {
            props.setLightTheme(true);
            props.setFullScreen(true);
          }
  ...
  return (
    !isLoading && (
      <>
        ...
        <div id="wrapper">
          {!props.openFullScreen && props.location.pathname !== "/responder-home" ? (
            <Header {...props} Handle={handle} OpenResponder={openResponderModal} />
          ) : (
            <></>
          )}
          <Rightbar />
          {!props.openFullScreen && props.location.pathname !== "/responder-home" ? <Menu {...props} /> : <></>}
