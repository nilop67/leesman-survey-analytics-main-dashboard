...
import { setMiniSidebarMenuOn, setMiniHover, setLightTheme, setOffcanvas } from "../../actions/settingsAction";
...
class Menu extends Component {
  ...
  componentDidMount() {
    if (window.location.pathname !== "/" && document.querySelector("#location-_link") !== null) {
  ...
  render() {
    ...
    console.log(metisMenu, this.props.reduxClient);

    let filteredAdminMenus = this.menus["Admin"];

    if (!this.props.reduxClient.enable_xp) {
      filteredAdminMenus = filteredAdminMenus.filter((i) => i.label !== "XP Score");
    }
    if (!this.props.reduxClient.enable_questions) {
      filteredAdminMenus = filteredAdminMenus.filter((i) => i.label !== "Campaigns");
    }
    if (!this.props.reduxClient.enable_feedbacks) {
      filteredAdminMenus = filteredAdminMenus.filter((i) => i.label !== "Comments");
    }
    ...
    console.log(role, filteredAdminMenus, this.menus["Admin"]);

    return (
      <>
        <div id="left-sidebar" className={`sidebar${!role ? " responder-sidebar-hide" : " full-mockup-logo"}`}>
          <div className="navbar-brand">
            <Link to="/">
              {!role ? (
                <img src="/assets/images/leesman-logos/wave-long-dark.png" alt="Leesman Logo" className="img-fluid logo" />
              ) : (
                <img src="/assets/images/leesman-logos/leesman-inside-bubble.svg" alt="Leesman Logo" className="img-fluid logo" />
              )}
            </Link>
          </div>
          <div className={`sidebar-scroll${!role ? " hidden-xl-" : ""}`}>
            <nav id="left-sidebar-nav" className="sidebar-nav" onClick={() => this.props.setOffcanvas(!this.props.offcanvas)}>
              <MetisMenu
                content={metisMenu}
