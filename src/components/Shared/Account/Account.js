...
const Account = ({ setBgImage }) => {
  ...
  const getChilds = (item) => {
    let getChild = [];
    getChild = data.departments.filter((i) => parseInt(i.parent_id) === parseInt(item.id));
    let buffer = [];
  ...
  function handleSpecial(e) {
    ...
    let noone_id = options.filter((i) => i.option_label === "No one")[0].option_id;
    let nwfh = options.filter((i) => i.option_label === "I never work from home")[0].option_id;
  ...
  function getData() {
    axios
      ...
      .then((res) => {
        ...
        let answers = res.data.result.answers.filter((i) => res.data.result.enabledOptions.includes(i));
        console.log(answers);
        ...
        setDepartmentCascadePath(findCascadePath(res.data.result.user.department_id, res.data.result.departments));
        setLocationCascadePath(findCascadePath(res.data.result.user.location_id, res.data.result.locations));
      })
            ...
            <div className="col-lg-5 col-md-12 bg-_color-left none-992">
              <div className="form-_section">
                ...
                <div className="account-_inner-form">
                  <div className="form-group form-box text-leftt">
                    <Link to="/my-account" className="item-_-link active-_-item">
                  ...
                  <div className="form-group form-box text-leftt">
                    <Link to="/alert-settings" className="item-_-link">
                      <i className="icon_a-alertssicon_a-" aria-hidden="true" title="Bell icon"></i> Alert settings
                    </Link>
                  </div>
                </div>
              </div>
                ...
                <div className="account-_inner-form">
                  <div className="form-group form-box text-leftt">
                    ...
                    <Input
                      ...
                      value={userData.first_name ? userData.first_name : data.user ? data.user.first_name : ""}
                      style={{ width: 300 }}
                    />
                  </div>
                  <div className="form-group form-box text-leftt">
                    ...
                    <Input
                      ...
                      id="last_name"
                      onChange={handleUserData}
                      value={userData.last_name ? userData.last_name : data.user ? data.user.last_name : ""}
                      style={{ width: 300 }}
                    />
                  </div>
                  ...
                  <div className="form-group form-box text-leftt">
                    <label className="info--_text" htmlFor="department_id">
                      <span className="info--sub-title">
                        {labels.filter((i) => i.type === "department").length > 0
                          ? labels.filter((i) => i.type === "department" && i.level === 0)[0].label
                          : "Department"}
                      </span>
                    </label>
                    <div className="ant-_-select-container">
                    ...
                    <div className="ant-_-select-container">
                      ...
                      >
                        {data.locations &&
                          data.locations
                            ?.filter((i) => !i.parent_id)
                            ?.filter((i) => !["code_office", "code_home"].includes(i.code))
