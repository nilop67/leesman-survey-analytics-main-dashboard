...
const Rightbar = ({ isNotificationbar }) => {
  ...
  useEffect(() => {
    axios
      ...
      .then((res) => {
        ...
        sessionStorage.setItem("notificationCount", [...res.data.campaign, ...res.data.demographics].length);
  ...
  return (
    <>
      <div id="rightbar" className={`z-index-noti rightbar${isNotificationbar ? " open" : ""}`}>
        <div className={`body ${!role ? " responder-mode" : ""}`}>
          <ul className="nav nav-tabs2  text-centerr">
            ...
            <h2 className="lead light-black notification-title z-index-9">Notifications</h2>
          </ul>
          <hr />
          <div className={`tab-content${!role ? " responder-alerts" : ""}`}>
            <div className="tab-pane vvivify fadeIn delay-100 active" id="Chat-list">
