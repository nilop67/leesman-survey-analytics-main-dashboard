...
import {
  ...
  makeUnique,
  makeUniqueWithAttributeArr,
  makeUniqueWithDoubleAttrubes,
  makeUniqueWithfourthAttributes,
  makeUniqueWithTripleAttrubes,
} from "../../util/functions";
export default function ImportExport(props) {
  ...
  useEffect(() => {
    console.log(mergeableColumn, props.data);
  ...
  const handleInput = (type, data) => {
    const handleDepartmentCascade = (data) => {
      ...
      flatDepartments = makeUnique(flatDepartments, "upper_path").sort((a, b) => a.level - b.level);
      ...
      flatDepartments = flatDepartments.map((i) => {
        return {
          ...
          parent_id: flatDepartments.filter(
            (o) => o.label === i.parent_label && JSON.stringify([...o.upper_path_arr, o.label]) === JSON.stringify(i.upper_path_arr)
          )[0]?.id,
        };
      });
          ...
          .map((i, index) => {
            if (index > 0) {
              if (checkDuplication(i[2], props?.allData, "name")) {
                notification.warning({ message: `Duplicate location: ${i[2]}, skipped` });
              } else {
                return {
                  id: parseInt(i[0]) || null,
    ...
    const handleLocationCascadeSurvey = (data) => {
      console.log(data);
      const findLocationIdFromlocationGroupAndLabel = (locationGroup, label) => {
        !props?.allData?.filter((i) => i.label === label && i.location_group_name === locationGroup)[0]?.id &&
          notification.warning({ message: `Cannot find location: ${label} and skipped` });
        return props?.allData?.filter((i) => i.label === label && i.location_group_name === locationGroup)[0]?.id || null;
      };
      ...
      let preparedData = data
        ...
        .map((i, index) => {
          if (index > 0) {
            if (checkDuplicationById(findLocationIdFromlocationGroupAndLabel(i[1], i[2]), props.data)) {
              notification.warning({
                message: `Duplicate location id: ${findLocationIdFromlocationGroupAndLabel(i[1], i[2])}, skipped`,
              });
            } else if (findLocationIdFromlocationGroupAndLabel(i[1], i[2]) == null) {
              notification.warning({
      ...
      preparedData.forEach((row) => {
        for (var key in row) {
          if (row.hasOwnProperty(key)) {
            let selectedAttribute = attributes.filter((i) => i.heading === key)[0];
            ...
            preparedAttributeAnswerData.push({
              ...
              option_id:
                selectedAttribute?.type === "dropdown"
                  ? selectedAttribute?.options?.filter((o) => o.option_label === row[key])[0]?.option_id
                  : null,
              answer: row[key],
      ...
      console.log(props.customAttributes, preparedAttributeAnswerData, preparedData);
      setVisible(true);
    ...
    const handleTranslation = (data) => {
      ...
      setMergeableColumn(
        data[0].map((i, index) => {
          ...
          return {
            title: i,
            dataIndex: [props.languages.filter((l) => l.label === i)[0]?.id || "key"],
          };
        })
      );
      ...
      data.forEach((i, index) => {
        ...
        if (index > 0) {
          data[0].forEach((c, columnIndex) => {
            arr = { ...arr, [props.languages.filter((l) => l.label === c)[0]?.id || "key"]: i[columnIndex] };
          });
          buffer.push(arr);
        }
      });
    ...
    const handleSurveySpecificTranslations = (data) => {
      ...
      setMergeableColumn(
        data[0].map((i, index) => {
          ...
          return {
            title: i,
            dataIndex: [props.allLanguages.filter((l) => l.label === i)[0]?.id || i],
          };
        })
      );
      ...
      data.forEach((i, index) => {
        ...
        if (index > 0) {
          data[0].forEach((c, columnIndex) => {
            arr = { ...arr, [props.allLanguages.filter((l) => l.label === c)[0]?.id || c]: i[columnIndex] };
          });
          buffer.push(arr);
        }
      });
    ...
    const handleFloors = (data) => {
      const checkDuplication = (allData, floor) => {
        console.log(allData, floor, allData.filter((i) => i.label === floor).length > 1);
        return allData.filter((i) => i.label === floor).length > 0;
      };

      const checkDuplicationOnFile = (data, floor) => {
        console.log(data, floor, data.filter((i) => i && i[0] === floor).length > 1);
      ...
      setTableData(
        d_
          ...
          .map((i, index) => {
            if (index > 0 && !checkDuplication(props.allData, i) && !checkDuplicationOnFile(d_, i)) {
              ...
              if (checkDuplication(props.allData, i)) notification.warning({ message: `Duplicatad floor ${i} skipped...` });
              if (checkDuplicationOnFile(d_, i)) notification.warning({ message: `Duplicatad floor in excel ${i} skipped...` });
            }
          })
          .filter((i) => i?.floor)
      );
    ...
    const handleCascadeFloors = (data) => {
      ...
      const checkDuplication = (allData, location, label) => {
        return allData.filter((i) => i.parent_id === getLocationIdFromLabel(location) && i.label === label).length > 0;
      };
      const checkDuplicationOnFile = (data, location, label) => {
        return data.filter((i) => i[1] === location && i[2] === label).length > 1;
      };
      setTableData(
        data
          .map((i, index) => {
            if (index > 0) {
              if (
                ...
                if (!getLocationIdFromLabel(i[1])) notification.warning({ message: `Cannot found location:${i[1]} skipped...` });
                if (!i[2]?.length) notification.warning({ message: `Cannot insert empty floor for location:${i[1]} skipped...` });
                if (checkDuplication(props.allData, i[1], i[2]))
                  notification.warning({ message: `Duplicatad floor ${i[2]} in location:${i[1]} skipped...` });
          ...
          props.data.length > 0
            ? props.data.map((i) => {
                return {
                  ...
                  label: i.label,
                  parent_label: props.data.filter((item) => item.id === i.parent_id)[0]?.label,
                  parent_id: isNaN(parseInt(i.parent_id)) ? null : parseInt(i.parent_id),
                };
              })
            : [
                {
      ...
      case "translation-locations":
        ...
        props.data.forEach((d) => {
          ...
          for (const item in d) {
            smallBuffer = {
              ...smallBuffer,
              [item !== "key" ? props?.languages?.filter((i) => i.id === item)[0]?.label : "key"]: d[item],
            };
          }
        ...
        data.length
          ? data.forEach((location) => {
              location.floor_labels.forEach((floor) => {
                arr.push({ location_id: location.id, location: location.label, floor: floor });
              });
            })
  ...
  const send = () => {
    ...
    axios
      ...
      .catch((err) => {
        ref.current.value = "";
        notification.warning({ message: "Cannot insert. Please check ids and labels in the excel file." });
        console.log(err);
      });
  };
  ...
  return (
    <>
      <a
        ...
        className={`btn btn-default bigger-_btn ml-3 ${!props.hideLine && "after-_r-border"} ${props?.additionalClasses?.join(
          " "
        )}`}
      >
      ...
      <Drawer width="%100" title="Import | Export management" placement="right" onClose={onClose} visible={visible}>
          ...
          <div className="row clearfix">
            <div className="col-xl-12 col-lg-12 col-md-12">
              <div className="row mb-4 page-__header">
                ...
                <div className="col-xl-6 col-lg-6 col-md-6 jus-start-iex">
                  <div className={`file-upload_section mr-4 ${!props.hideLine && "after-_r-border"}`}>
                ...
                <div className="col-xl-6 col-lg-6 col-md-6 jus-end">
                  <span className="text-table-info">Total: {tableData.length} rows</span>
                  {tableData.length > 0 && (
                    <>
                      <button
                        type="button"
                      ...
                      <button type="button" data-dismiss="modal" onClick={send} className="btn btn-primary ml-3">
                        Save
                      </button>
                    </>
                  )}
            ...
            <div className="col-lg-12">
              <div className="card without-btn-table">
                <Table pagination={false} columns={selectedColumn} dataSource={tableData} />
              </div>
            </div>
          </div>
        </div>
