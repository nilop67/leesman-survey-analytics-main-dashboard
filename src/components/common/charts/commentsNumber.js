...
import { BarChart, Bar, XAxis, YAxis, Tooltip, Legend, ResponsiveContainer, Label, tickFormatter } from "recharts";
...
function CommentsNumber(props) {
  ...
  const [getPng, { ref: areaRef }] = useCurrentPng({ backgroundColor: "#292b2b" });
  ...
  return (
    <>
      {props.data && props.data.length >= 1 ? (
        <>
          <ResponsiveContainer width="99%" height={250} className="mb-point-c comment-chart">
            <BarChart
              data={
                props.data
                  ? props.data.map((i) => {
              ...
              ref={areaRef}
            >
              <XAxis dataKey="name" tickFormatter={props.data.length < 20 ? undefined : formatXAxis} />
              <YAxis type="number">
                <Label value="Count" position="insideLeft" angle={-90} style={{ textAnchor: "middle" }} />
              </YAxis>
          ...
          <div className="apexcharts-toolbar comments-_-toolbar" title="Menu">
            <div className="apexcharts-menu-icon" onClick={() => setopenMenu(!openMenu)}>
              <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24">
                <path fill="none" d="M0 0h24v24H0V0z"></path>
                <path d="M3 18h18v-2H3v2zm0-5h18v-2H3v2zm0-7v2h18V6H3z"></path>
              </svg>
            </div>
            <div className={`apexcharts-menu ${openMenu ? " apexcharts-menu-open opacity-_1" : ""}`}>
