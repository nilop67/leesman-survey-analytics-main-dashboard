  ...
  render() {
    ...
    return (
      <div id="chart" className="mb-point-c have-filter-btn">
        {this.props.data && data.series[0].data.reduce((previousScore, currentScore, index)=>previousScore+currentScore, 0) >= 1 ?
        <ReactApexChart options={data.options} series={data.series} type="bar" height="250px" /> :
        <NoData/>}
      </div>
    );
  }
