...
export const AuthSvg = () => {
  return (
    <svg id="leesmanInsideLogo" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 72.23 73.91">
      <title id="leesmanInsideLogo">Leesman Inside Logo</title>
      <desc id="leesmanInsideLogo">If clicked, it will go to the Login page.</desc>
      <defs></defs>
      <g id="Layer_2" data-name="Layer 2">
        <g id="Layer_1-2" data-name="Layer 1">
          <path className="cls-1" d="M65.45,57.14a36.09,36.09,0,1,0-12.76,11L66.6,73.91Z" />
          <g id="_4" data-name="4">
            <circle className="cls-2" cx="36.12" cy="36.12" r="5.02" transform="translate(-14.96 36.12) rotate(-45)" />
            <circle className="cls-2" cx="55.99" cy="36.12" r="5.02" transform="translate(-9.14 50.17) rotate(-45)" />
          </g>
          <g id="_4-2" data-name="4">
            <circle className="cls-2" cx="16.24" cy="36.12" r="5.02" transform="translate(-20.78 22.06) rotate(-45)" />
          </g>
        </g>
      </g>
    </svg>
...
export const campaignFilter = (
  ...
  let data = {
    ...
    distribution: Array.apply(null, Array(5)).map((i, index) => {
      return basicBarData.distribution[index].filter((i, id) => indexes.includes(id));
    }),
...
export const campaignCommentsSearchPart = (bufferCommentData, query, setQuery) => {
  return (
    <div className="page-__header d-flex">
      <ImportExport data={bufferCommentData} export={true} import={false} type={`all_campaign_comments`} />
...
export const campaignCommonPart = (chartType, loading, orderBy, basicBufferData, heatBufferData, bufferRates, bufferTrends) => {
  return (
    <div className="col-lg-9 pt-20 scrollable-_charts">
      {chartType === "individual-chart" ? <IndividualChartSection data={bufferTrends} /> : ""}
      {chartType === "combined-chart" ? <CombinedChart data={bufferTrends} /> : ""}
      {/* Campaign Breakdown Charts */}
      {chartType === "bar-chart" ? loading ? <Loader></Loader> : <BasicBar orderBy={orderBy} data={basicBufferData} /> : ""}
