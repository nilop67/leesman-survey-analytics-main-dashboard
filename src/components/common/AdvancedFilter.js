...
import { Tag, Menu, Dropdown, DatePicker, Drawer, Tooltip, Checkbox } from "antd";
...
import { getRegionCountryCityTreeData, onlyIntArray, resetFilter, dbToTree } from "../../util/functions";
...
const AdvancedFilter = ({
    ...
    return (
      <Menu className="filter-dropdown">
        ...
        <div className="ant-table-filter-dropdown-btns">
          <button type="button" className="ant-btn ant-btn-link ant-btn-sm" onClick={() => SelectAll(type)}>
            <span>Select All</span>
          </button>
          {/* Add “Deselect” option to deselect all Region */}
          <button onClick={() => DeselectAll(type)} type="button" className="ant-btn ant-btn-link ant-btn-sm">
            <span>Deselect</span>
          </button>
        </div>
      </Menu>
  ...
  const handleQueryDemo = (e) => {
    ...
    setBufferFilterData((pre) => {
      return { ...pre, demos: e.length > 2 ? [...questionBuffer, ...dbToTree(buffer)] : demoToTree(dbData.demos) };
    });
  };
    ...
    return data
      ...
      .map((i) => {
        return {
          key: "parent-" + i.id,
          title: i.heading,
          children: i.options?.sortAlphabetically("option_label").map((item) => {
            return {
              key: item.option_id,
  ...
  const SelectAll = (type) => {
    ...
    setSelectedFilters((pre) => {
      return { ...pre, [type]: type === "demos" ? getDemoOptions(dbData) : dbData[type]?.map((i) => i.id) };
    });
  };
  ...
  const setMenus = () => {
    setClientsMenu(populateHoverMenuItems(bufferFilterData, "clients", queryClient, setQueryClient));
                ...
                <div key={index}>
                  ...
                  ${
                    ...
                    item.heading === "Home Working Setting"
                      ? " long_-labels"
                      : ""
                  } ${item.options && item.options.length <= 3 ? " threet_-labels" : ""}`}
                  >
                      ...
                      .map((i, index) => {
                        return (
                          <Checkbox
                            ...
                            checked={selectedFilters.demos?.includes(parseInt(i.option_id))}
                          >
  ...
  const handleTags = (clients, locations, departments, demos, regions) => {
    ...
    demoTags = demoTags.filter((i) => checkDemoQuestionOptions(demoTags.map((i) => i.option_id))?.includes(i.option_id));
    ...
    locationTags.forEach((item) => {
      buffer_locations.push({ id: item.id, label: item.label, type: "locations" });
    });
    departmentTags.forEach((item) => {
      buffer_departments.push({ id: item.id, label: item.label, type: "departments" });
    });
    demoTags.forEach((item) => {
      buffer_demos.push({ id: item.option_id, label: item.option_label, type: "demographics" });
    });
  ...
  const decideSelections = (clientData, role) => {
    ...
    if (role === "Admin") {
      ...
      if (clientData?.length > 1) {
        ...
        preferences = preferences.filter((i) => !["departments", "locations"]?.includes(i.value));
        selectedPreferences = selectedPreferences.filter((i) => !["departments", "locations"]?.includes(i.value));
  ...
  const filterInit = () => {
    ...
    setFetchClientData(dbData?.clients?.map((i) => i.id));
    setStart(defaultFilterData?.start ? defaultFilterData.start : weekBefore.splitDate());
    setEnd(defaultFilterData?.end ? defaultFilterData.end : tomorrow.toISOString().splitDate());
    reduxSaveFilter([]);
  ...
  const getDataByClientId = (ids) => {
    ...
    let url = ["Super Admin", "Leesman Admin"]?.includes(user.user_role) ? `/metrics/${JSON.stringify(ids)}` : "/metrics";
    axios
      .get(url)
      .then((res) => {
  ...
  useEffect(() => {
    ...
    setSelectedFilters((pre) => {
      return {
        ...pre,
        locations: reduxFilterData?.start ? reduxFilterData.locations : dbData?.locations?.map((i) => i.id),
        departments: reduxFilterData?.start ? reduxFilterData.departments : dbData?.departments?.map((i) => i.id),
        demos: reduxFilterData?.start ? reduxFilterData?.demos : getDemoOptions(dbData),
        regions: reduxFilterData?.start ? reduxFilterData?.regions : dbData?.regions?.map((i) => i.id),
      };
    });
  ...
  return (
    <>
        ...
        <div className="row clearfix">
          ...
          <div className="col-lg-12 col-md-12 filter-btns text-right client-dropdown">
            {(role === "Super Admin" || role === "Leesman Admin") &&
              (rawData.length > 0 ? (
                <ImportExport type={"raw"} data={rawData} import={false} export={true}>
                  <i className="iconx-download1"></i>
                </ImportExport>
              ) : (
                <button type="button" onClick={handleRaw} className="btn btn-sm btn-default mr--10" title="Fetch raw data">
                  {loginSpinner && <div className="sbl-circ"></div>} {loginSpinner ? "Fetching" : "Export data"}
                </button>
              ))}
            ...
            <button onClick={() => setDrawerDemos(true)} className="btn btn-sm btn-default open-more-modal btn-w_icon ml--10">
              <span className="iconx-compass"></span>Demographics
            </button>
            <button onClick={() => setDrawerRegions(true)} className="btn btn-sm btn-default open-more-modal btn-w_icon ml--10">
              <span className="iconx-globe1"></span>Regions
            </button>
            <button onClick={() => setDrawerFilters(true)} className="btn btn-sm btn-default open-more-modal btn-w_icon ml--10">
          ...
          <div className="col-lg-6 col-md-6 text-leftt float-right filter-tags">
            ...
            <div className={`scrolling-_-wrapper text-right ${selectedTags.length > 5 ? " right-_30" : ""}`}>
              {selectedTags
            ...
            {selectedTags.length > 5 ? (
              <button
                ...
                title={selectedTags.length === 0 ? "No filtered tags" : "Show all filtered tags"}
              >
      ...
      <Drawer
        ...
        title="Filter"
        placement={"right"}
        width={(filterPreference.length < 3 ? 60 : 60 + (filterPreference.length - 3) * 20) + "%"}
      ...
      >
        ...
        <button onClick={() => setDrawerFilters(false)} type="button" className="btn btn-sm btn-default open-more-_filtrs">
          Close
        </button>
      ...
      <Drawer
        ...
        footer={[
          <button onClick={() => setDrawerDemos(false)} type="button" className="btn btn-sm btn-default">
            Close
          </button>,
        ]}
      >
      ...
      >
        ...
        <div className="text-leftt filter-tags drawer-tags mb-5">
          {["clients", "locations", "departments", "demographics", "regions"].map((type) => {
            if (selectedTags.filter((i) => i.type === type).length) {
              return (
                <>
                  <p className="mb-5">
                    <h6 className="mb-2 d-block tags-_h6">{type.capitalize()}</h6>
                    {selectedTags
      ...
      <Drawer
        ...
        footer={[
          <button onClick={() => setDrawerRegions(false)} type="button" className="btn btn-sm btn-default">
            Close
          </button>,
        ]}
      >
