    ...
    <>
      <div className="multi-_step">
        <div className="row clearfix step-_header">
          ...
          <Link to={props.updateLink ? "/responder-home" : "/"} className="close-_step" title="Close">
        ...
        <div className="form-_actions min-h-366">
          ...
          <div className="row justify-content-center pb-5">
            ...
            <div className="col-12">
              <input onChange={radioChange} className="checkbox-tools" type="radio" name="comment" value="true" id="positive" />
              <label className="for-checkbox-tools to-_bigger" htmlFor="positive">
                <i className="icon_f-very-goodicon_f- give-_feedback rate-_positive" aria-hidden="true"></i>
                Positive
              </label>
              <input onChange={radioChange} className="checkbox-tools" type="radio" name="comment" value="false" id="negative" />
              <label className="for-checkbox-tools to-_bigger" htmlFor="negative">
                <i className="icon_f-very-badicon_f- give-_feedback rate-_negative" aria-hidden="true"></i>
                Negative
              </label>
            </div>
          </div>
...
const SelectLocation = (props) => {
  ...
  return (
    <>
            ...
            <div className="col-12 pb-15">
              ...
              >
                {props.questionOptions
                  ...
                  .map((item) => {
                    return (
                      <Option title={item.label} aria-label={item.label} tabIndex={0} value={item.id}>
                        {item.label}
                      </Option>
                    );
                  })}
      ...
      <Stats
        disable={
          location_type === undefined || (location_type === "One of my organisation’s other offices" && location_id === undefined)
        }
        step={2}
        {...props}
      />
...
const AnswerQuestion = (props) => {
  ...
  useEffect(() => {
    check();
    let l = props.selectedLocationId ? props.selectedLocationId : props.user.location_id;
    console.log(l);
    ...
    selections.forEach((item) => {
      item.theme_id.forEach((t) => {
        if (h.filter((i) => i.locations.includes(l)).filter((i) => i.themes.includes(t)).length > 0) {
          let helpdesk = h.filter((i) => i.locations.includes(l)).filter((i) => i.themes.includes(t));
  ...
  function populateSuggestions(e) {
    ...
    arr.forEach((item) => {
      buffer.forEach((i) => {
        i.label_arr.forEach((word) => {
          if (item !== "") {
            if (String(word).toLocaleLowerCase() === String(item).toLowerCase()) {
              exactMatch.push(i);
            } else if (String(word).toLocaleLowerCase().startsWith(item.toLocaleLowerCase())) {
              startsWith.push(i);
            } else if (String(word).toLowerCase().includes(item.toLowerCase())) {
              ...
              if (
                ...
                String(word).toLowerCase().startsWith(item.slice(0, -2).toLowerCase())
              ) {
                startsWith.push(i);
              }
            }
    ...
    suggestionArr = suggestionArr.filter((i) => !selections.map((o) => o.label.toLowerCase()).includes(i.label.toLowerCase()));
  ...
  function addTag(item) {
    ...
    if (selections.length >= maximumSelectableTag) {
      notification.warning({
        message: `You cannot select more then ${maximumSelectableTag + maximumSelectableTag <= 1 ? " tag" : " tags"}`,
      });
  ...
  function checkBlockWords(text) {
    const blockWords = ["paki", "nigger", "nigga", "cunt", "coon", "faggot", "homo", "whore", "motherfucker"];
  ...
  function send(input_, selections_) {
    ...
    console.log(props.alllocations.filter((i) => i.id === currentLocationId)[0].label);
    ...
    if (input_.length < 3) {
      ...
      let d = {
        ...
        location_title: props.alllocations.filter((i) => i.id === currentLocationId)[0].label,
      };
      console.log(d);
      axios
        .post("/feedbacks", d)
        .then((res) => {
          ...
          if (selectedHelpdesk.length > 0) {
            ...
            axios.post("/helpdesk", data).then((r) => {
              console.log(r);
              notification.success({ message: "Your feedback sent to helpdesk." });
            });
          }
          props.goToStep(4);
        })
        ...
        <div className="form-_actions min-h-366">
          ...
          <div className="row justify-content-center pb-5  auto-_resize">
            ...
            <div className="col-12 float-leftt">
              <span className="suggestions-title c-blue sub-_title">What is your feedback?</span>
            ...
            <div className="col-12 zero-top">
              {helpdesk[0] &&
                helpdesk[0].map((item) => {
                  return (
                    <label className="pt-20 fancy-checkbox responder-checkbox-form checkbox-flexible" htmlFor="flag">
                      <input
                        type="checkbox"
                        id="flag"
                        checked={selectedHelpdesk.includes(item.id)}
                        onChange={(e) => changeHelpDesk(e.target.value, item.id)}
                      />
                      <Tooltip
                      ...
                      >
                        <span className="light-black text-leftt">
                          Would you like to submit or flag issue to {item.title} helpdesk?
                        </span>
                      </Tooltip>
                    </label>
                  );
    ...
    <>
      <div className="multi-_step thanks-_step">
        ...
        <div className="form-_actions min-h-431">
          <div className="row clearfix step-_header">
            <div className="col text-left">
              <img className="thanks-_img" src={Wrap} title="That's a wrap" alt="That's a wrap"></img>
            </div>
          </div>
          <p className="text-centerr">Behind every response is an experience that matters</p>
          ...
          <div className="row justify-content-center text-centerr pb-5">
            ...
            {props.client.enable_questions && (
              <div className="col-12">
                <Link to="/answer-question" className="mb-0 btn btn-default to-_home">
                  Answer a question
                </Link>
              </div>
            )}
...
const Wizard = ({ setBgImage, openFullScreen, reduxClient }) => {
  ...
  function getFormData() {
    Promise.all([axios.get("/feedbacks/location/options"), axios.get("/tags/global"), axios.get("/helpdesk")])
      .then((value) => {
        console.log(value);
        ...
        AddMain(value[0].data.options, value[0].data.locations, value[0].data.user);
  ...
  function AddMain(options, locations, user) {
    ...
    console.log(options, userLocation, options.filter((i) => i.type === userLocation.code)[0]);
    ...
    setOtherLocations(locations.filter((i) => i.label !== userDefaultlocationTitle));
  }
    ...
    <>
      <div className="auth-main particles_js responder-form-center multi-_steps">
        <div className="auth_div responder-intro-card text-leftt">
          <div className={`card ${window.location.hash.substr(1) === " thanks-for-sharing" ? " thanks-for-sharing" : ""}`}>
            <div className="body">
              <StepWizard
              ...
              >
                ...
                />
                <ThankYou client={reduxClient} hashKey={"thanks-for-sharing"} updateLink={openFullScreen} refresh={refresh} />
              </StepWizard>
            </div>
          </div>
        </div>
...
const Stats = ({ nextStep, previousStep, totalSteps, step, disable, updateLink }) => (
  <div className="bottom">
    {step > 1 ? (
      ...
      <Link to={updateLink ? "/responder-home" : "/"} className="mb-0 float-l btn btn-default">
        Close
      </Link>
    )}
    {step < totalSteps ? (
      <button disabled={disable} className="mb-0 float-r btn btn-primary" onClick={nextStep}>
        Next
      </button>
    ) : (
