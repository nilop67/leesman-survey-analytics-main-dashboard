        ...
        <div className="row clearfix step-_header">
          ...
          <div className="col-auto">
            <div className="stamp" aria-hidden="true">
              <img src={InsideLogo} className="rp-_icon step-_icon" alt="Leesman inside icon" title="Leesman inside icon"></img>
            </div>
          </div>
        </div>
          ...
          <div className="row justify-content-center pb-5">
            <div className="col-12 pb-5">
              <p className="intro-_text">
                This is your space to provide ongoing feedback on your daily experience – wherever you’re working!
              </p>

              <p className="intro-_text">By joining the community, your voice can help shape better places for you to work.</p>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="multi-_step">
        ...
        <div className="form-_actions min-h-366">
          <div className="row justify-content-center pb-5">
            <div className="col-12 pb-5">
              <p className="intro-_text">
                You’ll receive the a few questions, but also feel free to submit any comments about your working environments,
                whenever you think we need to know about them.
              </p>

              <p className="intro-_text">
                First, we need to get to know you by creating your profile. It won’t take long and you can always come back and
                edit your details later.
              </p>
            </div>
          </div>
        </div>
  ...
  return (
    <>
          ...
          <div className="row justify-content-center pb-5">
            <div className="col-12 pb-15">
              <Input
                ...
                value={props.userData.first_name ? props.userData.first_name : props.data.user ? props.data.user.first_name : ""}
                style={{ width: 200 }}
              />
            </div>
            <div className="col-12 pb-15">
              <Input
                ...
                name="last_name"
                onChange={props.handleUserData}
                value={props.userData.last_name ? props.userData.last_name : props.data.user ? props.data.user.last_name : ""}
                style={{ width: 200 }}
              />
            </div>
              ...
              <div className="col-12 pb-15">
                ...
                >
                  {props.getOptions("DEMO_AGE").map((item) => {
                    return (
                      <Option title={item.option_label} aria-label={item.option_label} tabIndex={0} value={item.option_id}>
                        {item.option_label}
                      </Option>
                    );
                  })}
      ...
      <Stats
        ...
        {...props}
        disabled={
          (props.getOptions("DEMO_GEN").length > 0 && props.getSelected("DEMO_GEN").length < 1) ||
          (props.getOptions("DEMO_AGE").length > 0 && props.getSelected("DEMO_AGE").length < 1)
        }
      />
    </>
  );
  ...
  return (
    <>
      ...
      <div className="bottom">
        <button className="mb-0 float-l btn btn-default" onClick={props.previousStep}>
          Back
        </button>
        <button
          className="mb-0 float-r btn btn-primary"
          disabled={
            (props.getOptions("DEMO_ROLE").length > 0 && props.getSelected("DEMO_ROLE").length < 1) ||
            (props.getOptions("DEMO_EMP").length > 0 && props.getSelected("DEMO_EMP").length < 1) ||
            (props.getOptions("DEMO_TIME").length > 0 && props.getSelected("DEMO_TIME").length < 1) ||
            (location && location.option_label === "Office" && !props.userData.location_id) ||
...
const HomeWorking = (props) => {
  ...
  useEffect(() => {
    ...
    if (props.getOptions("DEMO_HOMESET").length === 0 && props.getOptions("DEMO_PRES").length === 0) {
      props.nextStep();
    }
  }, [props]);
  ...
  function handleSpecial(e) {
    ...
    const noone_id = options.filter((i) => i.option_label === "No one")[0].option_id;
    const nwfh = options.filter((i) => i.option_label === "I never work from home")[0].option_id;
                ...
                <div className="col-12 pb-15">
                  <div className="form-group">
                    {props.getOptions("DEMO_PRES").map((item) => {
                      return (
                        <label className="fancy-checkbox account-checkbox-form" htmlFor={item.option_id}>
                          <input
                            type="checkbox"
    ...
    <>
      <div className="multi-_step">
        ...
        <div className="form-_actions min-h-366 scrollable-_-366">
          <p>Finally, which activities are important to your role? Select all that apply.</p>
      ...
      <div className="bottom">
        ...
        <button
          className="mb-0 float-r btn btn-primary"
          disabled={props.getOptions("DEMO_ACT").length > 0 && props.getSelected("DEMO_ACT").length < 1}
          onClick={() => {
            props.send();
            props.lastStep();
          }}
    ...
    <>
      <div className="multi-_step">
        ...
        <div className="form-_actions min-h-431">
          <div className="row clearfix step-_header">
            <div className="col text-left">
              <img className="thanks-_img" src={SetupOk} alt="You're done" title="You're done"></img>
            </div>
          </div>
    ...
    <>
      <div className="auth-main particles_js responder-form-center multi-_steps">
        <div className="auth_div responder-intro-card text-leftt">
          <div className="card">
            <div className="body">
              <StepWizard isHashEnabled transitions={state.transitions} nav={<Steps />} isLazyMount={true}>
                <Intro1 />
                <Intro2 />
                <Account
                  data={data}
                ...
                <SelectLocation
                  ...
                  send={send}
                />
                {((data.enabledOptions && getOptions("DEMO_HOMESET").length > 0) ||
                  (data.enabledOptions && getOptions("DEMO_PRES").length > 0)) && (
                  <HomeWorking
                    data={data}
                    handleSelections={handleSelections}
...
const Stats = ({ nextStep, previousStep, totalSteps, step, disabled }) => (
  <div className="bottom">
    ...
    {step < totalSteps ? (
      <button className="mb-0 float-r btn btn-primary" disabled={disabled} onClick={nextStep}>
        Next
      </button>
    ) : (
