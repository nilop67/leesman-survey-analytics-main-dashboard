...
const Steps = (props) => {
  ...
  for (let i = 1; i <= props.totalSteps; i += 1) {
    ...
    dots.push(
      <span key={`step-${i}`} className={`step-_dot ${isActive ? "step-_active" : ""}`} title="Dot of step">
        &bull;
      </span>
    );
  }
    ...
    <>
      <div className="multi-_step">
        ...
        <div className="form-_actions min-h-366">
          <p>Thinking about the place/s you use for work, how would you rate your overall experience?</p>

          <div className="row justify-content-center pb-5">
            {/* !!! İFADE SEÇİMİ DEĞİŞTİ !!!*/}
            <div className="col-12 pb-5">
              <input onChange={change} value="-2" className="checkbox-tools" type="radio" name="xp" id="1" />
              <label className="for-checkbox-tools" htmlFor="1">
                <i className="icon_f-very-badicon_f- uil rate-first" aria-hidden="true"></i>
                <small className="emoji-screen-reader">Very bad</small>
              </label>
              <input onChange={change} value="-1" className="checkbox-tools" type="radio" name="xp" id="2" />
              <label className="for-checkbox-tools" htmlFor="2">
                <i className="icon_f-badicon_f- uil rate-2" aria-hidden="true"></i>
                <small className="emoji-screen-reader">Bad</small>
              </label>
              <input onChange={change} value="0" className="checkbox-tools" type="radio" name="xp" id="3" />
              <label className="for-checkbox-tools" htmlFor="3">
                <i className="icon_f-neutralicon_f- uil rate-3" aria-hidden="true"></i>
                <small className="emoji-screen-reader">Normal</small>
              </label>
              <input onChange={change} value="1" className="checkbox-tools" type="radio" name="xp" id="4" />
              <label className="for-checkbox-tools" htmlFor="4">
                <i className="icon_f-goodicon_f- uil rate-4" aria-hidden="true"></i>
                <small className="emoji-screen-reader">Good</small>
              </label>
              <input onChange={change} value="2" className="checkbox-tools" type="radio" name="xp" id="5" />
              <label className="for-checkbox-tools" htmlFor="5">
                <i className="icon_f-very-goodicon_f- uil rate-_last" aria-hidden="true"></i>
      ...
      <div className="bottom text-centerr">
        <button disabled={!score} className="mb-0 btn btn-_submit" onClick={send}>
          Submit
        </button>
      </div>
      {/* <Stats step={1} {...props} /> */}
    ...
    <>
      <div className="multi-_step">
        <div className="row clearfix step-_header">
          ...
          <div className="col-auto">
            <div className="stamp" aria-hidden="true">
              <img className="rp-_icon step-_icon" src={HandOk} alt="Okey icon" title="Okey icon"></img>
            </div>
          </div>
          <Link to={props.updateLink ? "/responder-home" : "/"} className="close-_step" title="Close">
            <i className="iconx-x"></i>
        ...
        <div className="form-_actions min-h-366">
          <p>Each time you rate your own experience you’re contributing to your company’s overall XP Score. Keep it up.</p>

          {props && props.avg > 0.0 ? (
            <div className="row row-_reset">
              <div className="col-4 col-lg-4 donut-respo">
                <span className="res-_chart-title let-_width">Company XP Score</span>

                <div className="donut-new donut-new-main" style={{ height: "120px" }}>
              ...
              <div className="col-8 col-lg-8 info-respo">
                <span className="res-_chart-title">Company XP Score breakdown</span>
...
const Wizard = ({ setBgImage, openFullScreen, reduxClient }) => {
  ...
  const xpAnswered = sessionStorage.getItem("xpAnswered") ? JSON.parse(sessionStorage.getItem("xpAnswered")) : false;
          ...
          <div className="card">
            <div className="body">
              <StepWizard
                ...
                isLazyMount={true}
              >
                {!xpAnswered && <Question setAvg={setAvg} setDonut={setDonut} updateLink={openFullScreen} />}
                <Result loading={loading} client={reduxClient} avg={avg} donut={donut} updateLink={openFullScreen} />
              </StepWizard>
            </div>
          </div>
        </div>
...
const Stats = ({ nextStep, totalSteps, step, updateLink, client }) => (
  <div className="bottom">
    {step < totalSteps ? (
      ...
      <>
        <Link className="mb-0 float-l btn btn-default" to={updateLink ? "/responder-home" : "/"}>
          Back to homepage
        </Link>
