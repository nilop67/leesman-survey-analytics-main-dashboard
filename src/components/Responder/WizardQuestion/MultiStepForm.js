...
const SelectLocation = (props) => {
  ...
  useLayoutEffect(() => {
    ...
    sessionStorage.getItem("selectedLocationName") && props.passLocation && props.nextStep();
  }, [props.passLocation]);
    ...
    <>
            ...
            <div className="col-12 pb-15">
              <Select
                ...
                onChange={(e, d) => {
                  ...
                  if (d.children === "One of my organisation’s other offices") {
                    ...
                    setLocationId();
                    props.setSelectedLocationName("Where you are working: " + d.children);
                  }
                }}
              ...
              >
                {props.questionOptions
                  ...
                  .map((item) => {
                    return (
                      <Option title={item.label} aria-label={item.label} tabIndex={0} key={item.id} value={item.id}>
                        {item.label}
                      </Option>
                    );
                  })}
              ...
              <div className="col-12">
                <Select
                  ...
                  onChange={(e, d) => {
                    ...
                    setLocationId(e);
                    props.setSelectedLocationName("Main office location: " + d.children);
                    console.log(e);
                  }}
                  optionFilterProp="children"
                  filterOption={(input, option) => option.children.toLowerCase().indexOf(input.toLowerCase()) >= 0}
      ...
      <Stats
        disable={
          ...
          (location_type === "One of my organisation’s other offices" && location_id === undefined)
        }
    ...
    <>
      <div className="multi-_step thanks-_step">
        ...
        <div className="form-_actions min-h-431">
          <div className="row clearfix step-_header">
            <div className="col text-left">
              <img className="thanks-_img" src={AnswerMore} alt="Answer more?" title="Answer more?"></img>
            </div>
          </div>
          <p className="text-centerr">If you’ve got a minute or two, there are a few more questions we’d love you to answer</p>
          ...
          <div className="row justify-content-center text-centerr pb-5">
            ...
            <div className="col-12">
              <Link to={props.updateLink ? "/responder-home" : "/"} className="mb-0 btn btn-default to-_home">
                Back to homepage
              </Link>
            </div>
          </div>
    ...
    <>
      <div className="multi-_step thanks-_step">
        ...
        <div className="form-_actions min-h-431">
          <div className="row clearfix step-_header">
            <div className="col text-left">
              <img className="thanks-_img" src={ThatsIt} alt="That's it for now" title="That's it for now"></img>
            </div>
          </div>
          <p className="white-_title text-centerr">
            There’s nothing more for you to answer at the moment. Check back soon or feel free to share your own comments.
          </p>
          ...
          <div className="row justify-content-center text-centerr pb-5">
            ...
            {props.client.enable_feedbacks && (
              <div className="col-12">
                <Link to="/share-thinking" className="mb-0 btn btn-default to-_home">
                  Share a thought
                </Link>
              </div>
            )}
...
const Wizard = ({ setBgImage, openFullScreen, totalSteps, reduxClient }) => {
  ...
  const getInit = () => {
    return Promise.all([axios.get("/campaigns/init"), axios.get("/feedbacks/location/options")]).then((value) => {
      console.log(value);
      decideRequest(value[0].data);
      setQuestionLabel(value[1].data.question_label);
      AddMain(value[1].data.options, value[1].data.locations, value[1].data.user);
  ...
  const decideRequest = (data) => {
    const selectedLocationType = sessionStorage.getItem("selectedLocationType")
      ...
      : null;
    const boolean_count =
      data.filter((i) => i.type === "boolean_scale").length > 0 ? data.filter((i) => i.type === "boolean_scale")[0].count : 0;
    const scale_count = data.filter((i) => i.type === "scale").length > 0 ? data.filter((i) => i.type === "scale")[0].count : 0;
    ...
    if (parseInt(scale_count) > 0 && !selectedLocationType) {
      ...
      console.log("decided to fetch scale questions by location type id", selectedLocationType);
  ...
  useEffect(() => {
    if (selectedLocationId) {
      ...
      if (otherLocations && otherLocations.length > 0) {
        setLocationName(otherLocations.filter((i) => i.id === selectedLocationId)[0].label);
      }
    }
  ...
  function send(bool, id, scale, comment) {
    let selected_location_type = selectedLocationType ? selectedLocationType : sessionStorage.getItem("selectedLocationType");
      ...
      <div className="auth-main particles_js responder-form-center multi-_steps">
        <div className="auth_div responder-intro-card text-leftt">
          <div className={`card ${backgroundColorCss}`}>
            <div className="body">
              {!loading ? (
                <StepWizard onStepChange={onStepChange} transitions={state.transitions} nav={<Steps />} isLazyMount={true}>
                  {!passLocation && (
                    <SelectLocation
                      selectLocationType={(e) => {
                        setSelectedLocationType(e);
                        if (e === locationOptions.filter((i) => i.type === "main_location")[0].id) {
                          let officeLocationId = locationOptions.filter((i) => i.type === "code_office")[0].id;
                          fetchQuestionByUrl(`/campaigns/scale/${officeLocationId}`);
                        } else {
                    ...
                    questions
                      .map((i) => {
                        if (i.label.includes("(name of building)")) {
                          return {
                            ...i,
                            label: i.label.replace("name of building", locationName),
                          };
                        } else return i;
                      })
                      .map((item, i) => {
                            ...
                            return (
                              <ScaleQuestion
                                ...
                                selectedLocationName={
                                  selectedLocationName ? selectedLocationName : sessionStorage.getItem("selectedLocationName")
                                }
                              />
                            );
                          case "boolean_scale":
...
const Stats = ({ nextStep, previousStep, totalSteps, step, updateLink, disable, fetching }) => {
  ...
  return (
    <div className="bottom">
      ...
      {step < totalSteps ? (
        <button className="mb-0 float-r btn btn-primary" disabled={disable} onClick={nextStep}>
