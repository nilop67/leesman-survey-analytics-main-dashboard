...
const Questions = (props) => {
  ...
  function getData() {
    Promise.all([axios.get(`/admin/clients/${clientId}/questions`), axios.get(`/admin/clients/${clientId}/questions/0`)])
      .then((value) => {
        console.log(value);
        let q = [];
        let m = [];
        ...
        if (!urlParam) {
          ...
          m = value[1].data.modules.filter((i) => String(i.client_id) === clientId);
          s = value[1].data.sections.filter((i) => String(i?.client_id) === clientId);
        }
        setQuestions(q);
        setBuffer(q);
        setModules(m);
        ...
        setLoading(false);
        val.length > 0 && customfilter(val, questions, ["heading", "section", "rule"], setBuffer);
        selectedSection.length > 0 && filterQuestionsBasedOnSection(selectedSection);
      })
      .catch((err) => {
        console.log(err);
  ...
  const sendCommonLocation = () => {
    axios.put(`/admin/clients/${clientId}/locations/code`, commonLocations).then((res) => {
      getData();
    });
  };
    ...
    <>
      <div className="container-fluid clients-page new-2022_class">
          ...
          <div className="row clearfix">
            <div className="col-md-12 col-sm-12">
              ...
              <ul className="nav nav-tabs3 mt-4 mb-2">
                ...
                <li className="nav-item">
                  <a
                    className={`nav-link ${selectedTab === "modules" && " active"}`}
                ...
                <li className="nav-item">
                  <a
                    className={`nav-link ${selectedTab === "sections" && " active"}`}
                ...
                {!urlParam && (
                  <li className="nav-item">
                    <a
                      className={`nav-link ${selectedTab === "common locations" && " active"}`}
            ...
            <div className="row mb-4 page-__header">
              <div className="col-xl-6 col-lg-6 col-md-6">
                ...
                {selectedTab === "questions" ? (
                  <span className="text-table-info">Total: {buffer.length} questions</span>
                ) : selectedTab === "modules" ? (
                  <span className="text-table-info">Total: {modules.length} modules</span>
                ) : (
                  <span className="text-table-info">Total: {commonLocations.length} common locations</span>
                )}
              </div>
        ...
        <div
          ...
          className={`modal fade bd-example-modal-lg ${state.openModal === "new-question" ? " d-block show" : ""}`}
        >
          {
            <QuestionModal
        ...
        <div className={`modal fade bd-example-modal-lg ${state.openModal === "new-module" ? " d-block show" : ""}`}>
          <SectionModuleModal
            scope={props.location.scope}
        ...
        <div className={`modal fade bd-example-modal-lg ${state.openModal === "new-section" ? " d-block show" : ""}`}>
          <SectionModuleModal
            scope={props.location.scope}
