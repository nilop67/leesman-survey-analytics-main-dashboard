...
export default function ModuleModal({ parents, selectedTag, scope, close }) {
  ...
  function send() {
    ...
    if (scope === "all") {
      ...
      d = { ...formData, type: "tag", client_id: localStorage.getItem("selectedClientId") };
    }

    if (formData.id) {
      axios
        .put(
          `/admin/clients/${localStorage.getItem("selectedClientId") ? localStorage.getItem("selectedClientId") : "0"}/tags/${
            formData.id
          }`,
          d
        )
      ...
      <div className="modal-content" style={{ padding: "15px" }}>
        <div className="row clearfix">
          ...
          {!formData.id && (
            <div className="col-lg-12 col-md-12 col-sm-12">
              <div className="form-group">
                <select value={formData.parent_id} onChange={change} name="parent_id" className="form-control show-tick">
