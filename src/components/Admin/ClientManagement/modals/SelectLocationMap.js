...
function SelectLocationMap(props) {
  ...
  const getLatLng = (country, city, postCode) => {
    return Geocode.fromAddress(`${getCountryNameByShortCode(country)} ,${city}, ${postCode}`).then(
      (res) => {
  ...
  const AsyncMap = withScriptjs(
    withGoogleMap(() => (
      <GoogleMap google={props.google} defaultZoom={props.zoom} defaultCenter={{ lat: mapPosition.lat, lng: mapPosition.lng }}>
        {/* marker üstündeki tooltip */}
        {state.city ||
          (state.address && (
            <InfoWindow onClose={onInfoWindowClose} position={{ lat: markerPosition.lat + 0.0018, lng: markerPosition.lng }}>
              <div>
                <span style={{ padding: 0, margin: 0 }}>{props.city ? props.city : state.address}</span>
              </div>
            </InfoWindow>
          ))}
        {/*Marker*/}
        ...
        {props.radius && <Circle defaultCenter={{ lat: mapPosition.lat, lng: mapPosition.lng }} radius={props.radius} />}
        <Autocomplete
          style={{
            width: "100%",
            height: "40px",
...
export default React.memo(SelectLocationMap, (props, nextProps) => {
  return props.city === nextProps.city && props.country === nextProps.country && props.postCode === nextProps.postCode;
});
