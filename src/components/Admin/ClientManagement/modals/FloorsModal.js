...
export default function FloorsModal({ clientId, close, location_id, data, refresh, locations }) {
  ...
  function sendFloor(id) {
    if (data.map((i) => i.label).includes(floors.filter((i) => i.id === id)[0]?.label)) {
  ...
  return (
    <>
      ...
      <div className="col-lg-8 col-md-8 col-sm-12 floor-_Add">
        ...
        <button type="button" onClick={sendNewFloor} className="btn btn-sm btn-primary">
          Add
        </button>
      </div>
      {floors?.length > 0 &&
