...
import { getPreferencesDemographicAction, sendPreferencesDemographicAction } from '../../../actions/adminActions'

const Demographics = () => {
  const dispatch = useDispatch()
  const { options, data, error } = useSelector(store => store.getPreferencesDemographic)
  const { error: sendError } = useSelector(store => store.sendPreferencesDemographic)
  ...
  useEffect(() => {
    setSelectedHeadings(options.filter((i) => i.options.filter((o) => o.is_selected === 1).length > 0).map((i) => i.heading));
  }, [options]);
  useEffect(() => {
    populate(options);
              ...
              <div className="body">
                <div className="row clearfix">
                  ...
                  <div className="col-xl-12 col-lg-12 col-md-12">
                    ...
                    &nbsp;&nbsp;
                    <a onClick={sendPreferences} className="btn btn-sm btn-primary mr-1 float-r">
                      Save
                    </a>
                  </div>
                </div>
