...
export default function LocationsList(props) {
  ...
  const filterData = (e) => {
    ...
    locationData.forEach((item) => {
      if (
        ...
        String(item.location_group_name).toLowerCase().includes(e.toLowerCase()) ||
        ...
        String(item.building_location).toLowerCase().includes(e.toLowerCase()) ||
        String(item.address).toLowerCase().includes(e.toLowerCase()) ||
        String(item.building_style).toLowerCase().includes(e.toLowerCase()) ||
        String(item.date_organisation_moved_in).toLowerCase().includes(e.toLowerCase())
      ) {
        buffer.push(item);
      }
    });
  ...
  const columns = [
    ...
    },
    {
      ...
      sorter: (b, a) => {
        if (a.location_group_name.toLowerCase() < b.location_group_name.toLowerCase()) return -1;
        if (a.location_group_name.toLowerCase() > b.location_group_name.toLowerCase()) return 1;
        return 0;
      },
    },
    ...
    },
    {
      ...
      render: (value, record) => (
        <>
          ...
          <Switch checked={record.enable} onChange={() => onChange(record.key, record.enable)} style={{ marginLeft: "15px" }} />
        </>
      ),
      fixed: "right",
    },
  ...
  return (
    <>
        ...
        <div className="col-xl-12 col-lg-12 col-md-12">
          <div className="row mb-4 page-__header">
            <div className="col-xl-6 col-lg-6 col-md-6">
              ...
              <span className="text-table-info">Total: {locationData.filter((i) => !i.is_location_group).length} locations</span>
            </div>
            <div className="col-xl-6 col-lg-6 col-md-6 jus-end toggle-_top">
              ...
              <div className="input-group ml-3">
                ...
                <input value={query} onChange={change} type="text" className="form-control" placeholder="Search" />
              </div>{" "}
              <button onClick={() => deleteAllNonUsedlocations()} className="btn btn-primary bigger-_btn ml-3 float-r">
      ...
      <div className={`modal fade bd-example-modal-lg ${state.isLocation === "location-edit-add" ? " d-block show" : ""}`}>
        <AddLocation
          refresh={getLocations}
          close={closeLocation}
