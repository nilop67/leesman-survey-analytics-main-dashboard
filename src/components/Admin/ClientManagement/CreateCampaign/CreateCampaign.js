...
const CreateCampaign = ({
  ...
  function deselectModules(q) {
    ...
    if (questions_.length > 0 && questions_[0] && flag) {
      let questionsByModules = uniqueModules(questions).map((module) => {
        return {
          module: module,
          question_ids: questions.filter((i) => i.modules.includes(module)).map((i) => i.id),
        };
      });
      ...
      modules.forEach((i) => {
        i.forEach((o) => {
          if (questionsByModules.filter((i) => i.module === o)[0].question_ids.Contains(questions_.map((i) => i.id))) {
            m.push(o);
          }
        });
      });
  ...
  function removeQuestions(module, allQuestions) {
    ...
    let filtered = selectedQuestions.filter((item) => !questionsId.includes(item));
    ...
    let modules = uModules.map((m) => {
      return (
        <label className="fancy-checkbox min-w-168">
          <input checked={selectedModules.includes(m)} onChange={selectModule} type="checkbox" name="modules" value={m} />
  ...
  function populateSection(module) {
    ...
    module.forEach((element) => {
      let q = [...questions_all, ...questions.filter((i) => i.modules.includes(element))];
      questions_all = q;
    });
          ...
          <div className="col-xl-12 col-lg-12 col-md-12">
            <div className="card">
              <div className="body min-h-381">
                ...
                <div className="row clearfix pb-20 modules--border mb--55">{heads}</div>
                  ...
                  <button
                    ...
                    onClick={() => {
                      selectedQuestions.length > 0
                        ? index(1)
                        : notification.warning({ message: "Please select at least one question" });
                    }}
                  >
                    Next
                  </button>
