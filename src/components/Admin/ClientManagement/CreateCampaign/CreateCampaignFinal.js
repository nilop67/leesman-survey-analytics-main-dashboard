...
const CreateCampaignFinal = ({ printSummary, formData, index, selections, send, SetSelections, isEdit }) => {
  ...
  }, [selectedDemos, selectedLocations, selectedDepartments, title, description, check, start, end, status]);
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    setStart(selections.start ? selections.start : new Date(moment().format("YYYY-MM-DD")).toISOString());
    setEnd(isEdit && selections.end ? selections.end : new Date(moment().add(14, "days").format("YYYY-MM-DD")).toISOString());
  ...
  const handleLocationsByRegion = (regionIds) => {
    let buffer = formData.locations.filter((i) => regionIds.includes(parseInt(i.region_id)) || !i.region_id);
  ...
  const changeDate = (e) => {
    let start_ = new Date(moment(e[0]).format("YYYY-MM-DD HH:mm:ss")).toISOString();
    let end_ = new Date(moment(e[1]).format("YYYY-MM-DD HH:mm:ss")).toISOString();
    setStart(start_);
    setEnd(end_);
  };
            ...
            <div className="card">
              <div className="body min-h-381">
                      ...
                      <span className="light-black list-group-item min-h-396">
                        <div className="row">
                          ...
                          <div className="col-lg-12 col-md-12">
                            ...
                            >
                              <label className="fancy-checkbox">
                                <input type="checkbox" name="modules" checked={check} onClick={() => setCheck((pre) => !pre)} />
                                <span className="light-black">
                          ...
                          <>
                            ...
                            <Tooltip title="Start date">
                              <DatePicker
                                ...
                                value={start ? moment(start ? start.splitDate() : null) : moment()}
                              />
                            </Tooltip>
                            <Tooltip title="End date">
                              <DatePicker
                                ...
                                format="YYYY-MM-DD"
                                defaultPickerValue={moment()}
                                value={end ? moment(end ? end.splitDate() : null) : moment()}
                              />
                            </Tooltip>
                          </>
                        )}
                        ...
                        <div className="row clearfix pb-20">
                          <div className="col-lg-12 col-md-12 campaigns-fancy  pt-20">
                            <input
                              ...
                              placeholder="Search.."
                            ></input>
                            <div className="targets-list-scrollign">{departments}</div>
                          </div>
                        </div>
                      </span>
                    </div>
                          ...
                          <div className="col-lg-12 col-md-12 campaigns-fancy  pt-20">
                            <label className="fancy-checkbox">
                              <input
                                ...
                                checked={formData.regions.length === selectedRegions.length}
                                type="checkbox"
                                name="modules"
                              />
                              <span className="light-black">
                                  ...
                                  return (
                                    <label className="fancy-checkbox">
                                      <input
                                        ...
                                        checked={selectedRegions.includes(i.id) || isAllLocationInThisRegion(i.id)}
                                        name="modules"
                                      />
                ...
                <div className="bottom-btns">
                  <button onClick={() => index(0)} className="btn btn-sm btn-default mr-1 float-l">
                    Back
                  </button>
                      ...
                      onClick={() =>
                        printSummary(
                          ...
                          selectedDepartments.map((item) => {
                            return allDepartments.filter((i) => i.id === item)[0].label;
                          }),
                          selectedLocations.map((item) => {
                            return allLocations.filter((i) => i.id === item)[0].label;
                          }),
                          [
                            ...new Set(
                              selectedDemos.map((item) => {
                                return allDemos.filter((i) => i.option_id === item)[0].heading;
                              })
                            ),
                          ],
