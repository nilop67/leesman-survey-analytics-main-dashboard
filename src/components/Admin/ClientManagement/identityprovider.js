...
function IdentityProvider(props) {
  ...
  useEffect(() => {
    API.get("CoreAPI", `/admin/clients/${clientId}/identityprovider`)
      ...
      .catch((err) => {
        console.error(err);
        notification.error({ message: "Unable to fetch idenity provider details" });
      });
  ...
  const confirmDelete = () => {
    ...
    API.del("CoreAPI", `/admin/clients/${clientId}/identityprovider`)
      .then((res) => {
        ...
        notification.success({ message: "Successfully deleted identity provider" });
        history.go(0);
      })
      .catch((err) => {
        console.error(err);
  ...
  const save = () => {
    if (domains.length === 0) {
      notification.warning({ message: "Please verify domains before setting up an identity provider" });
      return;
    }
    if (!!providerData?.ProviderDetails?.MetadataURL) {
      ...
      API.put("CoreAPI", `/admin/clients/${clientId}/identityprovider`, { body })
        .then((res) => {
          console.log("PUT /client/identityprovider", res.result);
          notification.success({ message: "Successfully updated identity provider" });
          history.go(0);
        })
        .catch((err) => {
          console.error(err);
      ...
      API.post("CoreAPI", `/admin/clients/${clientId}/identityprovider`, { body })
        .then((res) => {
          console.log("POST /client/identityprovider", res.result);
          notification.success({ message: "Successfully added identity provider" });
          history.go(0);
        })
        .catch((err) => {
          console.error(err);
    ...
    <>
      <Modal
        ...
        footer={[
          ...
          <button onClick={cancelDelete} type="button" className="btn btn-sm btn-default">
            <span>Cancel</span>
          </button>,
          <button
      ...
      >
      <p className="light-black">Are you sure you want to remove the provider?</p></Modal>
                ...
                <div className="body body-_sso">
                  <div className="row clearfix">
                    ...
                    <div className="col-lg-12 col-md-12">
                      <p>
                        <strong className="light-black">Entity Id: </strong>urn:amazon:cognito:sp:
                        {process.env.REACT_APP_COGNITO_USER_POOL_ID}
                        <br />
                        <strong className="light-black">Reply URL (Assertion Consumer Service URL): </strong>https://
                    ...
                    <div className="col-lg-12 col-md-12">
                      <div className="form-group">
                        <Input
                          type="text"
                          name="metadataurl"
                          value={providerData?.ProviderDetails?.MetadataURL || formData.metadataurl}
                          onChange={onChange}
                    ...
                    <div className="col-lg-12 col-md-12">
                      <List
                        ...
                        size="small"
                        dataSource={domains}
                        renderItem={(item) => <List.Item className="form-control">{item.name}</List.Item>}
                      />
                    </div>
                    <div className="col-lg-12 col-md-12 bottom-_btns">
                      <button onClick={promptDelete} className="btn btn-sm btn-default mr-1 float-l">
                        Delete
                      </button>{" "}
                      &nbsp;&nbsp;
                      <button onClick={save} className="btn btn-sm btn-primary mr-1 float-r">
                        Save
                      </button>
                    </div>
                  </div>
