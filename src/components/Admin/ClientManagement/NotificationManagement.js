...
const NotificationManagement = () => {
  ...
  const { questions,
    ...
    campaigns,
    notifications,
    data } = useSelector(store => store.getNotificationManagement)
  ...
  useEffect(() => {
    ...
    campaigns.length > 0 &&
      selectedCampaign &&
      setSelectedQuestions(campaigns.filter((i) => i.id === parseInt(selectedCampaign))[0].questions);
  }, [selectedCampaign]);
  ...
  const send = () => {
    ...
    if (selectedLocations.length < 1) {
      notification.warning({ message: "Please select at least one Location", placement: "topLeft" });
    } else if (emails.length < 1) {
      notification.warning({ message: "Please enter at least one email", placement: "topLeft" });
    } else if (title.length < 3) {
      notification.warning({ message: "Please enter a title", placement: "topLeft" });
    } else if (!count) {
      notification.warning({ message: "Please enter responder count", placement: "topLeft" });
    } else if (!score) {
      notification.warning({ message: "Please enter a threshould score (0-100)", placement: "topLeft" });
    } else if (notificationType === "admin_question" && selectedQuestions.length < 1) {
      notification.warning({ message: "Please select at least one question", placement: "topLeft" });
    } else if (notificationType === "admin_campaign" && !selectedCampaign) {
      notification.warning({ message: "Please select a campaign", placement: "topLeft" });
    } else {
      if (data.id) {
        console.log(data);
        axios
      ...
      >
        <div className="row clearfix">
          ...
          {emails.map((i) => {
            return (
              <div className="col-lg-12 col-md-12 multiple--add-location d-flex">
                <div className="form-group">
                  <input type="email" className="form-control" value={i} disabled />
                </div>
                <button type="button" onClick={() => removeEmail(i)} className="btn btn-sm btn-default add--q" title="Delete">
          ...
          <div className="col-lg-12 col-md-12 multiple--add-location d-flex">
            ...
            <button type="button" onClick={() => addEmail()} className="btn btn-sm btn-default add--q" title="Add">
          ...
          {(notificationType === "admin_question" || notificationType === "admin_campaign") && (
                              ...
                              <tr>
                                <td>
                                  <div className="d-flex text-left">
                                    <div>
                                      <p className="mb-0">
                                        {questions.filter((i) => i.id === q)[0] && questions.filter((i) => i.id === q)[0].heading}
                                      </p>
                                    </div>
                                  </div>
                                </td>
                                <td className="text-right">
                                  <p className="mb-0">
                                    {questions.filter((i) => i.id === q)[0] &&
                                      questions.filter((i) => i.id === q)[0].modules.toString()}
                                  </p>
                                </td>
                                <td className="text-right">
                                  <button
        ...
        <div className="dual--screen-bottom">
          <button type="button" className="btn btn-sm btn-secondary" onClick={() => setIsModalVisible(false)}>
            Close
          </button>
          ...
          >
            <div className="row clearfix">
              <div className="col-md-12 col-sm-12">
                <div className="form-group">
                  {/* modül seç */}
                  <select onChange={(e) => setSelectedModules(e.target.value)} className="form-control">
                ...
                <div className="questions--list-s">
                  ...
                  <div className="text-leftt light-black pb-20 pt--40">{selectedModules}</div>
                  {/* başlığa ait liste */}
                  {questions &&
                    questions
            ...
            <div className="dual--screen-bottom">
              <button type="button" className="btn btn-sm btn-secondary" onClick={() => setIsChildrenModalVisible(false)}>
                Close
              </button>
            </div>
          </Drawer>
                          ...
                          <div className="card">
                            <div className="body noti-rule--body">
                              <div className="d-flex">
                                {/* noti title */}
                                <div className="m-0 text-left alarm--title">{i.title}</div>
                              ...
                              <div className="bottom-_-infos">
                                <div className="text-muted dp--block mt-2">
                                  ...
                                  <ul className="noti-_ul mb-0">
                                    ...
                                    <li>when {i.respondent_count} users answer score</li>
                                    <li>
                              ...
                              <div className="d-flex mt-2">
                                ...
                                <div>
                                  ...
                                  <a
                                    onClick={() =>
                                      window.confirm("Are you sure to delete that notification?") && deleteNotification(i.id)
                                    }
