...
const TranslationTemplate = ({ type, languages, selectedLanguageIds, selections }) => {
  let langsArr = localStorage.getItem("selectedLangsArr") ? JSON.parse(localStorage.getItem("selectedLangsArr")) : [];
  let allLanguages_ = localStorage.getItem("allLanguages") ? JSON.parse(localStorage.getItem("allLanguages")) : [];
  ...
  const getTranslations = () => {
    axios
      ...
      .then((res) => {
        ...
        handleResultData(
          res.data.result.filter((i) => selectedLanguageIds?.includes(i.language_id) && selections.includes(i.id))
        );
        handleColumns(res.data.result.filter((i) => selectedLanguageIds?.includes(i.language_id) && selections?.includes(i.id)));
        setClientData(res.data.result.filter((i) => selectedLanguageIds?.includes(i.language_id) && selections?.includes(i.id)));
      })
      .catch((err) => {
        console.log(err);
    ...
    let column_ = [
      {
        ...
        render: (_, record) => {
          ...
          return editable ? (
            ...
            <a disabled={editingKey !== ""} onClick={() => edit(record)} className="btn btn-sm btn-liste cursorp">
