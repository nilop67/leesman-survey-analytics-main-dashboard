...
const Languages = () => {
  ...
  const [defaultLangId, setDefaultLangId] = useState(localStorage.getItem("selectedClientLang"));
  const [selectedLangsArr, setSelectedLangsArr] = useState(
    localStorage.getItem("selectedLangsArr") ? JSON.parse(localStorage.getItem("selectedLangsArr")) : []
  );
  ...
  const handleCheck = (e) => {
    let { value } = e.target;
    console.log(value);
    let arr = localStorage.getItem("selectedLangsArr") ? JSON.parse(localStorage.getItem("selectedLangsArr")) : [];
  ...
  const save = () => {
    axios
      ...
      .then((res) => {
        ...
        localStorage.setItem("selectedClientLang", res.data.result[0].default_language_id);
      })
      .catch((err) => {
        console.log(err);
          ...
          <div className="col-xl-12 col-lg-12 col-md-12">
            <div className="row mb-4 page-__header">
              ...
              <div className="col-xl-6 col-lg-6 col-md-6 jus-end">
                <Link to={"/translations?client_id=" + clientId} className="btn btn-primary ml-3" title="">
                  Translations
                </Link>
              </div>
            </div>
              ...
              <div className="body">
                <div className="row clearfix">
                  <div className="col-lg-12 col-md-12">
                    <div className="text-leftt light-black pb-20">All languages</div>

                    {boxAll}
                  </div>

                  <div className="col-lg-12 col-md-12 pt-40">
                    <div className="text-leftt light-black pb-20">Default language</div>

                    {boxSelected}
                  </div>

                  <div className="col-lg-12 col-md-12 mt-5">
                    <Link to="/clients" className="btn btn-sm btn-default mr-1 float-l">
                      Cancel
                    </Link>{" "}
                    &nbsp;&nbsp;
