...
import { getlocationGroupListAction , getLocationDataAction} from "../../../actions/adminActions";
 
export default function LocationGroupList(props) {
  const {locationGroupData} = useSelector(store => store.getlocationGroupList) 
  ...
  const createLocationGroup = () => {
    axios
      .post(`/admin/clients/${clientId}/locations/location_groups`, { name: name })
      .then((res) => {
  ...
  const sendLocations = () => {
    axios
      .put(`/admin/clients/${clientId}/locations/${selectedLocationGroup}/location_groups`, selectedLocations)
      .then((res) => {
        console.log(res.data);
  ...
  return (
    <>
      <Drawer
        ...
        footer={[
          <button onClick={() => sendLocations()} type="button" className="btn btn-sm btn-primary">
            Save
          </button>,
          <button onClick={() => setSelectedLocationGroup()} type="button" className="btn btn-sm btn-default">
            Close
          </button>,
        ]}
      >
        <div className={`filters-_column-1 mr-1`}>
          <Tree
            ...
            treeData={dbToTree(
              locationData.filter(
                (i) => (!i.location_group_id || i.location_group_id === selectedLocationGroup) && !i.is_location_group === true
              )
            )}
          />
        </div>
          ...
          <div className="row mb-4 page-__header">
            <div className="col-xl-6 col-lg-6 col-md-6">
              ...
              <span className="text-table-info">
                Total: {locationData.filter((i) => i.is_location_group).length} location Groups
              </span>
            </div>
