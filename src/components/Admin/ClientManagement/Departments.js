...
import {toggleEnableDepartmentAction, deleteAllNonUsedDepartmentsAction, sendDepartmentsDepartmentsAction, sendDepartmentsDepartmentsEditAction} from '../../../actions/adminActions'
import {useDispatch, useSelector} from 'react-redux'
const Departments = () => {
  const dispatch = useDispatch()
  const {error} = useSelector(store => store.toggleEnableDepartment)
  const {error: delErr} = useSelector(store => store.deleteAllNonUsedDepartments)
  ...
  const [subDepartmentDrawerVisible, setSubDepartmentDrawerVisible] = useState(false);
        ...
        <a className={`list-group-item department-_list ${isEdit && "edited"}`}>
          ...
          <div className="float-right department-_right-side">
            ...
            {item.isReferenced !== undefined && !item.isReferenced && (
              <a onClick={() => deleteDepartment(item.id)} className="btn btn-sm cursorp icon-_department">
  ...
  const updateDepartments = () => {
    axios
      .put(`/admin/clients/${clientId}/departments`, { cascade: true, data: nestedData })
      .then((res) => {
  ...
  const checkAndFilterTranslations = (e) => {
    ...
    let filtered = arr.filter((i) => i.language_id === parseInt(ClientDefaultLanguage));
  ...
  const checkChild = (item) => {
    let findChild = [];
    findChild = departments.filter((i) => parseInt(i.parent_id) === parseInt(item.id));

    findChild = findChild.map((i) => {
      let c = item.color ? "dd-handle- bg-" + item.color : "dd-handle- bg-department";
      return [
        {
          id: i.id,
  ...
  const getDepth = (nest) => {
    let depthArr = Array.apply(0, Array(nest.filter((i) => i.children.length === 0).length));
    let count = 0;
  ...
  const updateSubDepartmentLabel = () => {
    axios.put(`/admin/clients/${clientId}`, { department_labels: subDepartmentLabels }).then((res) => {
          ...
          <>
            <div className="row clearfix">
                        ...
                        <div className="form-group">
                          <label className="info--_text" htmlFor="first_name">
                            <span className="info--sub-title">
                              {index === 0 ? `Parent department label` : `Sub department label level:${index}`}
                            </span>
                          </label>
                          <input
                            type="text"
              ...
              <div className="col-lg-12 col-md-12 mt-4">
                <button onClick={updateSubDepartmentLabel} className="btn btn-sm btn-primary float-l">
                  Change
                </button>
              </div>
            </div>
              ...
              <ul className="nav nav-tabs3 mb-2 mt-4">
                {!cascadingAvailable ? (
                  <li className="nav-item">
                    <a
                      className={`nav-link ${openSection === "dropdown" && " active"}`}
                  ...
                  <li className="nav-item">
                    <a
                      className={`nav-link ${openSection === "cascading" && " active"}`}
          ...
          <div className="col-xl-12 col-lg-12 col-md-12">
            <div className="row mb-4 page-__header">
              <div className="col-xl-6 col-lg-6 col-md-6">
                ...
                <span className="text-table-info">Total: {nestDropdown.length} departments</span>
              </div>
              <div className="col-xl-6 col-lg-6 col-md-6 jus-end">
                ...
                <button className="btn btn-primary bigger-_btn after-_r-border ml-2" onClick={handleAddClick}>
                  Add department
                </button>
                <button onClick={() => setSubDepartmentDrawerVisible(true)} className="btn btn-primary bigger-_btn ml-3">
                  Change label
                </button>
              </div>
            </div>
              ...
              <>
                <div className="card mt-4">
                  <div className="body min-h-381">
                    {" "}
                    <button onClick={() => deleteAllNonUsedDepartments()} className="btn btn-primary bigger-_btn ml-3 float-r">
                      Delete all non-used departments
                    </button>
                    <h5 className="mb-0 text-leftt light-black">Department hierarchy</h5>
                    <p className="text-leftt light-black">Keep the items to be sublisted and slide to the right.</p>{" "}
                    <div className="dd dd4">
                      ...
                      <ol className="dd-list">
                        <Nestable
                          items={openSection === "dropdown" ? nestDropdown : nestCascading}
