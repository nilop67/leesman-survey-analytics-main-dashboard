  ...
  const send = (type, data) => {
    ...
    if (!surveyId) {
      axios
        ...
        .then((surveyId) => {
          history.push(`/create-edit-survey?client_id=${clientId}&survey_id=${surveyId.data}`);
          setLoading(false);
        })
        .catch((err) => {
          setLoading(false);
      ...
      axios
        .put(`/admin/clients/${clientId}/surveys/${surveyId}`, { data: data, type: type })
        .then((data) => {
          setLoading(false);
      ...
      <ul className="nav nav-tabs3 mt-4 mb-4">
        {!surveyId ? (
          <li className="nav-item">
            <a
              className={`nav-link ${selectedTab === "general setup" && " active"}`}
