...
import { Drawer, Table, Input, Popconfirm, Form, Typography, notification } from "antd";
import { CustomTreeColumn, EditableCell } from "../../../../common/commonComponents/formItems";
...
const Translations = ({ initData, setInitData, surveyId, clientId, formData, setFormData, close, send, reset }) => {
  ...
  const [selectedLanguages, setSelectedLanguages] = useState(formData.languages?.length ? formData.languages : [12]);
    ...
    },
    {
      ...
      render: (record) => (
        <>
          <>
            <a
              style={{ width: "100%", marginBottom: "5px", height: "30px", padding: "5px" }}
              href={`#!`}
              className="btn btn-sm btn-liste cursorp"
              title="Edit"
              onClick={() => {
                setLabel(record?.translations?.filter((i) => i.language_id === 12)[0]?.label);
  ...
  useEffect(() => {
    if (formData?.translations) {
      ...
      if (d.length) {
        ...
        d.forEach((row) => {
          let buffer = { type: tab, id: tab === "customisations" ? row.label : row.id };
          selectedLanguages.forEach((l) => {
            buffer = {
              ...buffer,
              [initData?.languages?.filter((i) => i.id === l)[0].label]: row?.translations?.filter((i) => i.language_id === l)[0]
                ?.label,
            };
          });
  ...
  useEffect(() => {
    setLabel(currentTranslation?.translations?.filter((i) => i.language_id === selectedLanguage)[0]?.label || "");
  }, [selectedLanguage]);
      ...
      <div className="drawer_sc-div create--sr" id="survey-_section">
        ...
        >
              ...
              <div className="form-group">
                ...
                >
                  {selectedLanguages.map((i) => {
                    return <option value={i}>{initData?.languages?.filter((o) => o.id === i)[0]?.label}</option>;
                  })}
                </select>
              </div>
            </div>
            <div className="col-lg-12 col-md-12">
              <div className="form-group">
                <span className="light-black">
                  {"English translation: " + currentTranslation?.translations?.filter((i) => i.language_id === 12)[0]?.label}
                </span>
              </div>
            </div>
            <div className="col-lg-12 col-md-12">
              <div className="form-group">
                <span className="light-black">
                  {"selected language translation: " +
                    currentTranslation?.translations?.filter((i) => i.language_id === selectedLanguage)[0]?.label}
                </span>
              </div>
            </div>
          ...
          <div className="dual--screen-bottom">
            <button type="button" className="btn btn-sm btn-secondary" onClick={() => setDrawerVisible(false)}>
              Close
            </button>
            ...
            <div className="row clearfix">
              ...
              <div className="col-lg-12 col-md-12 without-btn-table">
                <nav className="nav nav-tabs" role="tablist">
                  ...
                  <a
                    href="#!"
                    className={`custom_tab nav-item nav-link ${tab === "departments" && " active"}`}
                    onClick={() => setTab("departments")}
                  >
                    Departments
                  </a>
                  <a
                    className={`custom_tab nav-item nav-link ${tab === "questions" && " active"}`}
                  ...
                  <a
                    className={`custom_tab nav-item nav-link ${tab === "options" && " active"}`}
                  ...
                  <a
                    className={`custom_tab nav-item nav-link ${tab === "customisations" && " active"}`}
                  ...
                  <a
                    className={`custom_tab nav-item nav-link ${tab === "tailored" && " active"}`}
                  ...
                  <a
                    className={`custom_tab nav-item nav-link ${tab === "departmentLabels" && " active"}`}
