...
import { TreeColumn, EditableCell } from "../../../../common/commonComponents/formItems";
import { dbToTree } from "../../../../../util/functions";
import { notification } from "../../../../../../node_modules/antd/lib/index";
import { sortableContainer, sortableElement, sortableHandle, arrayMove } from "react-sortable-hoc";
import axios from "axios";
import moment from "moment";
const Departments = ({ initData, setInitData, surveyId, clientId, formData, setFormData, close, send, reset }) => {
  ...
  useEffect(() => {
    setData(
      formData?.departments?.map((i, index) => {
        return { ...initData?.departments?.filter((l) => l.id === i)[0], key: index };
      })
    );
  }, [formData]);
  ...
  const SortableItemDepartments = sortableElement(({ value, id, key, sortable }) => (
    <li className="sort_item dark sections">
      {sortable && !(formData?.general?.status === "live" && moment(formData?.general?.end_date).isBefore(moment())) && (
        <span
          onClick={() => {
            console.log(value, id, key);
            setFormData((pre) => {
              return { ...pre, departments: formData?.departments?.filter((i) => i !== id) };
            });
          }}
          ...
          <div className="sentiment-wrapper">
            <div className="row clearfix">
              <div className="col-lg-12 col-md-12">
                ...
                <button onClick={() => setDepartments(true)} className="btn btn-outline-default btn-sm">
                  Select department(s)
                </button>
                  ...
                  {formData?.departments?.map((department, index) => {
                    return (
                      <SortableItemDepartments
                        key={department}
                        index={index}
                        value={initData?.departments?.filter((i) => i.id === department)[0]?.label}
                        id={department}
                        sortable={true}
                      />
                    );
