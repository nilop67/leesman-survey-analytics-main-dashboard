...
const Customization = ({ initData, setInitData, surveyId, clientId, formData, setFormData, close, send }) => {
  ...
  useEffect(() => {
    ...
    console.log(data);
  }, [contentIntro, contentClosing, contentWelcome, contentPagePopup, selectedPage]);

  useEffect(() => {
    console.log(formData?.customisations?.filter((i) => i.type === "intro" && i.language_id === 12)[0]?.label);
    setContentIntro(formData?.customisations?.filter((i) => i.type === "intro" && i.language_id === 12)[0]?.label);
    setContentClosing(formData?.customisations?.filter((i) => i.type === "close" && i.language_id === 12)[0]?.label);
    setContentWelcome(formData?.customisations?.filter((i) => i.type === "welcome_popup" && i.language_id === 12)[0]?.label);
    selectedPage &&
      setContentPagePopup(
        formData?.customisations?.filter((i) => i.type === `page_${selectedPage}_popup` && i.language_id === 12)[0]?.label || ""
      );
  ...
  return (
    <>
      ...
      <div className="drawer_sc-div create--sr" id="survey-_section">
        <Drawer title="Popups" placement="right" width="30%" onClose={() => setPopupVisible(false)} visible={popupVisible}>
              ...
              <div className="form-group">
                ...
                >
                  ...
                  {formData?.pagesAndQuestions?.pages.map((i) => {
                    return <option value={i.page_id}>{initData?.pages?.filter((p) => p.id === i.page_id)[0]?.name}</option>;
                  })}
                </select>
              </div>
              <textarea
                style={{ width: "100%", height: "400px", backgroundColor: "#303030", color: "white", fontSize: "20px" }}
                onChange={(e) => {
