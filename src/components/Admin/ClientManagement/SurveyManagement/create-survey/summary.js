...
const Summary = ({ initData, setInitData, surveyId, clientId, formData, setFormData, close, send }) => {
  ...
  useEffect(() => {
    ...
    setReadyToLive(summaryData?.length === summaryData?.filter((i) => i.state === "saved")?.length);
  }, [summaryData]);
  return (
    <>
              ...
              <div className="col-lg-12 col-md-12 mt-4 bottoms-_btn-group">
                ...
                {readyToLive && formData?.general?.status !== "live" && (
                  <>
                    ...
                    <Popconfirm
                      ...
                      onConfirm={() => {
                        send("general", { ...formData.general, live: true, deleteOldData: deleteOldData });
                      }}
                      okText="Yes"
                      cancelText="No"
                    >
                ...
                {formData?.cacheDate && formData?.cacheDate[0]?.updated_at && (
                  <div className="float-right">
                    <span>Cache Updated:{formData?.cacheDate && formData?.cacheDate[0]?.updated_at}</span>
                    <br></br>

                    <span>Cache Created:{formData?.cacheDate && formData?.cacheDate[0]?.created_at}</span>
                  </div>
                )}
                <button
                  onClick={() => {
