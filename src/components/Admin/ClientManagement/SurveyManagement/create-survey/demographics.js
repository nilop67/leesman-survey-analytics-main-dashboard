...
const Demographics = ({ initData, setInitData, surveyId, clientId, formData, setFormData, close, send, reset }) => {
  ...
  const deleteAdditionalQuestion = (id) => {
    if (formData?.general?.status === "live") {
      ...
      axios
        .put(`/admin/clients/${clientId}/surveys/${surveyId}/deleteadditionalquestion`, { question_id: id })
        .then((data) => {
          reset();
  ...
  const questionComponent = (demo, isDeletable) => {
    ...
    return (
      <div className="col-lg-3 col-md-3 surveys-fancy  pt-20">
        <div className="text-leftt light-black pb-20">{truncate(demo?.heading, 20)}</div>
            ...
            <div className="float-right">
              <label className="switch">
                <input
                  ...
                  type="checkbox"
                  name="Age"
                  checked={selectedOptions.hasIntersection(demo?.options?.map((i) => i.option_id))}
                />
            ...
            <div className="row clearfix">
              ...
              <div className="col-lg-12 col-md-12">
                <div className="row">
                  ...
                  {formData?.demos
                    ?.filter((i) => i?.options?.sort((a, b) => a.position - b.position).length > 0)
                    .map((d) => {
              ...
              <div className="col-lg-12 col-md-12 mt-4 bottoms-_btn-group">
                ...
                &nbsp;&nbsp;
                <button onClick={() => send("demographics", selectedOptions)} className="btn btn-sm btn-primary ml-2 float-l">
                  Save
                </button>
              </div>
            </div>
