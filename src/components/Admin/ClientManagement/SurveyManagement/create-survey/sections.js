...
import { CustomTreeColumn, TreeColumn } from "../../../../common/commonComponents/formItems";
...
const Sections = ({
  ...
  const SortableItemSections = sortableElement(({ value, sortable, id, survey_module_id }) => (
    <li className={`${sortable && "sections"} sort_item dark sections`}>
      {sortable && formData?.general?.status !== "live" && (
        <span
          onClick={() => {
            setSelectedSections(selectedSections?.filter((i) => i.survey_module_id !== id));
  ...
  const SortableItemPages = sortableElement(({ value, sortable, id, survey_module_id }) => (
    <li className={`${sortable && "sections"} sort_item dark sections`}>
      {sortable && formData?.general?.status !== "live" && (
        <span
          onClick={() => {
  ...
  const SortableItemQuestion = sortableElement(({ value, id, key, sortable }) => (
    <li className="sort_item dark sections">
      {sortable && formData?.general?.status !== "live" && (
        <span
          onClick={() => {
            setselectedQuestions(selectedQuestions.filter((i) => i.question_id !== id));
          }}
          className="delete iconx-trash-2"
        ></span>
      )}
  ...
  const onPageSortEnd = ({ oldIndex, newIndex }, key, survey_module_id) => {
    ...
    let removedUpdatedPages = selectedPages.filter((i) => i.survey_module_id !== survey_module_id);
  ...
  const onSectionSortEnd = ({ oldIndex, newIndex }, key, survey_module_id) => {
    let updatedSections = selectedSections.sort((a, b) => a.position - b.position);
  ...
  const onQuestionSortEnd = ({ oldIndex, newIndex }) => {
    let questionSet = selectedQuestions.filter((p) => p.survey_page_id === selectedPage).sort((a, b) => a.position - b.position);
    ...
    setselectedQuestions(
      questionsArr.sort((a, b) => a.position - b.position).sort((a, b) => a.survey_page_id - b.survey_page_id)
    );
  };
    ...
    setBuffer((pre) => {
      return {
        ...
        sections: dbToTree(initData?.sections)
          ...
          .map((i) => {
            return { ...i, disabled: initData?.sections?.filter((p) => p.id === i.key)[0]?.is_core };
          }),
      };
    });
  }, [initData]);
  ...
  const handlePageData = (sections) => {
    ...
    let sectionPagesToRemove = currentPageSectionIds?.filter((i) => !sections?.map((o) => o.survey_module_id).includes(i));
    let sectionPagesToAdd = sections?.map((i) => i.survey_module_id)?.filter((o) => !currentPageSectionIds.includes(o));

    sectionPagesToAdd = [...new Set(sectionPagesToAdd)];
    sectionPagesToRemove = [...new Set(sectionPagesToRemove)];

    let pages = selectedPages;
    pages = pages?.filter((i) => !sectionPagesToRemove.includes(i.survey_module_id));
    let newPages = [];
  ...
  const handleQuestionData = (pages) => {
    ...
    questions = questions.filter((i) => availablePageIds.includes(i.survey_page_id));
    ...
    pages.forEach((p) => {
      if (questions.filter((q) => q.survey_page_id === p.id).length === 0) {
        ...
        let pageQuestions = initData?.pages?.filter((w) => w.id === p.page_id)[0]?.questions;
  ...
  useEffect(() => {
    ...
    data && setSelectedSectionIds(data?.sections?.map((i) => i.survey_module_id));
      ...
      <div className="aspect-tab ">
        ...
        <div className="aspect-content">
          <div className="aspect-info">
            ...
            <span className="aspect-name">Additional Modules and Questions</span>
          </div>
        </div>
        <div className="">
          <div className="sentiment-wrapper">
            <div className="row clearfix">
              <div className="col-lg-4 col-md-4 mb-3 br-dark">
                <h2>Core module: {formData?.general?.core_module.capitalize()}</h2>
                <h6>Additional Modules</h6>
                <button onClick={() => setSections(true)} className="btn btn-outline-default btn-sm mt-3">
                    ...
                    selectedSections?.map((section, index) => {
                      return (
                        <SortableItemSections
                          ...
                          index={index}
                          value={initData?.sections?.filter((p) => p.id === section.survey_module_id)[0]?.name?.capitalize()}
                          id={section.survey_module_id}
                          survey_module_id={section.survey_module_id}
                          sortable={formData?.general?.survey_module_id !== section.survey_module_id}
                        />
                      );
                    })}
                </SortableContainer>
              </div>
              <div className={selectedPage ? `col-lg-4 col-md-4 mb-3 br-dark` : `col-lg-8 col-md-8 mb-3 br-dark`}>
                <h6>Pages</h6>
                  ...
                  {selectedSectionIds
                    ...
                    ?.map((s) => {
                      return (
                        <>
                          <div className="form-group sort_items">
                            <p className="mt-3">{initData?.sections?.filter((i) => i.id === s)[0]?.name}</p>
                            <SortableContainer
                              onSortEnd={(...props) => onPageSortEnd(...props, s)}
                              useDragHandle
                              helperclassName="sortableHelper"
                            >
                              {selectedPages
              ...
              {selectedPage && (
                <div className="col-lg-4 col-md-4 mb-3">
                  ...
                  <div className="form-group sort_items questions">
                    <SortableContainer onSortEnd={onQuestionSortEnd} useDragHandle helperclassName="sortableHelper">
                      {selectedQuestions
                        ...
                        .map((q, index) => {
                          return (
                            <SortableItemQuestion
                              ...
                              index={index}
                              id={q.question_id}
                              value={initData?.questions?.filter((i) => i.id === q.question_id)[0].heading}
                              sortable={true}
                            />
                          );
                        })}
