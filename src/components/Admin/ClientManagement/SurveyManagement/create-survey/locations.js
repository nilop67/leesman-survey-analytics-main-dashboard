...
import { CustomTreeColumn, CustomEditableCell } from "../../../../common/commonComponents/formItems";
...
const Locations = ({ initData, setInitData, surveyId, clientId, formData, setFormData, close, send, reset }) => {
  ...
  const [tailoredLocationGroupQuestion, setTailoredLocationGroupQuestion] = useState("What region are you primarily based in?");
  ...
  const [tailoredFloorQuestion, setTailoredFloorQuestion] = useState("What floor do you primarily work on?");
  ...
  const sendTailoredQuestions = () => {
    axios
      .put(`/admin/clients/${clientId}/surveys/${surveyId}`, {
        data: [tailoredLocationGroupQuestion, tailoredLocationQuestion, tailoredFloorQuestion],
    ...
    let col = [
      {
        ...
        render: (_, record) => {
          return (
            <Typography.Link
              onClick={() => {
                if (formData?.general?.status === "live" && moment(formData?.general?.end_date).isBefore(moment())) {
                  notification.warning({ message: "Cannot remove" });
                } else {
                  let locationFloors = initData?.locations?.filter((i) => i.parent_id === record.id).map((i) => i.id);
                  setSelectedLocations(selectedLocations.filter((i) => i !== record.id && !locationFloors.includes(i)));
                }
              }}
            >
              Remove
            ...
            <Typography.Link
              onClick={() => {
                if (formData?.general?.status === "live" && moment(formData?.general?.end_date).isBefore(moment())) {
                  ...
                  let locationFloors = initData?.locations?.filter((i) => i.parent_id === record.id).map((i) => i.id);
                  setSelectedLocations(selectedLocations.filter((i) => !locationFloors.includes(i)));
                }
              }}
            >
              Remove '
  ...
  const addAdditionalAttributes = () => {
    return selectedLocations?.map((i, index) => {
      ...
      initData.location_attributes.forEach((la) => {
        options[la.heading] = formData?.locationOptions?.filter((o) => o.location_id === i && o.question_id === la.id)[0]?.answer;
      });

      return {
        ...initData?.locations?.filter((i) => !i.is_location_group).filter((l) => l.id === i)[0],
        key: i,
        ...options,
      };
    });
  ...
  useEffect(() => {
    initData?.locations &&
      setBuffer((pre) => {
        return {
          ...pre,
          locations: dbToTree(initData?.locations?.filter((i) => !i.is_location_group)),
        };
      });
  ...
  const attributeCheck = (d) => {
    return d?.map((location) => {
      ...
      attributes.forEach((a) => {
        options.push({
          question: a.id,
          option_id: a.type === "dropdown" ? a?.options?.filter((o) => o.option_label === location[a.heading])[0] : null,
      ...
      return {
        ...location,
        parent_location: initData?.locations?.filter((i) => i.id === d.parent_id)[0]?.label,
        options: options,
      };
    });
  };
  ...
  const checkData = (data) => {
    ...
    data
      ...
      .map((i) => i.options)
      .forEach((optionSet) => {
        optionSet?.filter((o) => o?.required && !o.answer)?.length > 0 && missingAttributedLocationCount++;
      });
    if (data.filter((i) => i.options).length !== data.length) {
      //not edited at all

      notification.warning({ message: "Please update locations required attributes" });
          ...
          <Typography.Link
            onClick={() => {
              if (formData?.general?.status === "live" && moment(formData?.general?.end_date).isBefore(moment())) {
                ...
                setSelectedLocations(selectedLocations.filter((i) => i !== record.id));
              }
            }}
          >
            Remove
  ...
  return (
    <>
      <div className="drawer_sc-div create--sr" id="survey-_section" ref={drawerContainer}>
        <Drawer
          title="Location list"
          placement="right"
          width="30%"
        ...
        >
          <CustomTreeColumn
            ...
            treeData={buffer}
            count={1}
            nonRemovable={formData?.general?.status === "live" && moment(formData?.general?.end_date).isBefore(moment())}
          />
        </Drawer>{" "}
        <Drawer
      ...
      <div className="col-lg-12 col-md-12 without-btn-table">
        <nav className="nav nav-tabs" role="tablist">
          <a
            href="#!"
            className={`custom_tab nav-item nav-link ${tab === "locations" && " active"}`}
          ...
          <a
            href="#!"
            className={`custom_tab nav-item nav-link ${tab === "floors" && " active"}`}
            ...
            <div className="sentiment-wrapper">
              <div className="row clearfix">
                <div className="col-lg-12 col-md-12">
                  ...
                  <button onClick={() => setTailoredDrawer(true)} className="btn btn-outline-default btn-sm">
                  ...
                  <ImportExport
                    ...
                    surveyId={surveyId}
                  />
                  <button onClick={() => setLocations(true)} className="btn btn-outline-default btn-sm">
                    Select location(s)
                  </button>{" "}
                  <button
                    onClick={() => {
                      let d = [...selectedLocations, ...initData?.locations?.filter((i) => i.parent_id).map((i) => i.id)];
                  ...
                  <Form form={form} component={false}>
                    <Table
                      ...
                      bordered
                      dataSource={data?.filter((i) => !i.parent_id).sort((a, b) => a.position - b.position)}
                      columns={column}
        ...
        <>
          <div className="aspect-tab ">
            ...
            <div className="">
              <div className="sentiment-wrapper">
                <div className="row clearfix">
                  {!(formData?.general?.status === "live" && moment(formData?.general?.end_date).isBefore(moment())) && (
                    <div className="col-lg-12 col-md-12">
                      <button
                        onClick={() =>
                          setSelectedLocations(
                  ...
                  <div className="col-lg-12 col-md-12 mt-4 bottoms-_btn-group">
                    <Table
                      ...
                      dataSource={selectedLocations
                        ?.map((i) => {
                          let floor = initData?.locations?.filter((o) => o.id === i)[0];

                          if (floor?.parent_id) {
                            return {
                              id: i,
                              location: initData?.locations?.filter((o) => o.id === floor?.parent_id)[0]?.label,
                              floor: floor?.label,
                              position: initData?.locations?.filter((o) => o.id === floor?.parent_id)[0]?.position,
                            };
                          }
                        })
                        .filter((i) => i)
