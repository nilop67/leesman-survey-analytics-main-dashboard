...
const PageDrawer = ({
  ...
  const getPageData = (page_id) => {
    axios.get(`/admin/clients/${clientId}/surveys/pages/${page_id}`).then((res) => {
  ...
  useEffect(() => {
    questions?.length && customfilter(query, questions, ["question_label", "section", "id"], setBuffer);
  }, [query]);
  useEffect(() => {
    setBuffer(questions);
  }, [questions]);
  ...
  const onQuestionSortEnd = ({ oldIndex, newIndex }) => {
    setFormData((pre) => {
      return { ...pre, questions: arrayMove(formData?.questions, oldIndex, newIndex) };
    });
  };
    ...
    <>
      ...
      >
                ...
                <div className="form-group sort_items mt-3">
                  ...
                  <SortableContainer onSortEnd={onQuestionSortEnd} useDragHandle helperclassName="sortableHelper">
                    {formData?.questions?.map((value, index) => {
                      return <SortableItemQuestion key={`item-${index}`} index={index} value={value} />;
                    })}
                  </SortableContainer>
                </div>
              </div>
        ...
        <div className="dual--screen-bottom">
          ...
          <button type="button" onClick={() => send()} className="btn btn-sm btn-primary" style={{ marginLeft: "15px" }}>
            Save
          </button>
        </div>
        <Drawer
          ...
          <div className="row clearfix">
            <div className="col-lg-12 col-md-12">
              <div className="form-group">
                <select
                  onChange={(e) => {
                    setBuffer(questions.filter((i) => i.page_id === e.target.value));
                  }}
                      ...
                      <div className="col-lg-12">
                        <label className="fancy-checkbox">
                          <input
                            ...
                            checked={formData?.questions?.map((q) => q?.id).includes(i?.id)}
                            type="checkbox"
                            name="questions"
                          />
                          <span className="light-black">{i.question_label}</span>
                        </label>
                      </div>
                    );
                  })}{" "}
