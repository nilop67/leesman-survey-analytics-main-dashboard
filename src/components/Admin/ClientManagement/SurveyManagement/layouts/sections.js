...
const Sections = () => {
  ...
  const send = () => {
    if (selectedSection) {
      axios.put(`/admin/clients/${clientId}/surveys/sections/${selectedSection}`, formData).then(() => getData());
    } else {
      axios.post(`/admin/clients/${clientId}/surveys/sections`, formData).then(() => getData());
    }
  };
