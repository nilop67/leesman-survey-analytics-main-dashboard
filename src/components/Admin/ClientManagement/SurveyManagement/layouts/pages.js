...
const Pages = () => {
  ...
  useEffect(() => {
    data && customfilter(query, data, ["title", "name", "questions", "heading"], setBufferData);
  }, [query]);
  ...
  const send = () => {
    if (selectedPage) {
      axios.put(`/admin/clients/${clientId}/surveys/pages/${selectedPage}`, formData).then(() => getData());
    } else {
      axios.post(`/admin/clients/${clientId}/surveys/pages`, formData).then(() => getData());
    }
  };
