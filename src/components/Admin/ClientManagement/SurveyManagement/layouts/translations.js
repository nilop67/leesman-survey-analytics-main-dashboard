...
const Translations = () => {
  ...
  const expandedRowRender = (key) => {
    ...
    return (
      <Table
        ...
        dataSource={key.translations.sort((a, b) => a.language_id - b.language_id)}
        pagination={false}
      />
    );
  };
  ...
  useEffect(() => {
    isFilter ? setBufferData(bufferData.filter((i) => i?.translations?.length === 1)) : setBufferData(data);
  }, [isFilter]);
  const send = () => {
    axios
  ...
  const sendBulk = (id) => {
    let bulkData = data.filter((i) => (tab === "ui" ? i.label === id : i.id === id))[0].translations;

    axios
      .put(`/translations/copytranslation`, { tab: tab, id: currentTranslation, overWrite: overWrite, data: bulkData })
      .then((res) => {
        getData();
        notification.success({ message: "Updated" });
      });
            ...
            <div className="row clearfix">
                ...
                <div className="form-group">
                  {copyFrom?.length > 0 && (
                    <Popconfirm
                      ...
                      cancelText="No"
                    >
                      <button type="button" className="btn btn-sm btn-primary" style={{ marginLeft: "15px" }}>
                        Copy
                      </button>
                    </Popconfirm>
                  )}
              ...
              <div className="col-lg-12 col-md-12">
                <Table
                  ...
                  dataSource={data?.filter((i) => (tab === "ui" ? i.label === copyFrom : i.id === copyFrom))[0]?.translations}
        ...
        <div className="col-xl-12 col-lg-12 col-md-12">
          ...
          <ul className="nav nav-tabs3 mt-4 mb-4">
            <li className="nav-item">
              <a className={`nav-link ${tab === "ui" && " active"}`} onClick={() => setTab("ui")} data-toggle="tab" href="#!">
                Ui
              </a>
            </li>
            <li className="nav-item">
