...
import { customfilter, generateExcelByDataAndName } from "../../../../../util/functions";
...
const Surveys = ({ isCreateSurvey, setCreateSurvey, setSurveyId }) => {
  ...
  const [clientId, setClientId] = useState(params.get("client_id") ? params.get("client_id") : "0");
  ...
  useEffect(() => {
    data && customfilter(query, data, ["id", "client", "status", "title", "name", "description", "survey_type"], setBufferData);
  }, [query]);
  ...
  const getReport = (type, survey_id) => {
    ...
    axios.get(url).then((res) => {
      res.data?.length && generateExcelByDataAndName(`survey_${survey_id}_${type}_export`, res.data);
      getData();
    });
  };

  const getSurveyReport = async (survey_id) => {
    axios.get(`/admin/clients/${clientId}/surveys/${survey_id}/surveyreport`).then((res) => {
      console.log(res, res.data.url);
      if (res.data.url) {
        window.open(res.data.url, "_blank", "noopener,noreferrer");
      } else {
        res.data?.data && generateExcelByDataAndName(`survey_${survey_id}_general_export`, res.data.data);
        getData();
      }
    });
  };
  ...
  const columns = [
      ...
      render: (text, record) => (
        <a
          className={`
            ...
            `}
        >
          <Tag>{record.status === "live" && moment(record?.end_date).isBefore(moment()) ? "Closed" : text?.capitalize()}</Tag>
        </a>
      ),
    },
    {
      title: "Survey",
      dataIndex: "title",
      filterIcon: (filtered) => <SearchOutlined style={{ color: filtered ? "#2793ff" : undefined }} />,
      ...
      onFilterDropdownVisibleChange: (visible) => visible && setTimeout(() => searchInput.select(), 100),
...

    {
      ...
      render: (text) => <a>{text ? moment(text?.split("T")[0]).format("DD MMM YYYY") : ""}</a>,
        ...
        <>
          {
            <>
              <a
                ...
                title="Edit"
                onClick={() => {
                  history.push(`/create-edit-survey?survey_id=${record.id}&client_id=${record.client_id}`);
                }}
              >
                Edit survey
              </a>
          ...
          <div className="col-xl-12 col-lg-12 col-md-12">
            ...
            <div className="row mb-4 page-__header">
              <div className="col-xl-6 col-lg-6 col-md-6">
                <h2 className="card-title">{!isCreateSurvey ? "Surveys" : "Create - Edit Survey"}</h2>
              </div>
              ...
              {!isCreateSurvey ? (
                <div className="col-xl-6 col-lg-6 col-md-6 jus-end">
                  {clientId !== "0" && (
                    <a
                      onClick={() => history.push(`/create-edit-survey?client_id=${clientId}`)}
