...
const SectionDrawer = ({
  ...
  const getSectionData = (section_id) => {
    axios.get(`/admin/clients/${clientId}/surveys/sections/${section_id}`).then((res) => {
      console.log(res);
      setFormData((pre) => {
        return {
          ...pre,
                ...
                <div className="form-group sort_items mt-3">
                  ...
                  <SortableContainer onSortEnd={onPageSortEnd} useDragHandle helperclassName="sortableHelper">
                    {formData?.pages?.map((value, index) => (
                      <SortableItemPage key={`item-${value}`} index={index} value={value} />
                    ))}
                  </SortableContainer>
                </div>
              </div>
                  ...
                  return (
                    <label className="fancy-checkbox">
                      <input
                        ...
                        checked={formData?.pages?.map((q) => q?.id).includes(i?.id)}
                        type="checkbox"
                        name="pages"
                      />
