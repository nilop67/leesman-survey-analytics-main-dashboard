...
const CampaignAllData = () => {
  ...
  const {basicBufferData} = useSelector(store => store.CampaignBasicBufferAllData)
  const {bufferCommentData} = useSelector(store => store.CampaignBufferCommentAllData)
  const {bufferRates} = useSelector(store => store.CampaignBufferRatesAllData)
  ...
  const {heatBufferData} = useSelector(store => store.CampaignHeatBufferAllData)

  useEffect(() => {
    if (window.location.pathname === "/all-campaigns" && document.querySelector("#campaigns-_link") !== null) {
  ...
  useEffect(() => {
    query.length > 3 &&
      customfilter(query, commentData, ["locationType", "question", "section_label", "message"], basicBufferData);
  }, [query]);
          ...
          <div className="col-md-12 col-sm-12 mb-3">
            <ul className="nav">
              <li className="nav-item mr--8">
                <a
                  className={`btn btn-default btn-block chartcategory-btn ${dataType === "modules" ? "active" : ""}`}
              ...
              <li className="nav-item">
                <a
                  className={`btn btn-default btn-block chartcategory-btn ${dataType === "groups" ? "active" : ""}`}
              ...
              <div className="body min-h-431 campaign-score-body">
                ...
                <div className="row headline-stat-closed pt-80">
                  <div className="col-lg-6 col-sm-12 max-h-200">
                    <h4 className="font-16r text-white text-leftt">{stats?.responses}</h4>
                    <label className="mb-0 text-leftt">Responses</label>
                  </div>
                  <div className="col-lg-6 col-sm-12 max-h-200">
                    <h4 className="font-16r text-white  text-leftt">{stats?.respondents}</h4>
                    <label className="mb-0  text-leftt">Respondents</label>
                  </div>
                  <div className="col-lg-6 col-sm-12 max-h-200 pt-40">
                    <h4 className="font-16r text-white  text-leftt">{stats?.departments}</h4>
                    <label className="mb-0  text-leftt">Departments</label>
                  </div>
                  <div className="col-lg-6 col-sm-12 max-h-200 pt-40">
                    <h4 className="font-16r text-white  text-leftt">{stats?.locations}</h4>
          ...
          <div className="col-lg-8 col-md-12 leesman-section">
            <div className="card">
              <div className="body min-h-431">
                <div className="d-flex justify-content-between align-items-center">
                  <div>
                    <h2 className="card-title mb-0">{dataType === "modules" ? "Modules" : "Question groups"}</h2>
                  </div>
                </div>
                  ...
                  <>
                    <div className="row text-center campaigns-details">
                      <div className="col-lg-6">
                        <label className="mb-0 text-leftt bold-it pt-20">Highest response rates</label>
                        {loading ? (
                          <Loader />
                        ) : (
                          <div className="responses-_sc">
                            {rates && rates.questions && rates.questions && rates.questions.length >= 1 ? (
                              <>
                                {rates &&
                                  rates.questions &&
                                  rates.questions
                                    ...
                                    .map((i) => {
                                      return (
                                        <div className="response-section pb--23">
                                          <div className="row clearfix">
                                            <div className="col-6">
                                              <h6 className="score-title-left">{i.heading}</h6>
                                            </div>
                                            <div className="col-6">
                                              <h6 className="score-title-right">{i.rate + "%"}</h6>
                                            </div>
                                          </div>
                      ...
                      <div className="col-lg-6">
                        <label className="mb-0 text-leftt bold-it pt-20">Highest / lowest scores</label>
                        {loading ? (
                          <Loader />
                        ) : (
                          <div className="responses-_sc">
                                      ...
                                      return (
                                        <div className="response-section pb--23">
                                          <div className="row clearfix">
                                            ...
                                            <div className="col-6">
                                              <h6 className="score-title-right">{i.score + "%"}</h6>
                                            </div>
                                          </div>
                                          ...
                                          <div className="progress progress-xs">
                                            <div
                                              className={`progress-bar ${
                                                i.score > 50 ? "progress-bar-green" : "progress-bar-red"
                                              }`}
                                              role="progressbar"
                                              aria-valuenow={60}
                                              aria-valuemin={0}
                    ...
                    <div className="row text-center campaigns-details">
                      <div className="col-lg-12">
                        {loading ? (
                          ...
                          <div className="responses-_sc">
                            {rates && rates.sections && rates.sections && rates.sections.length >= 1 ? (
                              <>
                                {rates &&
                                  rates.sections &&
                                  rates.sections
                ...
                <div className="row text-center">
                  <div className="col-lg-12">
                    <ul className="nav">
                      ...
                      <li className="nav-item mr--8 category-margin">
                        <a
                          className={`btn btn-default btn-block chartcategory-btn ${categoryType === "internal" ? "active" : ""}`}
                          onClick={() => {
                      ...
                      <li className="nav-item mr--8 category-margin">
                        <a
                          className={`btn btn-default btn-block chartcategory-btn ${categoryType === "comments" ? "active" : ""}`}
                          onClick={() => {
                      ...
                      {categoryType === "comments" && campaignCommentsSearchPart(bufferCommentData, query, setQuery)}
                    </ul>

                    {categoryType === "trends" ? <p className="small-_info text-leftt pt-10">Scores over time</p> : ""}
                    {categoryType === "breakdown" ? (
                      <p className="small-_info text-leftt pt-10">Breakdown of the campaign by question</p>
                    ) : (
                      ""
                    )}
                    {categoryType === "internal" && (
                      <p className="small-_info text-leftt pt-10">Comparison of campaign data between different groups</p>
                    )}
                  ...
                  <div className="col-lg-3 pt-40">
                        ...
                        <>
                          <label className="fancy-radio min-w-168">
                            <input
                              ...
                              name="chart-type"
                              value="bar-chart"
                              checked={chartType === "bar-chart" ? "checked" : ""}
                            />
                            <span className="light-black">
                              <i></i>Question positivity
                            </span>
                          </label>
                          <label onClick={() => setChartType("stacked-chart")} className="fancy-radio min-w-168">
                            <input
                              type="radio"
                              name="chart-type"
                              value="stacked-chart"
                              checked={chartType === "stacked-chart" ? "checked" : ""}
                            />
                            <span className="light-black">
                        ...
                        <>
                          <label className="fancy-radio min-w-168">
                            <input
                              ...
                              name="chart-type"
                              value="heat-map"
                              checked={chartType === "heat-map" ? "checked" : ""}
                            />
                            <span className="light-black">
                              <i></i>Respondent breakdown
                            </span>
                          </label>
                          <label onClick={() => setChartType("range-scores")} className="fancy-radio min-w-168">
                            <input
                              type="radio"
                              name="chart-type"
                              value="range-scores"
                              checked={chartType === "range-scores" ? "checked" : ""}
                            />
                            <span className="light-black">
                              <i></i>Internal range
                            </span>
                      ...
                      <>
                        ...
                        <div className="col-lg-12 col-md-12 pt-10">
                          <div className="form-group text-leftt">
                            <label className="text-leftt light-black">Sections</label>
                            <select
                            ...
                            >
                              ...
                              {[...new Set(basicBarData?.sections)].map((item, index) => {
                                return (
                    ...
                    {categoryType === "breakdown" && (
                      <div className="col-lg-12 col-md-12">
                        <div className="form-group text-leftt">
                          <label className="text-leftt light-black">Order by</label>
                          <select onChange={(e) => setOrderBy(e.target.value)} value={orderBy} className="form-control">
                  ...
                  {chartType !== "comments" ? (
                    campaignCommonPart(chartType, loading, orderBy, basicBufferData, heatBufferData, bufferRates)
                  ) : (
                    <div className="col-lg-12 without-btn-table">
                      {chartType === "comments" ? (
                        <Table
                          pagination={({ position: ["none", "bottomLeft"] }, (itemRender = { itemRender }))}
