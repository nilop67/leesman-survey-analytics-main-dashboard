...
import { notification, Table, Checkbox, Input, Button, Space, Tooltip } from "antd";
...
import {userManagementDepartmentsAction, userManagementLocationsAction, userManagementUsersAction, userManagementDomainHeaderAction} from '../../actions/adminActions'


const UserManagement = () => {
  const dispatch = useDispatch()

  const {departments} = useSelector(store => store.userManagementDepartments)
  ...
  useEffect(() => {
    customfilter(
      query,
      users,
      ["id", "client_name", "email", "first_name", "last_name", "department_label", "location_label", "role"],
      setBufferUsers
    );
  }, [query]);
  ...
  useEffect(() => {
    !loading &&
      domains.length === 0 &&
      clientId !== 0 &&
      notification.warning({ message: "To add a user, please add a domain first." });
  ...
  const saveUser = () => {
    if (editing) {
      ...
      if (!validator.isEmail(formData.email)) {
        ...
        notification.warning({ message: "You cannot create user with that domain!" });
      } else {
        // POST
  ...
  const confirmAnonymizeUser = () => {
    ...
    API.put("CoreAPI", `${urlHeader}/users/${id}`, { body: { action: "anonymize" } })
      .then(() => {
        setChanged(Date.now());
        setAnonymizingUser({});
        setOpenModal("");
      ...
      render: (user, record) => (
        <>
          <Tooltip title="Edit user">
            <button onClick={() => editUser(user)} className="btn btn-sm btn-liste cursorp">
              <span className="iconx-pencil"></span>
            </button>
          </Tooltip>
          {
            <Tooltip title="Delete user">
              <button onClick={() => deleteUser(user)} className="btn btn-sm btn-liste cursorp">
                <span className="iconx-trash"></span>
              </button>
            </Tooltip>
          }
          <Tooltip title="Anonymize user">
            <button onClick={() => anonymizeUser(user)} className="btn btn-sm btn-liste cursorp">
  ...
  return (
    <>
          ...
          <div className="col-xl-12 col-lg-12 col-md-12">
            <div className="row mb-4 page-__header">
              ...
              <div className="col-xl-6 col-lg-6 col-md-6 jus-end">
                <button onClick={() => setChanged(Date.now())} title="Refresh" className="btn btn-primary icon-_btn">
                  <i className="iconx-refresh" />
                </button>

                {(clientId !== 0 || role === "Admin") && (
                  <button onClick={() => setOpenModal("user")} className="btn btn-primary ml-3">
                    New user
                  </button>
                )}
                <ImportExport refresh={getData} type={"users"} data={bufferUsers} import={false} export={true} />
                <div className="input-group ml-3">
                  ...
                  <input value={query} onChange={changeQuery} type="text" className="form-control" placeholder="Search" />
                </div>
              </div>
            </div>
          </div>
      ...
      <div className={`modal fade bd-example-modal-lg ${openModal === "user" ? " d-block show" : ""}`}>
        <div className="modal-dialog modal-lg modal-dialog-centered">
          <div className="modal-content modal-border-w" style={{ padding: "15px" }}>
            <div className="row clearfix">
              <div className="col-lg-12 col-md-12 col-sm-12">
                <div className="card">
                  <h2 className="card-title mb-0">{editing ? "Edit" : "Add"} user</h2>
                </div>
              </div>
              ...
              <div className="col-lg-6 col-md-6 col-sm-6">
                <div className="form-group">
                  <select
                    ...
                    onChange={change}
                    required={true}
                    value={formData.department_id || editingUser.department_id || ""}
                  >
              ...
              <div className="col-lg-6 col-md-6 col-sm-6">
                <div className="form-group">
                  <select
                    ...
                    onChange={change}
                    required={true}
                    value={formData.location_id || editingUser.location_id || ""}
                  >
              ...
              <div className="col-12 pt-40">
                <button type="button" data-dismiss="modal" onClick={saveUser} className="btn btn-sm btn-primary">
                  Save
                </button>
                <button
                  type="button"
      ...
      {/* Delete User */}
      <div className={`modal fade ${openModal === "delete" ? " d-block show" : ""}`}>
        <div className="modal-dialog modal-dialog-centered">
          <div className="modal-content modal-border-w" style={{ padding: "15px" }}>
            <div className="row clearfix">
              ...
              <div className="col-12 pt-40">
                <button type="button" data-dismiss="modal" onClick={confirmDeleteUser} className="btn btn-sm btn-primary">
                  Confirm
                </button>
                <button
                  type="button"
      ...
      {/* Anonymize User */}
      <div className={`modal fade ${openModal === "anonymize" ? " d-block show" : ""}`}>
        <div className="modal-dialog modal-dialog-centered">
          <div className="modal-content modal-border-w" style={{ padding: "15px" }}>
            <div className="row clearfix">
              <div className="col-lg-12 col-md-12 col-sm-12">
              ...
              <div className="col-12 pt-40">
                <button type="button" data-dismiss="modal" onClick={confirmAnonymizeUser} className="btn btn-sm btn-primary">
                  Confirm
                </button>
                <button
                  type="button"
