...
import { ActivityContent, HomeContent, SpesContent, BufContent, DensContent, PieCContent } from "./components/info_contents";
...
const HOC = ({ setLoginBackground }) => {
  ...
  const change = (e) => {
    ...
    let val = name !== "headcount" && name !== "costPerSqm" ? (value > 100 ? 100 : value < 1 ? 0 : value) : value;
  ...
  useEffect(() => {
    setIsRolePercentage(
      checkPercentage([parseInt(formData.individualRole), parseInt(formData.balancedRole), parseInt(formData.collaborativeRole)])
    );
    setIsWorkPercentage(checkPercentage([parseInt(formData.workArea), parseInt(formData.workRoom), parseInt(formData.nonWork)]));
  }, [formData]);
  useEffect(() => {
  ...
  const calculate = (data) => {
    ...
    const avgDaysInWorkplace =
      remoteLearning > 0.0 || officeLearning > 0.0
        ? parseFloat(remoteLearning) * 0.5 + parseFloat(balancedHybrid) * 2.5 + parseFloat(officeLearning * 4.5)
        : 0.0;
    ...
    const headTimeAvgDays = (parseFloat(avgDaysInWorkplace) * parseFloat(data.headcount)) / 100.0;
    ...
    const getPeakLoad = (row) => {
      if (parseFloat(avgDaysInWorkplace) >= 4.5) {
        return row[5];
      } else if (parseFloat(avgDaysInWorkplace) < 4.5 && parseFloat(avgDaysInWorkplace) > 3.5) {
        return row[4];
      } else if (parseFloat(avgDaysInWorkplace) < 3.5 && parseFloat(avgDaysInWorkplace) > 2.5) {
        return row[3];
      } else if (parseFloat(avgDaysInWorkplace) < 2.5 && parseFloat(avgDaysInWorkplace) > 1.5) {
        return row[2];
      } else if (parseFloat(avgDaysInWorkplace) < 1.5 && parseFloat(avgDaysInWorkplace) > 0.5) {
        return row[1];
    ...
    const costTable = [
      {
        ...
        "Level load": estimatedWorkplaceLoad[0]["Level Load"] * MultiplyBy(data.buffer),
        "Level cost": "",
        "Peak load": getArrayMax(estimatedWorkplaceLoad.map((i) => parseFloat(i["Peak Load"]))) * MultiplyBy(data.buffer),
        "Peak cost": "",
      },
      {
        title: "Net leasable area (NLA, m2)",
        "Level load": estimatedWorkplaceLoad[0]["Level Load"] * MultiplyBy(data.buffer) * parseFloat(data.workspaceDensity),
        "Level cost":
        ...
        "Peak load":
          getArrayMax(estimatedWorkplaceLoad.map((i) => parseFloat(i["Peak Load"]))) *
          MultiplyBy(data.buffer) *
          parseFloat(data.workspaceDensity),
        "Peak cost":
          getArrayMax(estimatedWorkplaceLoad.map((i) => parseFloat(i["Peak Load"]))) *
      ...
      },
      {
        title: "M2 / person",
        "Level load":
          (estimatedWorkplaceLoad[0]["Level Load"] * MultiplyBy(data.buffer) * parseFloat(data.workspaceDensity)) /
      ...
      },
      {
        ...
        "Level load": parseFloat(data.headcount) / (estimatedWorkplaceLoad[0]["Level Load"] * MultiplyBy(data.buffer)),
        "Level cost": "",
        "Peak load":
          parseFloat(data.headcount) / (getArrayMax(estimatedWorkplaceLoad.map((i) => i["Peak Load"])) * MultiplyBy(data.buffer)),
        "Peak cost": "",
      },
    ];
    ...
    const others = {
      ...
      "Cost saved":
        ...
        estimatedWorkplaceLoad[0]["Level Load"] *
          ...
          parseFloat(data.workspaceDensity) *
          parseFloat(data.costPerSqm),
      "Max peak load": getArrayMax(estimatedWorkplaceLoad.map((i) => i["Peak Load"])) * MultiplyBy(data.buffer),
      "Min peak load": estimatedWorkplaceLoad[0]["Level Load"] * MultiplyBy(data.buffer),
      ...
      "Difference in workspaces (Peak vs Level load)":
        getArrayMax(estimatedWorkplaceLoad.map((i) => i["Peak Load"])) * MultiplyBy(data.buffer) -
    ...
    setTableData({
      ...
      estimatedPeopleCountBasedOnHeadcount: estimatedPeopleCountBasedOnHeadcount,
  ...
  const checkExportInputs = () => {
    ...
    if (exportData.name.length < 3) {
      arr.push({ type: "name", message: "Please enter a valid name with more then 3 characters" });
    }
    ...
    if (exportData.company.length < 2) {
      arr.push({ type: "company", message: "Please enter a valid company name with more then 2 characters" });
    }
    if (exportData.title.length < 3) {
      arr.push({ type: "title", message: "Please enter a valid title more then 3 characters" });
    }
    setErrors(arr);
    if (arr.length === 0) {
  ...
  return (
    <>
          ...
          <StyleRoot style={styles.fadeInUp} className="no_-scrl">
            <div className="modal-_export-container">
              <div className="row clearfix">
                ...
                <div className="col-lg-12 col-md-12">
                  <p>To receive a copy of your report, please confirm your details:</p>
                </div>
                <div className="col-lg-12 col-md-12 mt-3">
                  ...
                  {errors.filter((i) => i.type === "name").length > 0 && (
                    <p className="text-danger rer mb-0 mt-0">
                      <strong>{errors.filter((i) => i.type === "name")[0].message}</strong>
                    </p>
                  )}
                </div>
                <div className="col-lg-12 col-md-12">
                  ...
                  {errors.filter((i) => i.type === "email").length > 0 && (
                    <p className="text-danger rer mb-0 mt-0">
                      <strong>{errors.filter((i) => i.type === "email")[0].message}</strong>
                    </p>
                  )}
                </div>
                <div className="col-lg-12 col-md-12">
                  ...
                  {errors.filter((i) => i.type === "company").length > 0 && (
                    <p className="text-danger rer mb-0 mt-0">
                      <strong>{errors.filter((i) => i.type === "company")[0].message}</strong>
                    </p>
                  )}
                </div>
                <div className="col-lg-12 col-md-12">
                  ...
                  {errors.filter((i) => i.type === "title").length > 0 && (
                    <p className="text-danger rer mb-0 mt-0">
                      <strong>{errors.filter((i) => i.type === "title")[0].message}</strong>
                    </p>
                  )}
                </div>
                <div className="col-lg-12 col-md-12">
                  <p>
                    I agree to receive communications from Leesman and for my personal information being handled in accordance
                    with the Leesman{" "}
                    <a className="prvcl" href="https://www.leesmanindex.com/privacy-policy/" target="_blank">
                      privacy policy
                    </a>
      ...
      <div id="main-content" className={`hoc-_main mt-0 mb-0 ${isStart && " start-clc"}`}>
            ...
            {isStart ? (
              <>
                <div className="col-xl-5 col-lg-5 col-md-12 col-sm-12 left-_l">
                  <div className="card">
                    <div className="body trp">
                      ...
                      <p>
                        The Hybrid Occupancy Calculator uses insights from our global reach research to show quite how important
                        it is to know what your employees do in their roles, before deciding the optimal location for them. It
                        then aims to give a sense of what that might mean for your real estate. Numerous other factors will
                        ultimately need to be considered, like commuting distances and mode of travel, but this model aims to
                        illustrate the influence an employee’s role has on the adoption of hybrid working, possible weekly
                        utilisation high and low tide marks and the impact of load balancing.
                      </p>
                      <p>Start exploring your space options now.</p>

                      <button type="button" className="btn calc-btn mt-4" onClick={letStart}>
                        Start
                      </button>
                    </div>
                  </div>
                ...
                <div className="col-xl-7 col-lg-7 col-md-12 col-sm-12 left-_r">
                  <div className="card">
                    <div className="body trp">
                      <img className="hoc-main-bg" src={MainImage} alt="leesman hybrid occupancy calculator main image"></img>
                    </div>
                  </div>
                </div>
              </>
              ...
              <>
                ...
                {isSidebar && <div className="calc-_sidebar-overlay" onClick={() => setSidebar(false)}></div>}

                <div className={`calc-_sidebar ${!isSidebar && " closed"}`}>
                  <span onClick={() => setSidebar(false)} className="calc-_sidebar-cls">
                    X
                  </span>
                  <div className="card">
                    <div className="body">
                      <div className="header">
                        ...
                        <p>
                          Please scroll down to input your People and Space data by adjusting the fields below. If your portfolio
                          includes multiple workplaces, please think of the most representative one.
                        </p>
                      </div>
                      <div className="row clearfix">
                    ...
                    {selectedTab === "People" ? (
                      <>
                        <div className="body mt-0">
                          ...
                          <p className="d-block w-100 mb-4">Percentages must add up to 100</p>
                          <div className="row clearfix">
                            <div className="col-lg-12 col-md-12">
                              <SliderPercentage
                                name="individualRole"
                        ...
                        <div className="body mt-0">
                          <div className="item-_clc mb-2">
                            <h3 className="text-muted">Specific activities & equipment</h3>
                              ...
                              <label className="form-group ff">
                                ...
                                <div className="input-group">
                                  <input
                                    ...
                                    onChange={change}
                                    value={removeZero(formData.privateConversations)}
                                    type="number"
                                    className="form-control"
                                    min={1}
                                    max={100}
                              ...
                              <label className="form-group ff">
                                ...
                                <div className="input-group">
                                  <input
                                    ...
                                    onChange={change}
                                    value={removeZero(formData.businessConfidential)}
                                    type="number"
                                    className="form-control"
                                    placeholder="0"
                                    min={1}
                        ...
                        <div className="body mt-0">
                          ...
                          <div className="row clearfix">
                            <div className="col-lg-12 col-md-12">
                              <SliderDensity name="workspaceDensity" setFormData={setFormData} formData={formData} />
                            </div>
                          </div>
                        </div>
                        <div className="body mt-3">
                          ...
                          <div className="row clearfix">
                            <div className="col-lg-12 col-md-12">
                              <SliderBuffer name="buffer" setFormData={setFormData} formData={formData} />
                            </div>
                          </div>
                        </div>
                        <div className="body mt-3">
                        ...
                        <div className="body mt-0">
                          <div className="row clearfix">
                            ...
                            <div className="col-lg-12 col-md-12 d-flex">
                              <button type="button" className="btn calc-btn mb-2 hidden-_mobile" onClick={() => setModal(true)}>
                                Export my data
                              </button>
                              <button
                                type="button"
                        ...
                        <div className="col-lg-12">
                          <div className="card">
                            <div className="body min-h-450">
                              <div className="item-_clc">
                                <h3 className="text-muted mb-0">Hybrid profile distribution</h3>
                              ...
                              <span className="text-muted d-block ">
                                ...
                                <br /> Percentages may not add up to 100 due to rounding.
                              </span>

                              <PieChartHOC data={chartData?.pie} height={360} offsetY={"responsive"} />
                            </div>
                          </div>
                        </div>
                        <div className="col-lg-12">
                              ...
                              <div className="row">
                                  ...
                                  <div className="card">
                                    <div className="body txt-align-bottom min-h-173">
                                      <div className="card-_inf">
                                        <h3 className="text-muted mb-0">Avg number of days in workplace</h3>
                                        <span className="text-muted d-block mb-4">&nbsp;&nbsp;</span>
                                      </div>
                                      <div className="m-0 text-center h1">
                                        {parseFloat(tableData?.others?.avgDaysInWorkplace).toFixed(1)}
                                      </div>
                                    </div>
                                  </div>
                                </div>
                                <div className="col-lg-12">
                                  <div className="card">
                                    <div className="body txt-align-bottom min-h-173">
                                      <div className="card-_inf">
                                        <h3 className="text-muted mb-0">Net leasable area (SqM)</h3>
                                        ...
                                        <div className="hwa-item_c d-flex align-items-center mb-2">
                                          <div className="ml-0 flex-_that">
                                            <h4 className="mb-0 font-weight-medium">
                                              {numFormatter(
                                                parseFloat(
                                                  tableData?.costTable?.filter((i) =>
                                                    i.title.includes("Net leasable area (NLA, m2)")
                                                  )[0]["Peak load"],
                                                  1
                                                ).toFixed(1)
                                              )}
                              ...
                              <div className="row">
                                <div className="col-lg-12">
                                  <div className="card">
                                    <div className="body txt-align-bottom min-h-173">
                                      <div className="card-_inf">
                                        <h3 className="text-muted mb-0">SqM / person</h3>
                                        <span className="text-muted d-block mb-2">&nbsp;&nbsp;</span>
                                      </div>
                                      <div className="d-block align-items-center hwa-__items">
                                        <div className="hwa-item_c d-flex align-items-center mb-2">
                                          <div className="ml-0 flex-_that">
                                            <h4 className="mb-0 font-weight-medium">
                                              {numFormatter(
                                                parseFloat(
                                                  tableData?.costTable?.filter((i) => i.title.includes("M2 / person"))[0][
                                                    "Peak load"
                                                  ],
                                                  1
                                                ).toFixed(1)
                                ...
                                <div className="col-lg-12">
                                  <div className="card">
                                    <div className="body txt-align-bottom min-h-173">
                                      <div className="card-_inf">
                                        <h3 className="text-muted mb-0">Desk sharing ratio</h3>
                                        <span className="text-muted d-block mb-2">&nbsp;&nbsp;</span>
                                      </div>
                                      <div className="d-block align-items-center hwa-__items">
                                        <div className="hwa-item_c d-flex align-items-center mb-2">
                                          <div className="ml-0 flex-_that">
                                            <h4 className="mb-0 font-weight-medium">
                                              {numFormatter(
                                                parseFloat(
                                                  tableData?.costTable?.filter((i) => i.title.includes("Desk sharing ratio"))[0][
                                                    "Peak load"
                                                  ],
                                                  1
                                                ).toFixed(1)
                    ...
                    <div className="col-xl-8 col-lg-12 col-md-12 col-sm-12 right-_s">
                      <div className="row">
                        <div className="col-lg-12">
                          <div className="card">
                            <div className="body min-h-450">
                              <button onClick={() => setSidebar(true)} type="button" className="btn btn-primary ex-_p ww mt-4">
                                <span className="iconx-sound-mix"></span> Calculator
                              </button>
                              ...
                              <h3 className="text-muted mb-0">Estimated workplace utilisation</h3>
                              <span className="text-muted d-block mb-4">
                                Number of employees likely to use the workplace on any given day if no fixed occupancy
                                <br /> rules are in place (Expected load) or if balancing rules are enforced (Level load)
                              </span>
                              {chartData?.load?.length > 0 && <BarChartHOC data={chartData?.load} height={300} />}
                            </div>
                          </div>
                        </div>
                        <div className="col-lg-12">
                              ...
                              <div className="row">
                                <div className="col-lg-12">
                                  <div className="card">
                                    <div className="body txt-align-bottom min-h-173">
                                      <div className="card-_inf">
                                        <h3 className="text-muted mb-0">Cost per individual workspace</h3>
                                        <span className="text-muted d-block mb-4">Annual cost for each individual workspace</span>
                                      </div>
                                      <div className="m-0 text-center h1">
                                        £
                                        {numFormatter(
                                          tableData?.others && parseFloat(tableData.others["Cost per desk"]).toFixed(0),
                                          1
                                        ) || "0"}
                                      </div>
                                    </div>
                                ...
                                <div className="col-lg-12">
                                  <div className="card">
                                    <div className="body txt-align-bottom min-h-173">
                                      <div className="card-_inf">
                                        <h3 className="text-muted mb-0">Cost difference (Busiest day vs level load)</h3>
                                        <span className="text-muted d-block mb-2">
                                          Cost saving between the workspaces needed for the busiest expected day compared to the
                                          level load
                                        </span>
                                      </div>
                                      <div className="m-0 text-center h1">
                                        £
                                        {numFormatter(
                                          tableData?.others && parseFloat(tableData.others["Cost saved"] / 100.0).toFixed(0),
                                          1
                                        ) || "0"}
                                      </div>
                                    </div>
                              ...
                              <div className="row">
                                <div className="col-lg-12">
                                  <div className="card">
                                    <div className="body txt-align-bottom min-h-173">
                                      <div className="card-_inf">
                                        <h3 className="text-muted mb-0">Workspaces required</h3>
                                        <span className="text-muted d-block mb-4">
                                      ...
                                      <div className="d-flex align-items-center hwa-__items">
                                        <div className="hwa-item_c d-flex align-items-center border-right pr-5 first-itm">
                                          <div className="ml-0">
                                            <h4 className="mb-0 font-weight-medium">
                                              {tableData?.others &&
                                                numFormatter(parseFloat(tableData?.others["Max peak load"]).toFixed(0), 0)}
                                            </h4>
                                            <span>Exp. load</span>
                                          </div>
                                        </div>
                                        <div className="hwa-item_c d-flex align-items-center border-right pr-5 ml-5">
                                          <div className="ml-0">
                                            <h4 className="mb-0 font-weight-medium">
                                              {tableData?.others &&
                                                numFormatter(parseFloat(tableData?.others["Min peak load"]).toFixed(0), 0)}
                                            </h4>
                                          ...
                                          <div className="ml-0">
                                            <h4 className="mb-0 font-weight-medium">
                                              {tableData?.others &&
                                                numFormatter(
                                                  parseFloat(
                                                    tableData.others["Difference in workspaces (Peak vs Level load)"],
                                                    0
                                                  ).toFixed(0)
                                                )}
                                            </h4>
                                ...
                                <div className="col-lg-12">
                                  <div className="card">
                                    <div className="body txt-align-bottom min-h-173">
                                      <div className="card-_inf">
                                        <h3 className="text-muted mb-0">Workspace differential</h3>
                                        <span className="text-muted d-block mb-4">
                                      ...
                                      <div className="d-flex align-items-center hwa-__items">
                                        <div className="hwa-item_c d-flex align-items-center border-right pr-5 first-itm">
                                          <div className="ml-0">
                                            <h4 className="mb-0 font-weight-medium">
                                              {tableData?.others &&
                                                numFormatter(parseFloat(tableData.others.Busiest).toFixed(0), 0)}
                                            </h4>
                                            <span>Busiest</span>
                                          </div>
                                        </div>
                                        <div className="hwa-item_c d-flex align-items-center border-right pr-5 ml-5">
                                          <div className="ml-0">
                                            <h4 className="mb-0 font-weight-medium">
                                              {tableData?.others &&
                                                numFormatter(parseFloat(tableData.others.Quietest).toFixed(0), 0)}
                                            </h4>
                                            <span>Quietest</span>
                                          </div>
                                        </div>
                                          ...
                                          <div className="ml-0">
                                            <h4 className="mb-0 font-weight-medium">
                                              {tableData?.others &&
                                                numFormatter(
                                                  parseFloat(
                                                    tableData.others["Difference in workspaces (Busiest day vs quietest day)"],
                                                    0
                                                  ).toFixed(0)
                                                )}
                                            </h4>
