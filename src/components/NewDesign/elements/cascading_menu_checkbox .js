...
const CascadingMenuCheckbox =({data, setTopSelectFlag, isTopSelectFlag, label, id})=> {
    const dataList = [];
  ...
  return (
    <>
    {isTopSelect && isTopSelectFlag === id ? <div onClick={()=>setTopSelect(false)} className="n_modal_bg trans"></div> : <></>}

    <div className={`drop_dash_container ${isTopSelect && isTopSelectFlag === id ? " show" : ""} `} key={id}>
        <button 
        className="btn-dash drop has-icn cascading_select_item" 
        onClick={()=>{setTopSelect(isTopSelectFlag === id ? !isTopSelect : true);setTopSelectFlag(id)}}
        >
            {label}
        ...
        <div className="drop_dash-menu">
            ...
            <button onClick={startRefreshing} className={`btn-dash drop has-icn left-icn bb float-left refresh ${isRefreshing ? " refreshing" : " "} `}>
