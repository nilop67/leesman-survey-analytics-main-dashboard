...
const DropMenuRadio =({data, setTopSelectFlag, isTopSelectFlag})=> {
  ...
  return (
    <>
    <div className={`drop_dash_container ${isTopSelect && isTopSelectFlag === data.label ? " show" : ""} `} key={data.key}>
        <button className="btn-dash drop has-icn drop_radio_item" onClick={()=>{setTopSelect(isTopSelectFlag === data.label ? !isTopSelect : true);setTopSelectFlag(data.label)}} ref={ref_drop_radio_item}>
            {data.label}
