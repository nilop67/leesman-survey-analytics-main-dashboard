...
const Index =({ setLoginBackground })=> {
  ...
  return (
    ...
    <div className="n_menu_horizontal header support">
        <div className="left">
            <a href="javascript:void(0)" className={`${isPage === "Office" ? ' active' : ' '}`} onClick={()=>setPage("Office")}>Office</a>
            <a href="javascript:void(0)" className={`${isPage === "Home" ? ' active' : ' '}`} onClick={()=>setPage("Home")}>Home</a>
            <a href="javascript:void(0)" className={`${isPage === "Hybrid" ? ' active' : ' '}`} onClick={()=>setPage("Hybrid")}>Hybrid</a>
        </div>
        <div className="right">
            <button className="btn" onClick={()=>window.print()} ><span className="iconx-printer"></span> Print</button>
            <button className="btn" onClick={() => history.goBack()}><span className="iconx-corner-up-left"></span> Back</button>
        </div>
    </div>


    <LoaderPage/>
    <div id="element-to-print" className={`${(navigator.userAgent.indexOf("Firefox") !== -1 ) || (typeof InstallTrigger !== 'undefined') ? ' ' : ' hvv_h'}`}>
        {isPage === "Office" && <Office 
                                isPage={isPage}
                                totalPages={12}
                                />}
