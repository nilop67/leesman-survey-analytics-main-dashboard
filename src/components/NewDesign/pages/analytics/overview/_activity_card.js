        ...
        <ul>
            ...
            {data.map((item) => (
                ...
                <li className={`bordered ${item.left_color} `}>
                    <span className="n_result n_strong xl"><div className="result_card"><span className="up_to">{item.activity_label}</span><span className="percentage">{item.activity_value}%</span></div></span>
                    <span className="n_result n_strong xl">{item.no}</span>
                    <span className={`n_result n_strong green xl ${item.lmi_color} `}>{item.lmi}</span>
                    <span className={`n_result n_strong xl ${item.hlmi_color} `}>{item.hlmi}</span>
                </li>
                </>
            ))}
        </ul>
