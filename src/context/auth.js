...
const useAuth = () => {
  ...
  const fetchUser = async () => {
    Auth.currentAuthenticatedUser()
      .then((user) => {
        ...
        const role = user.signInUserSession && user.signInUserSession.idToken.payload.role;
        console.log("Role", role);
        const perm_list = user.signInUserSession && user.signInUserSession.idToken.payload.permissions;
    ...
    API.get("CoreAPI", "/auth/identityprovider", {
      ...
      .then((res) => {
        if (!res.result || !res.result.providerName) {
          ...
          Auth.federatedSignIn({ provider: res.result.providerName }).catch(console.error);
        }
      })
      .catch(console.error);
  };
  ...
  const completeNewPassword = (password) => {
    return Auth.completeNewPassword(state.user, password).then(() => fetchUser());
  };
...
const bucketName = process.env[`REACT_APP_s3_${process.env.REACT_APP_ENVIRONMENT}_bucket_name`];
...
const axiosSetDefaults = (token) => {
  ...
  axios.defaults.headers.post["Access-Control-Allow-Methods"] = "POST,GET,OPTIONS,DELETE,PUT";
